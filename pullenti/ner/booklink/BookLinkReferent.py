...
class BookLinkReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang_ : 'MorphLang'=None, lev : int=0) -> str:
        res = io.StringIO()
            ...
            for s in self.slots: 
                if (s.type_name == BookLinkReferent.ATTR_AUTHOR): 
                    ...
                    if (isinstance(s.value, Referent)): 
                        print(s.value.to_string_ex(True, lang_, lev + 1), end="", file=res)
    ...
    def year(self) -> int:
        """ Год """
        year_ = 0
        wrapyear504 = RefOutArgWrapper(0)
        inoutres505 = Utils.tryParseInt(Utils.ifNotNull(self.get_string_value(BookLinkReferent.ATTR_YEAR), ""), wrapyear504)
        year_ = wrapyear504.value
        if (inoutres505): 
            return year_
        else: 
            return 0
