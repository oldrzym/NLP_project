...
class BookLinkAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (re.number is not None): 
                li = [ ]
                wrapli494 = RefOutArgWrapper(None)
                inoutres495 = Utils.tryGetValue(refs_by_num, re.number, wrapli494)
                li = wrapli494.value
                if (not inoutres495): 
                    li = list()
        ...
        while True:
            ...
            li = [ ]
            wrapli496 = RefOutArgWrapper(None)
            inoutres497 = Utils.tryGetValue(refs_by_num, Utils.ifNotNull(re.number, ""), wrapli496)
            li = wrapli496.value
            if (not inoutres497): 
                continue
            i = 0
    ...
    def __try_parse(t : 'Token', is_in_lit : bool, max_char : int=0) -> typing.List['ReferentToken']:
        ...
        next_num = None
        nn = 0
        wrapnn502 = RefOutArgWrapper(0)
        inoutres503 = Utils.tryParseInt(Utils.ifNotNull(num, ""), wrapnn502)
        nn = wrapnn502.value
        if (inoutres503): 
            next_num = str((nn + 1))
        ...
        if (tn1 is None and start_of_name is None): 
            if (is_electr_res): 
                ...
                rts0 = list()
                bref0 = BookLinkRefReferent._new498(uri_re)
                if (num is not None): 
                    bref0.number = num
            ...
            if (decree is not None and num is not None): 
                rts0 = list()
                bref0 = BookLinkRefReferent._new498(decree.get_referent())
                if (num is not None): 
                    bref0.number = num
            ...
            if (book_prev is not None): 
                ...
                if (blt0 is not None and blt0.typ == BookLinkTyp.PAGERANGE): 
                    rts0 = list()
                    bref0 = BookLinkRefReferent._new498(book_prev)
                    if (num is not None): 
                        bref0.number = num
        ...
        for rr in corr_authors: 
            pits0 = PersonItemToken.try_attach_list(rr.begin_token, PersonItemToken.ParseAttr.CANINITIALBEDIGIT, 10)
        ...
        rts = list()
        bref = BookLinkRefReferent._new498(res)
        if (num is not None): 
            bref.number = num
