...
class ProcessorService:
            ...
            if (img is not None): 
                for kp in img.items(): 
                    if (not kp[0] in ProcessorService.__m_images): 
                        ProcessorService.__m_images[kp[0]] = ImageWrapper._new3491(kp[0], kp[1])
    ...
    def __reorder_cartridges() -> None:
        ...
        while k < len(ProcessorService.__m_analizer_instances): 
            ...
            first_pass4141 = True
            while True:
                if first_pass4141: first_pass4141 = False
                else: i += 1
    ...
    def get_image_by_id(image_id : str) -> 'ImageWrapper':
        ...
        if (image_id is not None): 
            res = None
            wrapres3492 = RefOutArgWrapper(None)
            inoutres3493 = Utils.tryGetValue(ProcessorService.__m_images, image_id, wrapres3492)
            res = wrapres3492.value
            if (inoutres3493): 
                return res
        if (ProcessorService.__m_unknown_image is None): 
            ProcessorService.__m_unknown_image = ImageWrapper._new3491("unknown", PullentiNerCoreInternalResourceHelper.get_bytes("unknown.png"))
    ...
    def add_image(image_id : str, content : bytearray) -> None:
        ...
        if (image_id is None): 
            return
        wr = ImageWrapper._new3491(image_id, content)
    ...
    __m_standard_processor = None
    
    @staticmethod
    def get_standard_processor() -> 'Processor':
        """ Экземпляр процессора с множеством общих анализаторов (IsSpecific = false) """
        if (ProcessorService.__m_standard_processor is None): 
            ProcessorService.__m_standard_processor = ProcessorService.create_processor()
        return ProcessorService.__m_standard_processor
