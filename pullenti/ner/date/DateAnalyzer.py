...
class DateAnalyzer(Analyzer):
        ...
        while True:
            ...
            if (pli is None or len(pli) == 0): 
                ...
                while True:
                    ...
                    if (not ((tt.morph.case_.is_genitive))): 
                        break
                if (not high and DateAnalyzer.is_user_param_date(t)): 
                    high = True
                ...
                if ((rts is None and len(pli) > 2 and pli[2].typ == DateItemToken.DateItemType.DELIM) and pli[2].begin_token.is_comma): 
                    ...
                    rts = DateAnalyzer.__try_attach(pli, high)
                if (rts is None and DateAnalyzer.is_user_param_date(t)): 
                    i = 0
                    while i < (len(pli) - 2): 
                        if (pli[i].typ != DateItemToken.DateItemType.NUMBER): 
                            break
                        elif (pli[i + 1].can_be_day and pli[i + 2].typ == DateItemToken.DateItemType.MONTH): 
                            pli1 = list()
                            pli1.extend(pli)
                            del pli1[0:0+i + 1]
                            rts = DateAnalyzer.__try_attach(pli1, high)
                            break
                        i += 1
            ...
            while i < len(rts): 
                ...
                if ((i < (len(rts) - 1)) and rt.tag is None): 
                    rt.referent.add_occurence(TextAnnotation._new894(kit.sofa, rt.begin_char, rt.end_char, rt.referent))
                else: 
            ...
            if ((dat is not None and t.previous is not None and t.previous.is_hiphen) and t.previous.previous is not None and (isinstance(t.previous.previous.get_referent(), DateReferent))): 
                ...
                dr = Utils.asObjectOrNull(ad.register_referent(DateRangeReferent._new895(dat0, dat)), DateRangeReferent)
            ...
            if ((dat is not None and t.previous is not None and ((t.previous.is_hiphen or t.previous.is_value("ПО", None) or t.previous.is_value("И", None)))) and (isinstance(t.previous.previous, NumberToken)) and t.previous.previous.int_value is not None): 
                ...
                if (dat.day > 0 and (num < dat.day) and num > 0): 
                    if (dat.higher is not None): 
                        dat0 = DateReferent._new896(dat.higher, num)
                    elif (dat.month > 0): 
                        dat0 = DateReferent._new897(dat.month, num)
                    ...
                    dat0 = DateReferent._new898(num)
                elif ((dat.year < 0) and num > (- dat.year)): 
                    dat0 = DateReferent._new898(- num)
                if (dat0 is not None): 
                    rt0 = ReferentToken(ad.register_referent(dat0), t0, t0)
                    kit.embed_token(rt0)
                    if (not t.previous.is_hiphen and not t.previous.is_value("ПО", None)): 
                        continue
    ...
    def process_referent_stat(begin : 'Token', param : str=None) -> 'ReferentToken':
        ...
        if (begin.is_value("ДО", None) and (isinstance(begin.next0_, ReferentToken)) and (isinstance(begin.next0_.get_referent(), DateReferent))): 
            drr = DateRangeReferent._new901(Utils.asObjectOrNull(begin.next0_.get_referent(), DateReferent))
        ...
        if (begin.is_value("ПОСЛЕ", None) and (isinstance(begin.next0_, ReferentToken)) and (isinstance(begin.next0_.get_referent(), DateReferent))): 
            drr = DateRangeReferent._new902(Utils.asObjectOrNull(begin.next0_.get_referent(), DateReferent))
            res1 = ReferentToken(drr, begin, begin.next0_)
            if (res1.end_token.next0_ is not None and res1.end_token.next0_.is_value("ВКЛЮЧИТЕЛЬНО", None)): 
                res1.end_token = res1.end_token.next0_
            else: 
    ...
    def __try_attach(dts : typing.List['DateItemToken'], high : bool) -> typing.List['ReferentToken']:
        ...
        if ((dts[0].can_be_hour and len(dts) > 2 and dts[1].typ == DateItemToken.DateItemType.DELIM) and dts[2].int_value >= 0 and (dts[2].int_value < 60)): 
            if (dts[0].typ == DateItemToken.DateItemType.HOUR or ((dts[0].typ == DateItemToken.DateItemType.NUMBER and ((dts[2].typ == DateItemToken.DateItemType.HOUR or dts[2].typ == DateItemToken.DateItemType.NUMBER))))): 
                if (len(dts) > 3 and dts[3].typ == DateItemToken.DateItemType.DELIM and dts[3].string_value == dts[1].string_value): 
                    pass
                elif (not DateAnalyzer.is_user_param_ignore_time(dts[0])): 
                    dts1 = list(dts)
                    del dts1[0:0+3]
                    ...
                    if (res1 is not None and (isinstance(res1[len(res1) - 1].referent, DateReferent)) and res1[len(res1) - 1].referent.day > 0): 
                        time = DateReferent._new903(dts[0].int_value, dts[2].int_value)
                        ...
                        return res1
        if ((dts[0].can_be_day and len(dts) > 6 and dts[1].string_value == "-") and dts[2].can_be_day and (dts[0].int_value < dts[2].int_value)): 
            dts1 = list(dts)
            del dts1[0:0+2]
            res1 = DateAnalyzer.__try_attach(dts1, False)
            if (res1 is not None and res1[len(res1) - 1].begin_token == dts[2].begin_token and res1[len(res1) - 1].referent.day > 0): 
                day1 = Utils.asObjectOrNull(res1[len(res1) - 1].referent, DateReferent)
                day0 = DateReferent()
                day0.higher = day1.higher
                day0.day = dts[0].int_value
                tend = res1[len(res1) - 1].end_token
                res1.append(ReferentToken(day0, dts[0].begin_token, dts[0].end_token))
                ra = DateRangeReferent()
                ra.date_from = day0
                ra.date_to = day1
                res1.append(ReferentToken(ra, dts[0].begin_token, tend))
                return res1
        if (dts[0].typ == DateItemToken.DateItemType.HOUR and len(dts) > 2 and dts[1].typ == DateItemToken.DateItemType.MINUTE): 
            ...
            res1 = DateAnalyzer.__try_attach(dts1, False)
            if (res1 is not None and (isinstance(res1[len(res1) - 1].referent, DateReferent)) and res1[len(res1) - 1].referent.day > 0): 
                time = DateReferent._new903(dts[0].int_value, dts[1].int_value)
                if (ii > 2): 
        ...
        year_is_dif = False
        b = False
        wrapyear967 = RefOutArgWrapper(None)
        wrapmon968 = RefOutArgWrapper(None)
        wrapday969 = RefOutArgWrapper(None)
        b = DateAnalyzer.__apply_rule_formal(dts, high, wrapyear967, wrapmon968, wrapday969)
        year = wrapyear967.value
        mon = wrapmon968.value
        day = wrapday969.value
        if (b): 
        ...
        if (len(dts) == 1 and dts[0].ptr == DatePointerType.TODAY): 
            res0 = list()
            res0.append(ReferentToken(DateReferent._new905(DatePointerType.TODAY), dts[0].begin_token, dts[0].end_token))
            return res0
        if (len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.YEAR and dts[0].year <= 0): 
            res0 = list()
            res0.append(ReferentToken(DateReferent._new905(DatePointerType.UNDEFINED), dts[0].begin_token, dts[0].end_token))
            return res0
        if (not b and dts[0].typ == DateItemToken.DateItemType.POINTER and len(dts) > 1): 
            if (dts[1].typ == DateItemToken.DateItemType.YEAR): 
                ...
                b = True
        if (not b): 
            wrapyear907 = RefOutArgWrapper(None)
            wrapmon908 = RefOutArgWrapper(None)
            wrapday909 = RefOutArgWrapper(None)
            wrapyear_is_dif910 = RefOutArgWrapper(False)
            b = DateAnalyzer.__apply_rule_with_month(dts, high, wrapyear907, wrapmon908, wrapday909, wrapyear_is_dif910)
            year = wrapyear907.value
            mon = wrapmon908.value
            day = wrapday909.value
            year_is_dif = wrapyear_is_dif910.value
        if (not b): 
            wrapyear911 = RefOutArgWrapper(None)
            wrapmon912 = RefOutArgWrapper(None)
            wrapday913 = RefOutArgWrapper(None)
            b = DateAnalyzer.__apply_rule_year_only(dts, wrapyear911, wrapmon912, wrapday913)
            year = wrapyear911.value
            mon = wrapmon912.value
            day = wrapday913.value
        if (not b): 
            ...
            if ((len(dts) == 4 and dts[0].typ == DateItemToken.DateItemType.MONTH and dts[1].typ == DateItemToken.DateItemType.DELIM) and dts[2].typ == DateItemToken.DateItemType.MONTH and dts[3].can_be_year): 
                res0 = list()
                yea = DateReferent._new898(dts[3].int_value)
                res0.append(ReferentToken._new916(yea, dts[3].begin_token, dts[3].end_token, dts[3].morph))
                mon1 = DateReferent._new917(dts[0].int_value, yea)
                res0.append(ReferentToken._new918(mon1, dts[0].begin_token, dts[0].end_token, mon1))
                mon2 = DateReferent._new917(dts[2].int_value, yea)
            ...
            if ((dts[0].typ == DateItemToken.DateItemType.MONTH and len(dts) == 1 and dts[0].end_token.next0_ is not None) and ((dts[0].end_token.next0_.is_hiphen or dts[0].end_token.next0_.is_value("ПО", None) or dts[0].end_token.next0_.is_value("НА", None)))): 
                ...
                if (rt is not None): 
                    ...
                    if ((dr0 is not None and dr0.year > 0 and dr0.month > 0) and dr0.day == 0 and dr0.month > dts[0].int_value): 
                        dr_year0 = DateReferent._new898(dr0.year)
                        res0 = list()
                        res0.append(ReferentToken(dr_year0, dts[0].end_token, dts[0].end_token))
                        dr_mon0 = DateReferent._new917(dts[0].int_value, dr_year0)
            ...
            if (((len(dts) == 3 and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].begin_token.is_hiphen) and dts[0].can_be_year and dts[2].can_be_year) and (dts[0].int_value < dts[2].int_value)): 
                ...
                if (ok): 
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new898(dts[0].year), dts[0].begin_token, dts[0].end_token))
                    res0.append(ReferentToken(DateReferent._new898(dts[2].year), dts[2].begin_token, dts[2].end_token))
                    return res0
            ...
            if (dts[0].ltyp != DateItemToken.FirstLastTyp.NO and len(dts) > 1): 
                high = True
            if (((len(dts) == 5 and dts[0].can_be_day and dts[1].typ == DateItemToken.DateItemType.MONTH) and dts[2].can_be_hour and dts[3].typ == DateItemToken.DateItemType.DELIM) and dts[4].can_be_minute): 
                res0 = list()
                mm = DateReferent._new935(dts[1].int_value)
                res0.append(ReferentToken(mm, dts[1].begin_token, dts[1].end_token))
                dd = DateReferent._new922(dts[0].int_value, mm)
                res0.append(ReferentToken(dd, dts[0].begin_token, dts[2].end_token))
                hh = DateReferent._new937(dts[2].int_value, dts[4].int_value, dd)
                res0.append(ReferentToken(hh, dts[0].begin_token, dts[4].end_token))
                return res0
            if (high): 
                ...
                if ((((len(dts) == 3 and dts[0].can_be_year and dts[0].typ == DateItemToken.DateItemType.NUMBER) and dts[2].can_be_year and dts[2].typ == DateItemToken.DateItemType.NUMBER) and (dts[0].year < dts[2].year) and dts[1].typ == DateItemToken.DateItemType.DELIM) and dts[1].begin_token.is_hiphen): 
                    res0 = list()
                    y1 = DateReferent._new898(dts[0].year)
                    res0.append(ReferentToken(y1, dts[0].begin_token, dts[0].end_token))
                    y2 = DateReferent._new898(dts[2].year)
                    res0.append(ReferentToken(y2, dts[2].begin_token, dts[2].end_token))
                    ra = DateRangeReferent._new895(y1, y2)
                    res0.append(ReferentToken(ra, dts[0].begin_token, dts[2].end_token))
                    return res0
                if (DateAnalyzer.is_user_param_date(dts[0])): 
                    if (((len(dts) == 3 and dts[0].can_be_day and dts[0].typ == DateItemToken.DateItemType.NUMBER) and dts[2].can_by_month and dts[2].typ == DateItemToken.DateItemType.NUMBER) and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].begin_token.is_char_of("./\\")): 
                        if (dts[0].begin_token.previous is not None and dts[0].begin_token.previous.is_value("В", None)): 
                            pass
                        else: 
                            res0 = list()
                            y1 = DateReferent._new935(dts[2].int_value)
                            res0.append(ReferentToken(y1, dts[2].begin_token, dts[2].end_token))
                            y2 = DateReferent._new922(dts[0].int_value, y1)
                            res0.append(ReferentToken(y2, dts[0].begin_token, dts[2].end_token))
                            return res0
                    if (((((len(dts) == 7 and dts[0].can_be_day and dts[0].typ == DateItemToken.DateItemType.NUMBER) and dts[2].can_by_month and dts[2].typ == DateItemToken.DateItemType.NUMBER) and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].begin_token.is_char_of("./\\")) and dts[3].begin_token.is_hiphen and dts[4].can_be_day) and dts[6].can_by_month and dts[5].begin_token.is_char_of("./\\")): 
                        res0 = list()
                        m1 = DateReferent._new935(dts[2].int_value)
                        res0.append(ReferentToken(m1, dts[2].begin_token, dts[2].end_token))
                        d1 = DateReferent._new922(dts[0].int_value, m1)
                        res0.append(ReferentToken(d1, dts[0].begin_token, dts[2].end_token))
                        m2 = DateReferent._new935(dts[6].int_value)
                        res0.append(ReferentToken(m2, dts[6].begin_token, dts[6].end_token))
                        d2 = DateReferent._new922(dts[4].int_value, m2)
                        res0.append(ReferentToken(d1, dts[4].begin_token, dts[6].end_token))
                        ra = DateRangeReferent._new895(d1, d2)
                        res0.append(ReferentToken(ra, dts[0].begin_token, dts[6].end_token))
                        return res0
            if (dts[0].typ == DateItemToken.DateItemType.QUARTAL or dts[0].typ == DateItemToken.DateItemType.HALFYEAR or ((dts[0].typ == DateItemToken.DateItemType.MONTH and dts[0].ltyp != DateItemToken.FirstLastTyp.NO))): 
                if (len(dts) == 1 or dts[1].typ == DateItemToken.DateItemType.YEAR): 
                    ...
                    if (len(dts) > 1): 
                        ii = 1
                        yea = DateReferent._new898(dts[1].int_value)
                        res0.append(ReferentToken._new916(yea, dts[1].begin_token, dts[1].end_token, dts[1].morph))
                    else: 
                        cou = 0
                    ...
                    return res0
            if (((len(dts) == 3 and dts[1].typ == DateItemToken.DateItemType.DELIM and ((dts[1].string_value == "." or dts[1].string_value == ":"))) and dts[0].can_be_hour and dts[2].can_be_minute) and not DateAnalyzer.is_user_param_ignore_time(dts[0])): 
                ok = False
                ...
                if (ok): 
                    ...
                    while tt is not None and (cou < 1000): 
                        if (isinstance(tt.get_referent(), DateReferent)): 
                            ...
                            if (dr.find_slot(DateReferent.ATTR_DAY, None, True) is not None): 
                                time.higher = dr
                                if ((tt.end_char + 10) >= t00.begin_char): 
                                    t00 = tt
                                break
                        tt = tt.previous; cou += 1
                    tt1 = dts[2].end_token
                    ...
                    if (tt2 is not None): 
                        tt1 = tt2
                    res0 = list()
                    res0.append(ReferentToken(time, t00, tt1))
                    return res0
            if ((len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.MONTH and dts[0].begin_token.previous is not None) and dts[0].begin_token.previous.morph.class0_.is_preposition): 
                if (dts[0].chars.is_latin_letter and dts[0].chars.is_all_lower): 
                    ...
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new935(dts[0].int_value), dts[0].begin_token, dts[0].end_token))
                    return res0
            return None
        res = list()
        dr_year = None
        ...
        t1 = None
        if (cent is not None): 
            ce = DateReferent._new955((- cent.int_value if cent.new_age < 0 else cent.int_value), cent.relate)
            t1 = cent.end_token
            rt = ReferentToken(ce, cent.begin_token, t1)
            res.append(rt)
        if (tenyears is not None): 
            ce = DateReferent._new956(tenyears.int_value, tenyears.relate)
            if (cent is not None): 
        ...
        if (year is not None and year.year > 0): 
            dr_year = DateReferent._new898((- year.year if year.new_age < 0 else year.year))
            if (not year_is_dif): 
                t1 = year.end_token
                if (t1.next0_ is not None and t1.next0_.is_value("ГОРОД", None)): 
                    ...
                    if (tt2 is None): 
                        ...
                        year.end_token = t1
            t0 = year.begin_token
            res.append(ReferentToken._new916(dr_year, t0, year.end_token, year.morph))
            if (((len(dts) == 3 and year == dts[2] and mon is None) and day is None and dts[0].year > 0) and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].end_token.is_hiphen): 
                dr_year0 = DateReferent._new898((- dts[0].year if year.new_age < 0 else dts[0].year))
                t0 = dts[0].begin_token
                res.append(ReferentToken(dr_year0, t0, dts[0].end_token))
        if (mon is not None): 
            dr_mon = DateReferent._new935(mon.int_value)
            ...
            if (dr_year is None and t1.next0_ is not None and ((t1.next0_.is_value("ПО", None) or t1.next0_.is_value("НА", None)))): 
                ...
                if (rt is not None): 
                    ...
                    if (dr0 is not None and dr0.year > 0 and dr0.month > 0): 
                        dr_year = DateReferent._new898(dr0.year)
                        t0 = t1
                        res.append(ReferentToken(dr_year, t0, t1))
                        dr_mon.higher = dr_year
            res.append(ReferentToken._new916(dr_mon, t0, t1, mon.morph))
            if (day is not None): 
                dr_day = DateReferent._new963(day.int_value)
                ...
                res.append(ReferentToken._new916(dr_day, t0, t1, day.morph))
                if (dts[0].typ == DateItemToken.DateItemType.HOUR): 
                    hou = DateReferent._new965(dr_day)
        ...
        if (dr_day is not None and not year_is_dif): 
            ...
            if (rt is not None): 
                ...
                rt.begin_token = t0
                res.append(rt)
                if (rt.end_token.next0_ is not None and rt.end_token.next0_.is_hiphen): 
                    rt1 = DateAnalyzer.__try_attach_time(rt.end_token.next0_.next0_, True)
                    if (rt1 is not None): 
                        rt1.referent.higher = dr_day
                        res.append(rt1)
                        rr = DateRangeReferent()
                        rr.date_from = Utils.asObjectOrNull(rt.referent, DateReferent)
                        rr.date_to = Utils.asObjectOrNull(rt1.referent, DateReferent)
                        res.append(ReferentToken(rr, rt.begin_token, rt1.end_token))
            else: 
                i = 1
                while i < len(dts): 
            ...
            if (dt0.month == 0): 
                ...
                if (tt is not None and tt.is_char_of("(,")): 
                    ...
                    if (dit is not None and dit.typ == DateItemToken.DateItemType.MONTH): 
                        dr_mon = DateReferent._new966(dt0, dit.int_value)
    ...
    def __try_attach_time(t : 'Token', after_date : bool) -> 'ReferentToken':
        ...
        if (dts is None or len(dts) == 0): 
            return None
        if (dts[0].is_newline_before and dts[0].typ != DateItemToken.DateItemType.HOUR and not DateAnalyzer.is_user_param_date(dts[0])): 
            return None
    ...
    def __try_attach_time_li(dts : typing.List['DateItemToken'], after_date : bool) -> 'ReferentToken':
        ...
        if (len(dts) == 1): 
            if (dts[0].typ == DateItemToken.DateItemType.HOUR and after_date): 
                time = DateReferent._new903(dts[0].int_value, 0)
                t1 = dts[0].end_token
            else: 
                return None
            ...
            if (len(dts) >= 5): 
                dat = DateAnalyzer.__try_attach(dts, False)
    ...
    def __apply_rule_formal(its : typing.List['DateItemToken'], high : bool, year : 'DateItemToken', mon : 'DateItemToken', day : 'DateItemToken') -> bool:
        ...
        mon.value = (None)
        day.value = (None)
        if (not high and len(its) == 5): 
            if (its[0].begin_token.previous is not None and its[0].begin_token.previous.is_value("ОТ", None)): 
                high = True
        i = 0
        j = 0
        i = 0
        ...
        while True:
            ...
            if ((its[i].begin_token.previous is not None and its[i].begin_token.previous.is_char(')') and (its[i].whitespaces_before_count < 2)) and i > 0): 
                return False
        ...
        if (high and its[0].can_be_year and len(its) == 1): 
            year.value = its[0]
            if (its[0].new_style is not None and len(its[0].new_style) > 0): 
                year.value = DateItemToken._new973(its[0].begin_token, its[0].begin_token, its[0].int_value)
            return True
        ...
        if ((len(its) == 3 and its[0].typ == DateItemToken.DateItemType.NUMBER and its[0].can_be_day) and its[1].can_by_month): 
            if (its[2].typ == DateItemToken.DateItemType.YEAR or ((its[2].can_be_year and its[0].begin_token.previous is not None and its[0].begin_token.previous.is_value("ОТ", None)))): 
                if (((BracketHelper.is_bracket(its[0].begin_token, False) and BracketHelper.is_bracket(its[0].end_token, False))) or ((its[0].begin_token.previous is not None and its[0].begin_token.previous.get_morph_class_in_dictionary().is_preposition))): 
                    ...
                    return True
        if (high): 
            if (((len(its) >= 5 and its[0].can_be_day and its[1].string_value == ".") and its[2].can_by_month and its[3].string_value == ".") and its[4].typ == DateItemToken.DateItemType.NUMBER): 
                if (len(its) == 5 or its[4].end_token.next0_.is_hiphen): 
                    year.value = its[4]
                    mon.value = its[2]
                    day.value = its[0]
                    return True
    ...
    def __apply_date_range(ad : 'AnalyzerData', its : typing.List['DateItemToken'], lang : 'MorphLang') -> 'DateRangeReferent':
        ...
        if ((its[0].can_be_year and its[1].string_value == "-" and its[2].typ == DateItemToken.DateItemType.YEAR) and (its[0].year < its[2].year)): 
            ...
            res.date_from = Utils.asObjectOrNull(ad.register_referent(DateReferent._new898(its[0].year)), DateReferent)
            rt1 = ReferentToken(res.date_from, its[0].begin_token, its[0].end_token)
            res.date_to = Utils.asObjectOrNull(ad.register_referent(DateReferent._new898(its[2].year)), DateReferent)
    ...
    def __apply_date_range0(kit : 'AnalysisKit', ad : 'AnalyzerData') -> None:
        ...
        while True:
            ...
            if (str0_ == "ON" and (isinstance(t0, TextToken))): 
                ...
                if (tok is not None): 
                    dow = DateReferent._new976(tok.termin.tag)
            ...
            if (str0_ == "С" or str0_ == "C"): 
                ...
                if ((isinstance(t.next0_, ReferentToken)) and (isinstance(t.next0_.get_referent(), DateReferent))): 
                    dr = DateRangeReferent._new901(Utils.asObjectOrNull(t.next0_.get_referent(), DateReferent))
                ...
                dr = DateRangeReferent._new902(Utils.asObjectOrNull(t.next0_.get_referent(), DateReferent))
                rt0 = ReferentToken(ad.register_referent(dr), t, t.next0_)
                if (rt0.end_token.next0_ is not None and rt0.end_token.next0_.is_value("ВКЛЮЧИТЕЛЬНО", None)): 
                    rt0.end_token = rt0.end_token.next0_
                else: 
            ...
            if (year_val1 == year_val2): 
                ...
                if (DateReferent.compare(date1, date2) >= 0): 
                    continue
            if (date1 is None): 
                date1 = (Utils.asObjectOrNull(ad.register_referent(DateReferent._new898(year_val1)), DateReferent))
                rt0 = ReferentToken(date1, t0, t0)
                kit.embed_token(rt0)
                if (t0 == t): 
                    t = (rt0)
            if (date2 is None): 
                date2 = (Utils.asObjectOrNull(ad.register_referent(DateReferent._new898(year_val2)), DateReferent))
                rt1 = ReferentToken(date2, t1, t1)
                kit.embed_token(rt1)
                t1 = (rt1)
            rt = ReferentToken(ad.register_referent(DateRangeReferent._new895(date1, date2)), t, t1)
    ...
    @staticmethod
    def is_user_param_date(t : 'Token') -> bool:
        if (t is None): 
            return False
        if (t.kit.sofa.user_params is not None): 
            if ("DATE" in t.kit.sofa.user_params): 
                return True
        return False
    
    @staticmethod
    def is_user_param_ignore_time(t : 'Token') -> bool:
        if (t is None): 
            return False
        if (t.kit.sofa.user_params is not None): 
            if ("IGNORETIME" in t.kit.sofa.user_params): 
                return True
        return False
