...
class TransportAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        first_pass4103 = True
        while True:
            if first_pass4103: first_pass4103 = False
            else: t = t.next0_
            ...
            if (rts is not None): 
                for rt in rts: 
                    ...
                    first_pass4104 = True
                    while True:
                        if first_pass4104: first_pass4104 = False
                        ...
                        if (s.type_name == TransportReferent.ATTR_MODEL): 
                            ...
                            for k in range(2):
                                if (not str.isdigit(mod[0])): 
                                    li = [ ]
                                    wrapli3333 = RefOutArgWrapper(None)
                                    inoutres3334 = Utils.tryGetValue(objs_by_model, mod, wrapli3333)
                                    li = wrapli3333.value
                                    if (not inoutres3334): 
                                        li = list()
                            ...
                            obj_by_names.add(Termin._new92(str(s.value), rt.referent))
        if (len(objs_by_model) == 0 and len(obj_by_names.termins) == 0): 
            return
        t = kit.first_token
        first_pass4105 = True
        while True:
            if first_pass4105: first_pass4105 = False
            else: t = t.next0_
            if (not (t is not None)): break
            if (t.is_ignored): 
                continue
    ...
    def __try_attach(self, its : typing.List['TransItemToken'], attach : bool) -> typing.List['ReferentToken']:
        ...
        first_pass4106 = True
        while True:
            if first_pass4106: first_pass4106 = False
            else: i += 1
