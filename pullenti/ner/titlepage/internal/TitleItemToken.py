...
class TitleItemToken(MetaToken):
            ...
            if (tt.term == "ТЕМА"): 
                ...
                if (tit is not None and tit.typ == TitleItemToken.Types.TYP): 
                    ...
                    if (t1.next0_ is not None and t1.next0_.is_char(':')): 
                        t1 = t1.next0_
                    return TitleItemToken._new3286(t, t1, TitleItemToken.Types.TYPANDTHEME, tit.value)
            ...
            if (tok is not None): 
                ...
                if (ty == TitleItemToken.Types.TYP): 
                    ...
                    if (tit is not None and tit.typ == TitleItemToken.Types.THEME): 
                        return TitleItemToken._new3286(npt.begin_token, tit.end_token, TitleItemToken.Types.TYPANDTHEME, s)
                    ...
                    if (s == "ДИССЕРТАЦИЯ" or s == "ДИСЕРТАЦІЯ"): 
                        ...
                        first_pass4091 = True
                        while True:
                            if first_pass4091: first_pass4091 = False
                            else: ttt = ttt.next0_
                            ...
                            if (npt1 is not None and npt1.noun.is_value("СТЕПЕНЬ", "СТУПІНЬ")): 
                                ...
                                t1 = ttt
                                continue
                            rt = t1.kit.process_referent("PERSON", ttt, None)
                            if (rt is not None and (isinstance(rt.referent, PersonPropertyReferent))): 
                                ...
                                nam = Utils.ifNotNull(ppr.name, "")
                                if (nam.startswith("доктор")): 
                                    t1 = rt.end_token
                                    s = "ДОКТОРСКАЯ ДИССЕРТАЦИЯ"
                                    break
                                elif (nam.startswith("кандидат")): 
                                    t1 = rt.end_token
                                    s = "КАНДИДАТСКАЯ ДИССЕРТАЦИЯ"
                                    break
                                elif (nam.startswith("магистр")): 
                                    t1 = rt.end_token
                    ...
                    if (s.endswith("ОТЧЕТ") and t1.next0_ is not None and t1.next0_.is_value("О", None)): 
                        ...
                        if (npt1 is not None and npt1.morph.case_.is_prepositional): 
                            t1 = npt1.end_token
                    return TitleItemToken._new3286(npt.begin_token, t1, ty, s)
    ...
    def __try_attach_speciality(t : 'Token', key_word_before : bool) -> 'TitleItemToken':
        ...
        first_pass4092 = True
        while True:
            if first_pass4092: first_pass4092 = False
            else: tt = tt.next0_
            ...
            if (br is not None): 
                ...
                continue
            t = tt
        return TitleItemToken._new3286(t0, t, TitleItemToken.Types.SPECIALITY, Utils.toStringStringIO(val))
    ...
    def initialize() -> None:
        ...
        for s in ["РАБОТА", "ДИССЕРТАЦИЯ", "ОТЧЕТ", "ОБЗОР", "ДИПЛОМ", "ПРОЕКТ", "СПРАВКА", "АВТОРЕФЕРАТ", "РЕФЕРАТ", "TECHNOLOGY ISSUES", "TECHNOLOGY COURSE", "УЧЕБНИК", "УЧЕБНОЕ ПОСОБИЕ"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.TYP))
        for s in ["РОБОТА", "ДИСЕРТАЦІЯ", "ЗВІТ", "ОГЛЯД", "ДИПЛОМ", "ПРОЕКТ", "ДОВІДКА", "АВТОРЕФЕРАТ", "РЕФЕРАТ"]: 
            TitleItemToken.M_TERMINS.add(Termin._new483(s, MorphLang.UA, TitleItemToken.Types.TYP))
        for s in ["ДОПУСТИТЬ К ЗАЩИТА", "РЕКОМЕНДОВАТЬ К ЗАЩИТА", "ДОЛЖНОСТЬ", "ЦЕЛЬ РАБОТЫ", "НА ПРАВАХ РУКОПИСИ", "ПО ИЗДАНИЮ", "ПОЛУЧЕНО"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.DUST))
        for s in ["ДОПУСТИТИ ДО ЗАХИСТУ", "РЕКОМЕНДУВАТИ ДО ЗАХИСТ", "ПОСАДА", "МЕТА РОБОТИ", "НА ПРАВАХ РУКОПИСУ", "ПО ВИДАННЮ", "ОТРИМАНО"]: 
            TitleItemToken.M_TERMINS.add(Termin._new483(s, MorphLang.UA, TitleItemToken.Types.DUST))
        for s in ["УТВЕРЖДАТЬ", "СОГЛАСЕН", "СТВЕРДЖУВАТИ", "ЗГОДЕН"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.ADOPT))
        for s in ["НАУЧНЫЙ РУКОВОДИТЕЛЬ", "РУКОВОДИТЕЛЬ РАБОТА", "НАУКОВИЙ КЕРІВНИК", "КЕРІВНИК РОБОТА"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.BOSS))
        for s in ["НАУЧНЫЙ КОНСУЛЬТАНТ", "КОНСУЛЬТАНТ", "НАУКОВИЙ КОНСУЛЬТАНТ"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.CONSULTANT))
        for s in ["РЕДАКТОР", "РЕДАКТОРСКАЯ ГРУППА", "РЕЦЕНЗЕНТ", "РЕДАКТОРСЬКА ГРУПА"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.EDITOR))
        for s in ["ОФИЦИАЛЬНЫЙ ОППОНЕНТ", "ОППОНЕНТ", "ОФІЦІЙНИЙ ОПОНЕНТ"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.OPPONENT))
        for s in ["ИСПОЛНИТЕЛЬ", "ОТВЕТСТВЕННЫЙ ИСПОЛНИТЕЛЬ", "АВТОР", "ДИПЛОМНИК", "КОЛЛЕКТТИВ ИСПОЛНИТЕЛЕЙ", "ВЫПОЛНИТЬ", "ИСПОЛНИТЬ"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.WORKER))
        for s in ["ВИКОНАВЕЦЬ", "ВІДПОВІДАЛЬНИЙ ВИКОНАВЕЦЬ", "АВТОР", "ДИПЛОМНИК", "КОЛЛЕКТТИВ ВИКОНАВЦІВ", "ВИКОНАТИ", "ВИКОНАТИ"]: 
            TitleItemToken.M_TERMINS.add(Termin._new483(s, MorphLang.UA, TitleItemToken.Types.WORKER))
        for s in ["КЛЮЧЕВЫЕ СЛОВА", "KEYWORDS", "КЛЮЧОВІ СЛОВА"]: 
            TitleItemToken.M_TERMINS.add(Termin._new92(s, TitleItemToken.Types.KEYWORDS))
    
    @staticmethod
    def _new3286(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types', _arg4 : str) -> 'TitleItemToken':
