...
class TitlePageAnalyzer(Analyzer):
    ...
    def process_referent1(self, begin : 'Token', end : 'Token') -> 'ReferentToken':
        et = None
        wrapet3302 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(begin, (0 if end is None else end.end_char), begin.kit, wrapet3302)
        et = wrapet3302.value
        if (tpr is None): 
            return None
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        et = None
        wrapet3303 = RefOutArgWrapper(None)
        tpr = TitlePageAnalyzer._process(kit.first_token, 0, kit, wrapet3303)
        et = wrapet3303.value
        if (tpr is not None): 
    ...
    def _process(begin : 'Token', max_char_pos : int, kit : 'AnalysisKit', end_token : 'Token') -> 'TitlePageReferent':
        ...
        if (term is not None and kit is not None): 
            ...
            first_pass4094 = True
            while True:
                if first_pass4094: first_pass4094 = False
                else: t = t.next0_
        ...
        first_pass4095 = True
        while True:
            if first_pass4095: first_pass4095 = False
            else: t = t.next0_
            if (not (t is not None)): break
            ...
            for r in rli: 
                if (isinstance(r, PersonReferent)): 
                    ...
                    if (typ != TitleItemToken.Types.UNDEFINED): 
                        ...
                        first_pass4096 = True
                        while True:
                            if first_pass4096: first_pass4096 = False
                            else: tt = tt.next0_
                ...
                if (isinstance(r, DateReferent)): 
                    if (res.date is None): 
                        ...
                        if (res.date.find_slot(DateReferent.ATTR_HOUR, None, True) is not None and res.date.higher is not None): 
                            res.date = res.date.higher
                        if (t.end_char > end_token.value.end_char): 
                            end_token.value = t
                ...
                if (isinstance(r, OrganizationReferent)): 
                    ...
                    if ("курс" in org0_.types and org0_.number is not None): 
                        i = 0
                        wrapi3304 = RefOutArgWrapper(0)
                        inoutres3305 = Utils.tryParseInt(org0_.number, wrapi3304)
                        i = wrapi3304.value
                        if (inoutres3305): 
                            if (i > 0 and (i < 8)): 
                                res.student_year = i
        ...
        if (res.date is None): 
            ...
            first_pass4097 = True
            while True:
                if first_pass4097: first_pass4097 = False
                else: t = t.next0_
                ...
                if (isinstance(t.next0_, TextToken)): 
                    if (t.next0_.is_char_of(":,") or t.next0_.is_hiphen): 
                        t = t.next0_
                rt = t.kit.process_referent(DateAnalyzer.ANALYZER_NAME, t.next0_, None)
                if (rt is not None): 
