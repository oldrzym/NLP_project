...
class OrganizationAnalyzer(Analyzer):
    ...
    def __attach_global_org(t : 'Token', attach_typ : 'AttachType', ext_geo : object=None) -> 'ReferentToken':
        ...
        if ((isinstance(t, TextToken)) and t.chars.is_latin_letter): 
            if (MiscHelper.is_eng_article(t)): 
                ...
                if (res11 is not None): 
                    res11.begin_token = t
                    return res11
        rt00 = OrganizationAnalyzer.__try_attach_politic_party(t, True)
        if (rt00 is not None): 
            return rt00
        if (not (isinstance(t, TextToken))): 
            if (t is not None and t.get_referent() is not None and t.get_referent().type_name == "URI"): 
                rt = OrganizationAnalyzer.__attach_global_org(t.begin_token, attach_typ, None)
        ...
        if (t.chars.is_all_upper and term == "ВС"): 
            if (t.previous is not None): 
                if (t.previous.is_value("ПРЕЗИДИУМ", None) or t.previous.is_value("ПЛЕНУМ", None) or t.previous.is_value("СЕССИЯ", None)): 
                    ...
                    te = OrganizationAnalyzer.__attach_tail_attributes(org00, t.next0_, False, OrganizationAnalyzer.AttachType.NORMAL, True)
                    if (len(org00.geo_objects) == 1 and org00.geo_objects[0].alpha2 == "SU"): 
                        ...
                        org00.add_name("ВЕРХОВНЫЙ СУД", True, None)
                        org00.add_name("ВС", True, None)
            ...
            if (t.next0_ is not None and (isinstance(t.next0_.get_referent(), GeoReferent))): 
                ...
                if (is_vc): 
                    ...
                    te = OrganizationAnalyzer.__attach_tail_attributes(org00, t.next0_, False, OrganizationAnalyzer.AttachType.NORMAL, True)
                    return ReferentToken(org00, t, Utils.ifNotNull(te, t))
        if ((t.chars.is_all_upper and ((term == "АН" or term == "ВАС" or term == "АМН")) and t.next0_ is not None) and (isinstance(t.next0_.get_referent(), GeoReferent))): 
            org00 = OrganizationReferent()
            if (term == "АН"): 
                ...
                org00.add_name("АКАДЕМИЯ МЕДИЦИНСКИХ НАУК", True, None)
                org00.add_type_str("академия")
                org00.add_profile(OrgProfile.SCIENCE)
            else: 
        ...
        if (t.chars.is_all_upper and term == "ГД" and t.previous is not None): 
            rt = t.kit.process_referent("PERSONPROPERTY", t.previous, None)
        ...
        if (t.chars.is_all_upper and ((term == "СК" or term == "CK"))): 
            if (t.next0_ is not None and (isinstance(t.next0_.get_referent(), GeoReferent))): 
                ok1 = False
                ad = t.kit.get_analyzer_data_by_analyzer_name("ORGANIZATION")
                for e0_ in ad.referents: 
                    for s in e0_.slots: 
                        if (isinstance(s.value, str)): 
                            if ("СЛЕДСТВЕННЫЙ КОМИТЕТ" in s.value.upper()): 
                                ok1 = True
                                break
                    if (ok1): 
                        break
        ...
        for ot in gt: 
            ...
            if (ot.begin_token == ot.end_token): 
                if (len(gt) == 1): 
                    if ((isinstance(ot.begin_token, TextToken)) and ot.begin_token.term == "МГТУ"): 
                        ty = OrgItemTypeToken.try_attach(ot.begin_token, False)
                        if (ty is not None): 
                            continue
                else: 
                    ad = OrganizationAnalyzer._get_data(t)
            ...
            if (((t.chars.is_all_lower and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY and ext_geo is None) and not t.is_value("МИД", None) and not org0._types_contains("факультет")) and org0.kind != OrganizationKind.JUSTICE): 
                ...
                if (ot.morph.number == MorphNumber.PLURAL): 
                    continue
                okk = False
                tyty = OrgItemTypeToken.try_attach(t, True)
                if (tyty is not None and tyty.end_token == ot.end_token): 
                    if (tyty.name == "ТАМОЖЕННЫЙ СОЮЗ" or tyty.name == "МИТНИЙ СОЮЗ"): 
                        okk = True
                    else: 
                        continue
                if (t.next0_ is not None and (isinstance(t.next0_.get_referent(), GeoReferent))): 
                    ...
                    pass
                elif (tyty is not None and tyty.end_token.next0_ is not None and (isinstance(tyty.end_token.next0_.get_referent(), GeoReferent))): 
                    pass
                elif (not okk): 
                    continue
            ...
            if (ot.begin_token == ot.end_token): 
                ...
                if (isinstance(t, TextToken)): 
                    ...
                    if (t.length_char < 3): 
                        ...
                        if (not ok1): 
                            continue
                rt = t.kit.process_referent("TRANSPORT", t, None)
                if (rt is not None): 
                    continue
            org0_ = None
            if (isinstance(t, TextToken)): 
            ...
            if (org0_ is None): 
                ...
                sl = org0_.find_slot("NAME", "МИНКУЛЬТ", True)
                if (sl is not None): 
                    org0_.slots.remove(sl)
            ...
            if ((isinstance(t.previous, TextToken)) and (t.whitespaces_before_count < 2) and t.previous.morph.class0_.is_adjective): 
                gg = t.kit.process_referent("GEO", t.previous, None)
            ...
            if (not "академия" in org0.types and attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                t1 = OrganizationAnalyzer.__attach_tail_attributes(org0_, ot.end_token.next0_, False, OrganizationAnalyzer.AttachType.NORMAL, True)
            ...
            if (t1.whitespaces_after_count < 2): 
                typ = OrgItemTypeToken.try_attach(t1.next0_, False)
                if (typ is not None): 
                    if (OrgItemTypeToken.is_type_accords(org0_, typ)): 
                        if (typ.chars.is_latin_letter and typ.root is not None and typ.root.can_be_normal_dep): 
                            ...
                            rrr = OrganizationAnalyzer.process_referent_stat(t1.next0_, None)
                            if (rrr is not None and not rrr.referent.can_be_equals(org0_, ReferentsEqualType.WITHINONETEXT)): 
                                pass
                            else: 
                                org0_.add_type(typ, False)
                                t1 = typ.end_token
            if (len(org0_.geo_objects) == 0 and t.previous is not None and t.previous.morph.class0_.is_adjective): 
                grt = t.kit.process_referent("GEO", t.previous, None)
            ...
            if (not t.is_whitespace_before and not t1.is_whitespace_after): 
                if (BracketHelper.can_be_start_of_sequence(t.previous, True, False) and BracketHelper.can_be_end_of_sequence(t1.next0_, True, None, False)): 
                    t = t.previous
                    t1 = t1.next0_
            gt = OrgGlobal.GLOBAL_ORGS.try_attach(t1.next0_, None, True)
            if (gt is not None and gt[0].item.referent == org0 and (t1.whitespaces_after_count < 3)): 
                t1 = gt[0].end_token
            elif (BracketHelper.can_be_start_of_sequence(t1.next0_, False, False)): 
                gt = OrgGlobal.GLOBAL_ORGS.try_attach(t1.next0_.next0_, None, True)
                if (gt is not None and gt[0].item.referent == org0): 
                    if (BracketHelper.can_be_end_of_sequence(gt[0].end_token.next0_, False, None, False)): 
                        t1 = gt[0].end_token.next0_
    ...
    def __try_attach_org_med_typ(t : 'Token') -> 'MetaToken':
        ...
        if (((t is not None and s == "Г" and t.next0_ is not None) and t.next0_.is_char_of("\\/.") and t.next0_.next0_ is not None) and t.next0_.next0_.is_value("Б", None)): 
            ...
            if (t.next0_.is_char('.') and t1.next0_ is not None and t1.next0_.is_char('.')): 
                t1 = t1.next0_
            return MetaToken._new2951(t, t1, "городская больница", MorphCollection._new2950(MorphGender.FEMINIE))
        if ((s == "ИН" and t.next0_ is not None and t.next0_.is_hiphen) and t.next0_.next0_ is not None and t.next0_.next0_.is_value("Т", None)): 
            return MetaToken._new2951(t, t.next0_.next0_, "институт", MorphCollection._new2950(MorphGender.MASCULINE))
        if ((s == "Б" and t.next0_ is not None and t.next0_.is_hiphen) and (isinstance(t.next0_.next0_, TextToken)) and ((t.next0_.next0_.is_value("ЦА", None) or t.next0_.next0_.is_value("ЦУ", None)))): 
            return MetaToken._new2951(t, t.next0_.next0_, "больница", MorphCollection._new2950(MorphGender.FEMINIE))
        if (s == "ГКБ"): 
            return MetaToken._new2951(t, t, "городская клиническая больница", MorphCollection._new2950(MorphGender.FEMINIE))
        if (t.is_value("ПОЛИКЛИНИКА", None)): 
            return MetaToken._new2951(t, t, "поликлиника", MorphCollection._new2950(MorphGender.FEMINIE))
        if (t.is_value("БОЛЬНИЦА", None)): 
            return MetaToken._new2951(t, t, "больница", MorphCollection._new2950(MorphGender.FEMINIE))
    ...
    @staticmethod
    def __try_attach_org_med(t : 'Token') -> 'ReferentToken':
        if (t is None): 
            return None
    ...
    @staticmethod
    def __try_attach_prop_names(t : 'Token') -> 'ReferentToken':
        rt = OrganizationAnalyzer.__try_attach_org_sport_associations(t)
        if (rt is None): 
            rt = OrganizationAnalyzer.__try_attach_org_names(t)
        if (rt is None): 
            return None
        t0 = rt.begin_token.previous
        if ((isinstance(t0, TextToken)) and (t0.whitespaces_after_count < 2) and t0.morph.class0_.is_adjective): 
            rt0 = t0.kit.process_referent("GEO", t0, None)
        ...
        if (rt.end_token.whitespaces_after_count < 2): 
            tt1 = OrganizationAnalyzer.__attach_tail_attributes(Utils.asObjectOrNull(rt.referent, OrganizationReferent), rt.end_token.next0_, True, OrganizationAnalyzer.AttachType.NORMAL, True)
            if (tt1 is not None): 
                rt.end_token = tt1
        return rt
    ...
    def __try_attach_org_names(t : 'Token') -> 'ReferentToken':
        ...
        if (not (isinstance(t, TextToken)) or not t.chars.is_letter): 
            ...
            if (t is None or t.whitespaces_before_count > 2): 
                return None
            re = OrganizationAnalyzer.__try_attach_org_names(t)
            if (re is not None): 
                re.begin_token = t0
        ...
        if ((tok is None and not ok and tname1 is None) and prof == OrgProfile.UNDEFINED): 
            if (br is None or not t.chars.is_capital_upper): 
                return None
            if (not (isinstance(t00, TextToken))): 
                return None
            ...
            if (tok1 is not None): 
                ...
                if (t.chars.is_letter and not t.chars.is_cyrillic_letter): 
                    ...
                    first_pass4007 = True
                    while True:
                        if first_pass4007: first_pass4007 = False
                        else: tt = tt.next0_
                        if (not (tt is not None)): break
                        if (isinstance(tt.get_referent(), GeoReferent)): 
                            continue
        ...
        if (not ok): 
            ...
            first_pass4008 = True
            while True:
                if first_pass4008: first_pass4008 = False
            ...
            if (not ok): 
                ...
                first_pass4009 = True
                while True:
                    if first_pass4009: first_pass4009 = False
        ...
        if ((((((prof) & (OrgProfile.MEDIA))) != (OrgProfile.UNDEFINED))) and t0.previous is not None): 
            if (t0.previous.is_value("ГРУППА", None) or t0.previous.is_value("РОКГРУППА", None)): 
                t0 = t0.previous
                org0_.add_type_str(t0.get_normal_case_text(MorphClass.NOUN, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False).lower())
                org0_.add_profile(OrgProfile.CULTURE)
            elif ((t0.previous.is_value("ЖУРНАЛ", None) or t0.previous.is_value("ИЗДАНИЕ", None) or t0.previous.is_value("ИЗДАТЕЛЬСТВО", None)) or t0.previous.is_value("АГЕНТСТВО", None)): 
                t0 = t0.previous
                org0_.add_type_str(t0.get_normal_case_text(MorphClass.NOUN, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False).lower())
                if (t0.previous is None or not t0.previous.is_value("АГЕНТСТВО", None)): 
    ...
    @staticmethod
    def __try_attach_org_sport_associations(t : 'Token') -> 'ReferentToken':
        if (t is None): 
            return None
        cou = 0
        typ = None
    ...
    def __init_sport() -> None:
        ...
        for s in ["байдарка", "каноэ", "лук", "трава", "коньки", "трамплин", "двоеборье", "батут", "вода", "шпага", "сабля", "лыжи", "скелетон"]: 
            OrganizationAnalyzer.M_SPORTS.add(Termin._new2962(s.upper(), s))
        ...
        for s in ["СПАРТАК", "ЦСКА", "ЗЕНИТ!", "ТЕРЕК", "КРЫЛЬЯ СОВЕТОВ", "ДИНАМО", "АНЖИ", "КУБАНЬ", "АЛАНИЯ", "ТОРПЕДО", "АРСЕНАЛ!", "ЛОКОМОТИВ", "МЕТАЛЛУРГ!", "РОТОР", "СКА", "СОКОЛ!", "ХИМИК!", "ШИННИК", "РУБИН", "ШАХТЕР", "САЛАВАТ ЮЛАЕВ", "ТРАКТОР!", "АВАНГАРД!", "АВТОМОБИЛИСТ!", "АТЛАНТ!", "ВИТЯЗЬ!", "НАЦИОНАЛЬНАЯ ХОККЕЙНАЯ ЛИГА;НХЛ", "КОНТИНЕНТАЛЬНАЯ ХОККЕЙНАЯ ЛИГА;КХЛ", "СОЮЗ ЕВРОПЕЙСКИХ ФУТБОЛЬНЫХ АССОЦИАЦИЙ;УЕФА;UEFA", "Женская теннисная ассоциация;WTA", "Международная федерация бокса;IBF", "Всемирная боксерская организация;WBO", "РЕАЛ", "МАНЧЕСТЕР ЮНАЙТЕД", "манчестер сити", "БАРСЕЛОНА!", "БАВАРИЯ!", "ЧЕЛСИ", "ЛИВЕРПУЛЬ!", "ЮВЕНТУС", "НАПОЛИ", "БОЛОНЬЯ", "ФУЛХЭМ", "ЭВЕРТОН", "ФИЛАДЕЛЬФИЯ", "ПИТТСБУРГ", "ИНТЕР!", "Аякс", "ФЕРРАРИ;FERRARI", "РЕД БУЛЛ;RED BULL", "МАКЛАРЕН;MCLAREN", "МАКЛАРЕН-МЕРСЕДЕС;MCLAREN-MERCEDES"]: 
            ...
            t = Termin._new92(pp[0], OrgProfile.SPORT)
            if (not is_bad): 
                t.tag2 = (ss)
            if (len(pp) > 1): 
                if (len(pp[1]) < 4): 
        ...
        for s in ["ИТАР ТАСС;ТАСС;Телеграфное агентство советского союза", "Интерфакс;Interfax", "REGNUM", "ЛЕНТА.РУ;Lenta.ru", "Частный корреспондент;ЧасКор", "РИА Новости;Новости!;АПН", "Росбалт;RosBalt", "УНИАН", "ИНФОРОС;inforos", "Эхо Москвы", "Сноб!", "Серебряный дождь", "Вечерняя Москва;Вечерка", "Московский Комсомолец;Комсомолка", "Коммерсантъ;Коммерсант", "Афиша", "Аргументы и факты;АИФ", "Викиновости", "РосБизнесКонсалтинг;РБК", "Газета.ру", "Русский Репортер!", "Ведомости", "Вести!", "Рамблер Новости", "Живой Журнал;ЖЖ;livejournal;livejournal.ru", "Новый Мир", "Новая газета", "Правда!", "Известия!", "Бизнес!", "Русская жизнь!", "НТВ Плюс", "НТВ", "ВГТРК", "ТНТ", "Муз ТВ;МузТВ", "АСТ", "Эксмо", "Астрель", "Терра!", "Финанс!", "Собеседник!", "Newsru.com", "Nature!", "Россия сегодня;Russia Today;RT!", "БЕЛТА", "Ассошиэйтед Пресс;Associated Press", "France Press;France Presse;Франс пресс;Agence France Presse;AFP", "СИНЬХУА", "Gallup", "Cable News Network;CNN", "CBS News", "ABC News", "GoogleNews;Google News", "FoxNews;Fox News", "Reuters;Рейтер", "British Broadcasting Corporation;BBC;БиБиСи;BBC News", "MSNBC", "Голос Америки", "Аль Джазира;Al Jazeera", "Радио Свобода", "Радио Свободная Европа", "Guardian;Гардиан", "Daily Telegraph", "Times;Таймс!", "Independent!", "Financial Times", "Die Welt", "Bild!", "La Pepublica;Република!", "Le Monde", "People Daily", "BusinessWeek", "Economist!", "Forbes;Форбс", "Los Angeles Times", "New York Times", "Wall Street Journal;WSJ", "Washington Post", "Le Figaro;Фигаро", "Bloomberg", "DELFI!"]: 
            ...
            if (ss.endswith("!")): 
                ...
                ss = ss[0:0+len(ss) - 1]
            pp = Utils.splitString(ss, ';', False)
            t = Termin._new92(pp[0], OrgProfile.MEDIA)
            if (not is_bad): 
                t.tag2 = (ss)
            ii = 1
            while ii < len(pp): 
        ...
        for s in ["Машина времени!", "ДДТ", "Биттлз;Bittles", "ABBA;АББА", "Океан Эльзы;Океан Эльзи", "Аквариум!", "Крематорий!", "Наутилус;Наутилус Помпилиус!", "Пусси Райот;Пусси Риот;Pussy Riot", "Кино!", "Алиса!", "Агата Кристи!", "Чайф", "Ария!", "Земфира!", "Браво!", "Черный кофе!", "Воскресение!", "Урфин Джюс", "Сплин!", "Пикник!", "Мумий Троль", "Коррозия металла", "Арсенал!", "Ночные снайперы!", "Любэ", "Ласковый май!", "Noize MC", "Linkin Park", "ac dc", "green day!", "Pink Floyd;Пинк Флойд", "Depeche Mode", "Bon Jovi", "Nirvana;Нирвана!", "Queen;Квин!", "Nine Inch Nails", "Radioheads", "Pet Shop Boys", "Buggles"]: 
            ...
            if (ss.endswith("!")): 
                ...
                ss = ss[0:0+len(ss) - 1]
            pp = Utils.splitString(ss, ';', False)
            t = Termin._new92(pp[0], OrgProfile.MUSIC)
            if (not is_bad): 
                t.tag2 = (ss)
            ii = 1
            while ii < len(pp): 
        ...
        for s in ["ФАНАТ", "БОЛЕЛЬЩИК", "гонщик", "вратарь", "нападающий", "голкипер", "полузащитник", "полу-защитник", "центрфорвард", "центр-форвард", "форвард", "игрок", "легионер", "спортсмен"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new92(s.upper(), OrgProfile.SPORT))
        for s in ["защитник", "капитан", "пилот", "игра", "поле", "стадион", "гонка", "чемпионат", "турнир", "заезд", "матч", "кубок", "олипмиада", "финал", "полуфинал", "победа", "поражение", "разгром", "дивизион", "олипмиада", "финал", "полуфинал", "играть", "выигрывать", "выиграть", "проигрывать", "проиграть", "съиграть"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new94(s.upper(), OrgProfile.SPORT, s))
        for s in ["корреспондент", "фотокорреспондент", "репортер", "журналист", "тележурналист", "телеоператор", "главный редактор", "главред", "телеведущий", "редколлегия", "обозреватель", "сообщать", "сообщить", "передавать", "передать", "писать", "написать", "издавать", "пояснить", "пояснять", "разъяснить", "разъяснять", "сказать", "говорить", "спрашивать", "спросить", "отвечать", "ответить", "выяснять", "выяснить", "цитировать", "процитировать", "рассказать", "рассказывать", "информировать", "проинформировать", "поведать", "напечатать", "напоминать", "напомнить", "узнать", "узнавать", "репортаж", "интервью", "информации", "сведение", "ИА", "информагенство", "информагентство", "информационный", "газета", "журнал"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new92(s.upper(), OrgProfile.MEDIA))
        for s in ["сообщение", "статья", "номер", "журнал", "издание", "издательство", "агентство", "цитата", "редактор", "комментатор", "по данным", "оператор", "вышедший", "отчет", "вопрос", "читатель", "слушатель", "телезритель", "источник", "собеедник"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new94(s.upper(), OrgProfile.MEDIA, s))
        for s in ["музыкант", "певец", "певица", "ударник", "гитарист", "клавишник", "солист", "солистка", "исполнять", "исполнить", "концерт", "гастроль", "выступление", "известный", "известнейший", "популярный", "популярнейший", "рокгруппа", "панкгруппа", "артгруппа", "группа", "пластинка", "грампластинка", "концертный", "музыка", "запись", "студия"]: 
            OrganizationAnalyzer.M_PROP_PREF.add(Termin._new92(s.upper(), OrgProfile.MUSIC))
    ...
    def __try_attach_army(t : 'Token') -> 'ReferentToken':
        ...
        if (typ.root is not None and OrgProfile.ARMY in typ.root.profiles): 
            rt = OrganizationAnalyzer.__try_attach_org(t.next0_, OrganizationAnalyzer.AttachType.HIGH, None, False, -1)
            if (rt is not None): 
    ...
    BAN_AUTO_ABBREVIATIONS = False
    """ Если установить это в true, то для имён организаций не будут автоматически создаваться
    сокращения (аббревиатуры), которые искаться в тексте. """
    ...
    def create_analyzer_data(self) -> 'AnalyzerData':
        return OrgAnalyzerData()
    
    @staticmethod
    def _get_data(t : 'Token') -> 'OrgAnalyzerData':
        if (t is None): 
            return None
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        OrgItemTypeToken._prepare_all_data(kit.first_token)
        ad.tregime = True
        t = kit.first_token
        while t is not None: 
            t.inner_bool = False
        ...
        while step < steps: 
            ...
            while True:
                ...
                if (step > 0 and (isinstance(t, ReferentToken)) and (isinstance(t.get_referent(), OrganizationReferent))): 
                    ...
                    if (mt is not None): 
                        if (ad is not None): 
                            ...
                            ad.aliases.add(term)
                        rt = ReferentToken._new916(t.get_referent(), t, mt.end_token, t.morph)
                        kit.embed_token(rt)
                        t = (rt)
                for kk in range(5):
                    rts = OrganizationAnalyzer.__try_attach_orgs(t, step)
                    ...
                    if ((len(rts) == 1 and t == rts[0] and (isinstance(t.next0_, ReferentToken))) and (isinstance(t.next0_.get_referent(), OrganizationReferent))): 
                        ...
                        if (org1.higher is None and OrgOwnershipHelper.can_be_higher(org0, org1, False) and not OrgOwnershipHelper.can_be_higher(org1, org0, False)): 
                            ...
                            kit.debed_token(rtt)
                            org1.higher = org0
                            rt1 = ReferentToken._new916(ad.register_referent(org1), t, rtt.end_token, t.next0_.morph)
                            kit.embed_token(rt1)
                            t = (rt1)
                    ...
                    break
                if (step > 0): 
                    rt = OrganizationAnalyzer.__check_ownership(t)
                    if (rt is not None): 
                        kit.embed_token(rt)
                        t = (rt)
                if ((isinstance(t, ReferentToken)) and (isinstance(t.get_referent(), OrganizationReferent))): 
                    ...
                    kk = 0
                    while rt0 is not None and (kk < 4): 
                        rt00 = self.__try_attach_org_before(rt0)
                        if (rt00 is None): 
                            break
                        rt0 = rt00
                        OrganizationAnalyzer.__do_post_analyze(rt0)
        ...
        while True:
            ...
            if (((t1 is not None and t1.is_char('(') and t1.next0_ is not None) and (isinstance(t1.next0_.get_referent(), OrganizationReferent)) and t1.next0_.next0_ is not None) and t1.next0_.next0_.is_char(')')): 
                ...
                if (org0 == org0_ or org0_.higher == org0): 
                    rt1 = ReferentToken._new916(org0_, t, t1.next0_.next0_, t.morph)
                    kit.embed_token(rt1)
                    t = (rt1)
                    t1 = t.next0_
            ...
            if (of_tok is None): 
                if (org0_.higher is None): 
                    if (not OrgOwnershipHelper.can_be_higher(org1, org0_, False)): 
                        ok = False
                        if (t1.previous == t and (t1.whitespaces_after_count < 3)): 
                            pp = t.kit.process_referent("PERSON", t1.next0_, None)
                            if (pp is not None): 
                                ok = True
                        if ((t1.previous == t and (t1.whitespaces_before_count < 3) and len(org0_.names) == 0) and org0_.number is None and OrgProfile.UNIT in org0_.profiles): 
                            if (OrgOwnershipHelper.can_be_higher(org1, org0_, False)): 
                                ok = True
                        if (not ok): 
                            continue
        ...
        while True:
            ...
            for ty in org0_.types: 
                li = [ ]
                wrapli2976 = RefOutArgWrapper(None)
                inoutres2977 = Utils.tryGetValue(owners, ty, wrapli2976)
                li = wrapli2976.value
                if (not inoutres2977): 
                    li = list()
                    owners[ty] = li
        ...
        while True:
            ...
            for ty in org0_.types: 
                li = [ ]
                wrapli2978 = RefOutArgWrapper(None)
                inoutres2979 = Utils.tryGetValue(owners, ty, wrapli2978)
                li = wrapli2978.value
                if (not inoutres2979): 
                    continue
                for h in li: 
        ...
        while True:
            ...
            if ((nam == "ОВД" or nam == "ГУВД" or nam == "УВД") or nam == "ГУ" or nam == "МУ"): 
                continue
    ...
    def _check_alias_after(rt : 'ReferentToken', t : 'Token') -> 'MetaToken':
        ...
        while t is not None:
            if (not (isinstance(t, TextToken))): 
                ...
                t = t.next0_
            elif ((t.morph.class0_.is_preposition or t.morph.class0_.is_misc or t.is_value("ИМЕНОВАТЬ", None)) or t.is_value("ТЕКСТ", None) or t.is_value("ТАКЖЕ", None)): 
                t = t.next0_
            else: 
                break
        if (t is None): 
    ...
    def process_referent(self, begin : 'Token', param : str) -> 'ReferentToken':
        return OrganizationAnalyzer.process_referent_stat(begin, param)
    ...
    def process_referent_stat(begin : 'Token', param : str=None) -> 'ReferentToken':
        ...
        if (param == "TYPE"): 
            ad.level += 1
            ty = OrgItemTypeToken.try_attach(begin, False)
            ad.level -= 1
            if (ty is not None and ty.begin_token == begin): 
                org0_ = OrganizationReferent()
                org0_.add_type(ty, False)
                return ReferentToken._new916(org0_, begin, ty.end_token, ty.morph)
        if (param == "MINTYPE"): 
            ad.level += 1
            ty = OrgItemTypeToken.try_attach_pure_keywords(begin)
            ad.level -= 1
            if (ty is None): 
                return None
            org0_ = OrganizationReferent()
            ...
            return ReferentToken._new916(org0_, begin, ty.end_token, ty.morph)
        ad.level += 1
        rt = OrganizationAnalyzer.__try_attach_org(begin, (OrganizationAnalyzer.AttachType.HIGH if param == "STRONG" else OrganizationAnalyzer.AttachType.NORMAL), None, False, -1)
        if (rt is None): 
    ...
    @staticmethod
    def __try_attach_orgs(t : 'Token', step : int) -> typing.List['ReferentToken']:
        from pullenti.ner.org.internal.OrgItemNameToken import OrgItemNameToken
        if (t is None): 
            return None
        if (t.chars.is_latin_letter and MiscHelper.is_eng_article(t)): 
            res11 = OrganizationAnalyzer.__try_attach_orgs(t.next0_, step)
            if (res11 is not None and len(res11) > 0): 
                res11[0].begin_token = t
                return res11
        rt = None
        typ = None
        if (step == 0 or t.inner_bool or t.tag is not None): 
            typ = OrgItemTypeToken.try_attach(t, False)
            if (typ is not None): 
                t.inner_bool = True
            if (typ is None or typ.chars.is_latin_letter): 
                ...
                if (ltyp is not None): 
                    ...
                    if (rte is not None): 
                        OrganizationAnalyzer.__do_post_analyze(rte)
                        ree = list()
                        ree.append(rte)
                        return ree
        rt00 = OrganizationAnalyzer.__try_attach_spec(t)
        if (rt00 is None): 
            rt00 = OrganizationAnalyzer.__try_attach_org_by_alias(t)
        if (rt00 is not None): 
            res0 = list()
            OrganizationAnalyzer.__do_post_analyze(rt00)
            res0.append(rt00)
            return res0
        if (typ is not None): 
                ...
                if (((typ.morph.number) & (MorphNumber.PLURAL)) != (MorphNumber.UNDEFINED)): 
                    ...
                    if (ok): 
                        if (not ok1 and typ.coef > 0): 
                            ok1 = True
                        if (ok1): 
                            rt = OrganizationAnalyzer.__try_attach_org(t1.next0_, OrganizationAnalyzer.AttachType.MULTIPLE, typ, False, -1)
                    if (rt is not None): 
                        OrganizationAnalyzer.__do_post_analyze(rt)
                        res = list()
                        res.append(rt)
                        ...
                        while t1 is not None: 
                            ...
                            if (t1.is_char(',')): 
                                ...
                                break
                            t1 = t1.next0_
                            typ1 = OrgItemTypeToken.try_attach(t1, True)
                            if (typ1 is not None): 
                                ok = False
                                break
                            rt = OrganizationAnalyzer.__try_attach_org(t1, OrganizationAnalyzer.AttachType.MULTIPLE, typ, False, -1)
        ...
        if (typ is not None and ((typ.is_dep or typ.can_be_dep_before_organization))): 
            rt = OrganizationAnalyzer.__try_attach_dep_before_org(typ, None)
            if (rt is None): 
                rt = OrganizationAnalyzer.__try_attach_dep_after_org(typ)
            if (rt is None): 
                rt = OrganizationAnalyzer.__try_attach_org(typ.end_token.next0_, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
        ad = OrganizationAnalyzer._get_data(t)
        ...
        if (((step == 0 and rt is None and tt is not None) and not tt.chars.is_all_lower and tt.chars.is_cyrillic_letter) and tt.get_morph_class_in_dictionary().is_undefined): 
            s = tt.term
            if ((((s.startswith("ГУ") or s.startswith("РУ") or s.startswith("МУ")) or s.startswith("МО"))) and len(s) > 3 and ((len(s) > 4 or s == "ГУВД"))): 
                ...
                OrgItemTypeToken._recalc_data(tt)
                rt = OrganizationAnalyzer.__try_attach_org(tt, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
                tt.term = s
                ...
                if (rt is not None): 
                    ...
                    typ.typ = ("главное управление" if s.startswith("ГУ") else ("межмуниципальное" if s.startswith("МУ") else ("межмуниципальное отделение" if s.startswith("МО") else "региональное управление")))
                    if (s.startswith("МО")): 
                        typ.root = OrgItemTypeToken.M_MEJMUN_OTDEL
                    rt0 = OrganizationAnalyzer.__try_attach_dep_before_org(typ, rt)
                    if (rt0 is not None): 
                        if (ad is not None): 
                            rt.referent = ad.register_referent(rt.referent)
                ...
                if (rt is not None): 
                    ...
                    if (rt0 is not None): 
                        ...
                        if (rt0.referent.find_slot(OrganizationReferent.ATTR_GEO, None, True) is None): 
                            geo_ = OrganizationAnalyzer.__is_geo(rt0.end_token.next0_, False)
                            if ((geo_) is not None): 
                                if (rt0.referent._add_geo_object(geo_)): 
                                    rt0.end_token = OrganizationAnalyzer.__get_geo_end_token(geo_, rt0.end_token.next0_)
                                ...
                                if (nam is not None and not nam.value.startswith("СУБЪЕКТ")): 
                                    geo_ = OrganizationAnalyzer.__is_geo(nam.end_token.next0_, False)
                                    if ((geo_) is not None): 
                                        if (rt0.referent._add_geo_object(geo_)): 
                                            rt0.end_token = OrganizationAnalyzer.__get_geo_end_token(geo_, nam.end_token.next0_)
                        ...
                        if (len(rt0.referent.slots) > 3): 
                            if (tt.previous is not None and ((tt.previous.morph.class0_.is_adjective and not tt.previous.morph.class0_.is_verb)) and tt.whitespaces_before_count == 1): 
                                adj = None
                                try: 
                                    adj = MorphologyService.get_wordform(tt.previous.get_source_text().upper(), MorphBaseInfo._new2982(MorphClass.ADJECTIVE, gen, tt.previous.morph.language))
                                except Exception as ex2983: 
                                    pass
                                if (adj is not None and not adj.startswith("УПОЛНОМОЧ") and not adj.startswith("ОПЕРУПОЛНОМОЧ")): 
                                    ...
                                    if (rt0.begin_token.previous is not None and rt0.begin_token.previous.is_hiphen and rt0.begin_token.previous.previous is not None): 
                                        ...
                                        if (tt0.chars.equals(rt0.begin_token.chars) and (isinstance(tt0, TextToken))): 
                                            adj = tt0.term
                                            if (tt0.morph.class0_.is_adjective and not tt0.morph.contains_attr("неизм.", None)): 
                                                try: 
                                                    adj = MorphologyService.get_wordform(adj, MorphBaseInfo._new2982(MorphClass.ADJECTIVE, gen, tt0.morph.language))
                                                except Exception as ex2985: 
                                                    pass
            ...
            if (step > 0 and typ is None): 
                if (not BracketHelper.is_bracket(t, False)): 
                    ...
                    if (t.chars.is_all_lower): 
                        return None
            rt = OrganizationAnalyzer.__try_attach_org(t, OrganizationAnalyzer.AttachType.NORMAL, None, False, step)
        ...
        if (((rt is None and step == 1 and typ is not None) and typ.is_dep and typ.root is not None) and not typ.root.can_be_normal_dep): 
            if (OrgItemTypeToken.check_org_special_word_before(typ.begin_token.previous)): 
                rt = OrganizationAnalyzer.__try_attach_dep(typ, OrganizationAnalyzer.AttachType.HIGH, True)
        if (rt is None and step == 0 and t is not None): 
            ...
            if (ok): 
                ...
                if (t.chars.is_latin_letter and NumberHelper.try_parse_roman(t) is None): 
                    ...
                    if (br is not None): 
                        rt11 = OrganizationAnalyzer.__try_attach_org(t.next0_, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
            ...
            if (BracketHelper.can_be_start_of_sequence(t, False, False)): 
                ...
                if (br is not None): 
                    ...
                    if (t1 is not None and (t1.whitespaces_before_count < 3)): 
                        typ = OrgItemTypeToken.try_attach(t1, False)
                        if ((typ) is not None and typ.root is not None and typ.root.typ == OrgItemTypeTyp.PREFIX): 
                            ...
                            if (ok): 
                                ...
                                org0_.add_name(nam, True, None)
                                rt11 = OrganizationAnalyzer.__try_attach_org(br.begin_token.next0_, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
                ...
                if (BracketHelper.can_be_end_of_sequence(rt.end_token, True, None, False)): 
                    ...
                    while True:
                        ...
                        if (br is None): 
                            break
                        add_typ = False
                        rt1 = OrganizationAnalyzer.__try_attach_org_(ttt.next0_.next0_, ttt.next0_.next0_, None, True, OrganizationAnalyzer.AttachType.NORMAL, None, False)
                        if (rt1 is None or (rt1.end_char < (br.end_char - 1))): 
                            add_typ = True
                            rt1 = OrganizationAnalyzer.__try_attach_org_(ttt.next0_, ttt.next0_, None, True, OrganizationAnalyzer.AttachType.HIGH, None, False)
                        if (rt1 is None or (rt1.end_char < (br.end_char - 1))): 
                            break
                        li.append(rt1)
    ...
    @staticmethod
    def __try_attach_spec(t : 'Token') -> 'ReferentToken':
        rt = OrganizationAnalyzer.__try_attach_prop_names(t)
        if (rt is None): 
            rt = OrganizationAnalyzer.__try_attach_politic_party(t, False)
        if (rt is None): 
            rt = OrganizationAnalyzer.__try_attach_army(t)
        return rt
    ...
    def __do_post_analyze(rt : 'ReferentToken') -> None:
        ...
        if (o.higher is None and o._m_temp_parent_org is None): 
            ...
            if (o._m_temp_parent_org is None): 
                rt1 = OrganizationAnalyzer.__try_attach_org(rt.end_token.next0_, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
        ...
        if (rt.begin_token.previous is not None and rt.begin_token.previous.morph.class0_.is_adjective and (rt.whitespaces_before_count < 2)): 
            if (len(rt.referent.geo_objects) == 0): 
                geo_ = OrganizationAnalyzer.__is_geo(rt.begin_token.previous, True)
                if (geo_ is not None): 
        ...
        while True:
            ...
            if (isinstance(ttt, TextToken)): 
                ...
                if ((ttt.is_value("REGISTERED", None) and ttt.next0_ is not None and ttt.next0_.is_value("IN", None)) and (isinstance(ttt.next0_.next0_, ReferentToken)) and (isinstance(ttt.next0_.next0_.get_referent(), GeoReferent))): 
                    ...
                    continue
                if (br): 
                    otyp = OrgItemTypeToken.try_attach(ttt, True)
        ...
        while True:
            ...
            for n in org00.names: 
                ...
                if (t.is_value(str0_, None)): 
                    if (ad is not None): 
                        ad.aliases.add(Termin._new92(str0_, org00))
                    term = t.term
                    if (ii < 0): 
    ...
    @staticmethod
    def __attach_middle_attributes(org0_ : 'OrganizationReferent', t : 'Token') -> 'Token':
        te = None
        first_pass4021 = True
        while True:
    ...
    @staticmethod
    def __is_geo(t : 'Token', can_be_adjective : bool=False) -> object:
        if (t is None): 
            return None
        ...
        if (r is not None): 
            if (r.type_name == OrganizationAnalyzer.GEONAME): 
                if (t.begin_token.is_value("ТЕРРИТОРИЯ", None)): 
                    return None
                if (t.whitespaces_before_count <= 20 or t.morph.case_.is_genitive): 
                    return r
        ...
        if (t.whitespaces_before_count > 15 and not can_be_adjective): 
            return None
        if (t.is_value("ТЕРРИТОРИЯ", None) or t.is_value("ПОСЕЛЕНИЕ", None)): 
            return None
    ...
    @staticmethod
    def __get_geo_end_token(geo_ : object, t : 'Token') -> 'Token':
    ...
    def __attach_tail_attributes(org0_ : 'OrganizationReferent', t : 'Token', attach_for_new_org : bool, attach_typ : 'AttachType', is_global : bool=False) -> 'Token':
        ...
        while True:
            ...
            if (((t.is_value("ПО", None) or t.is_value("В", None) or t.is_value("IN", None))) and t.next0_ is not None): 
                ...
                if (not can_has_geo): 
                    break
                r = OrganizationAnalyzer.__is_geo(t.next0_, False)
                if (r is None): 
                    break
                if (not org0_._add_geo_object(r)): 
                    break
                t1 = OrganizationAnalyzer.__get_geo_end_token(r, t.next0_)
                t = t1
                continue
            ...
            if (can_has_geo and org0_.find_slot(OrganizationReferent.ATTR_GEO, None, True) is None and not t.is_newline_before): 
                r = OrganizationAnalyzer.__is_geo(t, False)
                if (r is not None): 
                    if (not org0_._add_geo_object(r)): 
                        break
                    t1 = OrganizationAnalyzer.__get_geo_end_token(r, t)
                    t = t1
                    continue
                if (t.is_char('(')): 
                    r = OrganizationAnalyzer.__is_geo(t.next0_, False)
            ...
            if (t.chars.is_latin_letter and (t.whitespaces_before_count < 2)): 
                ...
                if (has_latin_name): 
                    ...
                    if (eng is not None): 
                        ...
                        t = t1
                        continue
            re = OrganizationAnalyzer.__is_geo(t, False)
            if (re is None and t.is_char(',')): 
                re = OrganizationAnalyzer.__is_geo(t.next0_, False)
            if (re is not None): 
                if (attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                    ...
                    if (t.is_char(',')): 
                        t = t.next0_
                    t1 = OrganizationAnalyzer.__get_geo_end_token(re, t)
            ...
            if (t.is_char('(')): 
                    ...
                    if (tt0 is not None): 
                        ...
                        if (old_name and tt0 is not None): 
                            if (tt0.is_hiphen or tt0.is_char_of(",:")): 
                                tt0 = tt0.next0_
                    rt = OrganizationAnalyzer.__try_attach_org(tt0, OrganizationAnalyzer.AttachType.HIGH, None, False, 0)
                    if (rt is None): 
                        break
                ...
                if (nam is not None): 
                    org0_.add_name(nam, True, br.begin_token.next0_)
                rt1 = OrganizationAnalyzer.__try_attach_org(t.next0_, OrganizationAnalyzer.AttachType.HIGH, None, True, -1)
        ...
        if (t is not None and (t.whitespaces_before_count < 2) and ((ki == OrganizationKind.UNDEFINED or ki == OrganizationKind.BANK))): 
            ty1 = OrgItemTypeToken.try_attach(t, False)
            if (ty1 is not None and ty1.root is not None and ty1.root.is_pure_prefix): 
                rt22 = OrganizationAnalyzer.__try_attach_org(t, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
                if (rt22 is None): 
    ...
    @staticmethod
    def __correct_owner_before(res : 'ReferentToken') -> None:
        if (res is None): 
            return
        ...
        while True:
            ...
            if (isinstance(t, ReferentToken)): 
                ...
                break
            rt = OrganizationAnalyzer.__try_attach_org(t, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
            if (rt is not None): 
    ...
    @staticmethod
    def __check_ownership(t : 'Token') -> 'ReferentToken':
        if (t is None): 
            return None
        res = None
        org0_ = Utils.asObjectOrNull(t.get_referent(), OrganizationReferent)
                ...
                if (org2 is not None): 
                    ...
                    if (always or ok): 
                        ...
                        if (t0 is not None and t0.is_char(',')): 
                            t0 = t0.previous
                        rt = t.kit.process_referent("PERSON", t0, None)
                    ...
                    if (ok and ((org0_.higher is None or org0_.higher.can_be_equals(org2, ReferentsEqualType.WITHINONETEXT)))): 
                        ...
                        if (org0_.higher == org2): 
                            if (res is None): 
                                res = ReferentToken._new916(org0_, t, tt, tt0.morph)
                            else: 
                                res.end_token = tt
                            t = tt
                            if (len(org0_.geo_objects) == 0): 
                                ...
                                if (ttt is not None and ttt.is_value("В", None)): 
                                    ttt = ttt.next0_
                                if (OrganizationAnalyzer.__is_geo(ttt, False) is not None): 
    ...
    def process_ontology_item(self, begin : 'Token') -> 'ReferentToken':
        if (begin is None): 
            return None
        rt = OrganizationAnalyzer.__try_attach_org(begin, OrganizationAnalyzer.AttachType.EXTONTOLOGY, None, begin.previous is not None, -1)
        if (rt is not None): 
        ...
        b0 = begin
        e0 = et
        if (isinstance(begin, NumberToken)): 
            org0_.number = begin.value
            b0 = begin.next0_
            if (b0 is not None and b0.is_hiphen): 
                b0 = b0.next0_
            if (b0 is None or b0.end_char > et.end_char): 
                return None
        elif (isinstance(et, NumberToken)): 
            org0_.number = et.value
            e0 = et.previous
            if (e0 is not None and e0.is_hiphen): 
                e0 = e0.previous
            if (e0 is None or (e0.begin_char < b0.end_char)): 
                return None
        name_ = MiscHelper.get_text_value(b0, e0, GetTextAttr.NO)
    ...
    @staticmethod
    def __try_attach_org(t : 'Token', attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken'=None, is_additional_attach : bool=False, step : int=-1) -> 'ReferentToken':
        ad = OrganizationAnalyzer._get_data(t)
        if (ad is None): 
            return None
        if (ad.level > 4): 
            return None
        ad.level += 1
        res = OrganizationAnalyzer.__try_attach_org_int(t, attach_typ, mult_typ, is_additional_attach, step)
        ad.level -= 1
        return res
    ...
    def __try_attach_org_int(t : 'Token', attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken', is_additional_attach : bool, step : int) -> 'ReferentToken':
        if (t is None): 
            return None
        if (t.chars.is_latin_letter and MiscHelper.is_eng_article(t)): 
            re = OrganizationAnalyzer.__try_attach_org(t.next0_, attach_typ, mult_typ, is_additional_attach, step)
            if (re is not None): 
                re.begin_token = t
                return re
        org0_ = None
        ...
        while True:
            ...
            if (isinstance(t.get_referent(), OrganizationReferent)): 
                break
            rt0 = OrganizationAnalyzer.__attach_global_org(t, attach_typ, None)
            if ((rt0 is None and typ is not None and typ.geo is not None) and typ.begin_token.next0_ == typ.end_token): 
                rt0 = OrganizationAnalyzer.__attach_global_org(typ.end_token, attach_typ, typ.geo)
                if (rt0 is not None): 
            ...
            if (t.is_hiphen): 
                ...
                if (t.is_whitespace_before and t.is_whitespace_after): 
                    if (OrgItemTypeToken.try_attach(t.next0_, False) is not None): 
                        break
            ...
            if (ad is not None and ot_ex_li is None): 
                ...
                if (ok1 and tt is not None): 
                    ...
                    if (ot_ex_li is None and t.kit.ontology is not None): 
                        ...
                        if ((ot_ex_li) is not None): 
                            pass
                    if ((ot_ex_li is None and tt.length_char == 2 and tt.chars.is_all_upper) and (len(ad.local_ontology.items) < 1000)): 
            ...
            if ((step >= 0 and not t.inner_bool and t == t0) and (isinstance(t, TextToken))): 
                ...
                typ = OrgItemTypeToken.try_attach(t, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY)
                if (typ is None and BracketHelper.can_be_start_of_sequence(t, False, False)): 
                    br = BracketHelper.try_parse(t, BracketParseAttr.NO, 100)
                    if (br is not None): 
                        typ = OrgItemTypeToken.try_attach(t.next0_, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY)
            ...
            if (types is None): 
                ...
                if (typ0.typ == "банк" and typ.root is not None and typ.root.typ == OrgItemTypeTyp.PREFIX): 
                    rt = OrganizationAnalyzer.__try_attach_org(typ.begin_token, attach_typ, None, False, -1)
        ...
        if ((types is None and ot_ex_li is None and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))) and rt0 is None): 
            ...
            if (not ok): 
                if (t0 is not None and t0.morph.class0_.is_adjective and t0.next0_ is not None): 
                    rt0 = OrganizationAnalyzer.__try_attach_org(t0.next0_, attach_typ, mult_typ, is_additional_attach, step)
                    if ((rt0) is not None): 
                        if (rt0.begin_token == t0): 
                            return rt0
                if (attach_typ == OrganizationAnalyzer.AttachType.NORMAL): 
                    rt0 = OrganizationAnalyzer.__try_attach_org_med(t)
                    if ((rt0) is not None): 
                        return rt0
                if ((((isinstance(t0, TextToken)) and t0.previous is not None and t0.length_char > 2) and not t0.chars.is_all_lower and not t0.is_newline_after) and not MiscHelper.can_be_start_of_sentence(t0)): 
                    typ = OrgItemTypeToken.try_attach(t0.next0_, False)
                    if (typ is not None): 
                        rrr = OrganizationAnalyzer.__try_attach_org(t0.next0_, attach_typ, mult_typ, is_additional_attach, step)
                        if (rrr is None): 
                            if (spec_word_before or t0.previous.is_value("ТЕРРИТОРИЯ", None)): 
                                ...
                                t1 = typ.end_token
                                t1 = (Utils.ifNotNull(OrganizationAnalyzer.__attach_tail_attributes(org0, t1.next0_, False, OrganizationAnalyzer.AttachType.NORMAL, False), t1))
                ...
                while True:
                    ...
                    if ((isinstance(tt.next0_, ReferentToken)) and (isinstance(tt.next0_.get_referent(), OrganizationReferent))): 
                        ...
                        if (OrgProfile.UNIT in own.profiles): 
                            break
                        if ("СОЮЗ" in str(own).upper()): 
                            break
                        if (nam == "НК" or nam == "ГК"): 
                            return ReferentToken(own, tt0, tt.next0_)
                        org0 = OrganizationReferent()
                        org0.add_profile(OrgProfile.UNIT)
                        ...
                        org0.higher = own
                        t1 = tt.next0_
                        ttt1 = OrganizationAnalyzer.__attach_tail_attributes(org0, t1, True, attach_typ, False)
                ...
                if (((isinstance(t, TextToken)) and t.is_newline_before and t.length_char > 1) and not t.chars.is_all_lower and t.get_morph_class_in_dictionary().is_undefined): 
                    ...
                    if (t1 is not None and t1.is_newline_before): 
                        typ0 = OrgItemTypeToken.try_attach(t1, False)
                        if ((typ0 is not None and typ0.root is not None and typ0.root.typ == OrgItemTypeTyp.PREFIX) and typ0.is_newline_after): 
                            if (OrganizationAnalyzer.__try_attach_org(t1, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1) is None): 
                                org0_ = OrganizationReferent()
                                org0_.add_type(typ0, False)
                                org0_.add_name(MiscHelper.get_text_value(t, t1.previous, GetTextAttr.NO), True, None)
                                t1 = typ0.end_token
                                ttt1 = OrganizationAnalyzer.__attach_tail_attributes(org0_, t1.next0_, True, attach_typ, False)
                                return ReferentToken(org0_, t, Utils.ifNotNull(ttt1, t1))
                        if (t1.is_char('(')): 
                            typ0 = OrgItemTypeToken.try_attach(t1.next0_, False)
                            if ((typ0) is not None): 
        ...
        if (rt0 is None): 
            rt0 = OrganizationAnalyzer.__try_attach_org_(t0, t, types, spec_word_before, attach_typ, mult_typ, is_additional_attach)
            ...
            if (rt0 is not None): 
                ...
                if ((rt0.begin_token == t0 and t0.previous is not None and t0.previous.morph.class0_.is_adjective) and (t0.whitespaces_before_count < 2)): 
                    if (len(rt0.referent.geo_objects) == 0): 
                        geo_ = OrganizationAnalyzer.__is_geo(t0.previous, True)
                        if (geo_ is not None): 
                            if (rt0.referent._add_geo_object(geo_)): 
        ...
        if (ot_ex_li is not None and rt0 is None and (len(ot_ex_li) < 10)): 
            for ot in ot_ex_li: 
                ...
                if (len(org0.names) == 0 and len(org0.eponyms) == 0): 
                    continue
                tyty = OrgItemTypeToken.try_attach(ot.begin_token, True)
                ...
                if (ot.end_token.next0_ is not None and (isinstance(ot.end_token.next0_.get_referent(), OrganizationReferent))): 
                    ...
                    if (step == 0): 
                        if (not "o2step" in ot.kit.misc_data): 
                            ot.kit.misc_data["o2step"] = None
                        continue
                ...
                if (ok): 
                    ...
                    if (org0_.number is None): 
                        ...
                        if (nnn is None and not ot.is_whitespace_after and ttt is not None): 
                            ...
                            if (isinstance(ttt, NumberToken)): 
                                nnn = OrgItemNumberToken._new2359(ot.end_token.next0_, ttt, str(ttt.value))
                        if (nnn is not None): 
                    ...
                    if (t1.is_char(')') and t1.is_newline_after): 
                        ...
                        t1 = (Utils.ifNotNull(OrganizationAnalyzer.__attach_middle_attributes(org0_, t1.next0_), t1))
                        if (attach_typ != OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                            t1 = (Utils.ifNotNull(OrganizationAnalyzer.__attach_tail_attributes(org0_, t1.next0_, False, OrganizationAnalyzer.AttachType.NORMAL, False), t1))
                    hi = None
                    ...
                    if (org0_.kind == OrganizationKind.DEPARTMENT): 
                        OrganizationAnalyzer.__correct_dep_attrs(rt0, typ, False)
                    OrganizationAnalyzer.__correct_after(rt0)
                ...
                while tt00 is not None and cou > 0: 
                    if (isinstance(tt00.get_referent(), OrganizationReferent)): 
                        if (OrgItemTypeToken.is_type_accords(Utils.asObjectOrNull(tt00.get_referent(), OrganizationReferent), types[0])): 
                            if ((types[0].whitespaces_after_count < 3) and OrgItemTypeToken.try_attach(types[0].end_token.next0_, True) is not None): 
                                pass
                            else: 
        ...
        if (rt0 is not None): 
            OrganizationAnalyzer.__correct_owner_before(rt0)
        if (hiph and not in_brackets and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))): 
            ...
            if (rt0 is not None and BracketHelper.can_be_end_of_sequence(rt0.end_token, True, None, False)): 
                ...
                ty = OrgItemTypeToken.try_attach(rt0.end_token.next0_.next0_, False)
                if (ty is None): 
                    ok1 = True
            if (not ok1): 
                return None
        ...
        if (doubt and ad is not None and (len(ad.local_ontology.items) < 1000)): 
                ...
                for it in ad.local_ontology.items: 
                    if (it.referent is not None): 
                        if (it.referent.can_be_equals(org0_, ReferentsEqualType.WITHINONETEXT)): 
                            doubt = False
                            break
        if ((t is not None and t.kit.ontology is not None and attach_typ == OrganizationAnalyzer.AttachType.NORMAL) and doubt): 
            ...
            if (isinstance(tt, TextToken)): 
                if (tt.is_char(')')): 
                    ...
                    if (org0_.kind == OrganizationKind.BANK and tt.chars.is_latin_letter): 
                        pass
                    rt1 = OrganizationAnalyzer.__try_attach_org(tt, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
                    if (rt1 is not None and rt1.end_token.next0_ is not None and rt1.end_token.next0_.is_char(')')): 
                        if (org0_.can_be_equals(rt1.referent, ReferentsEqualType.FORMERGING)): 
                            org0_.merge_slots(rt1.referent, True)
                            rt0.end_token = rt1.end_token.next0_
        if (rt0.is_newline_before and rt0.is_newline_after and rt0.end_token.next0_ is not None): 
            t1 = rt0.end_token.next0_
            typ1 = OrgItemTypeToken.try_attach(t1, False)
            ...
            if (t1.is_char('(')): 
                typ1 = OrgItemTypeToken.try_attach(t1.next0_, False)
                if ((typ1) is not None): 
    ...
    def __try_attach_org_(t0 : 'Token', t : 'Token', types : typing.List['OrgItemTypeToken'], spec_word_before : bool, attach_typ : 'AttachType', mult_typ : 'OrgItemTypeToken', is_additional_attach : bool) -> 'ReferentToken':
        ...
        if (typ is not None): 
            if (typ.is_dep): 
                rt0 = OrganizationAnalyzer.__try_attach_dep(typ, attach_typ, spec_word_before)
                if (rt0 is not None): 
                    return rt0
        ...
        if (typ is not None and typ.root is not None and typ.root.is_pure_prefix): 
            if ((isinstance(t, TextToken)) and t.chars.is_all_upper and not t.is_newline_after): 
                ...
                rtg = Utils.asObjectOrNull(OrganizationAnalyzer.__is_geo(t, True), ReferentToken)
        ...
        if (((((ki0 == OrganizationKind.GOVENMENT or ki0 == OrganizationKind.AIRPORT or ki0 == OrganizationKind.FACTORY) or ki0 == OrganizationKind.SEAPORT or ki0 == OrganizationKind.PARTY) or ki0 == OrganizationKind.JUSTICE or ki0 == OrganizationKind.MILITARY)) and t is not None): 
            g = OrganizationAnalyzer.__is_geo(t, False)
            if (g is None and t.morph.class0_.is_preposition and t.next0_ is not None): 
                g = OrganizationAnalyzer.__is_geo(t.next0_, False)
            if (g is not None): 
                if (org0_._add_geo_object(g)): 
                    t1 = OrganizationAnalyzer.__get_geo_end_token(g, t)
                    te = t1
                    t = t1.next0_
                    gt = OrgGlobal.GLOBAL_ORGS.try_attach(t, None, False)
                    if (gt is None and t is not None and t.kit.base_language.is_ua): 
        ...
        if (typ is not None and typ.root is not None and ((typ.root.can_be_single_geo and not typ.root.can_has_single_name))): 
            ...
            if (t is not None and t1.next0_ is not None): 
                r = OrganizationAnalyzer.__is_geo(t1.next0_, False)
                if (r is None and t1.next0_.morph.class0_.is_preposition): 
                    r = OrganizationAnalyzer.__is_geo(t1.next0_.next0_, False)
            if (r is not None): 
                if (not org0_._add_geo_object(r)): 
                    return None
                te = OrganizationAnalyzer.__get_geo_end_token(r, t1.next0_)
            if (len(org0_.geo_objects) > 0 and te is not None): 
                ...
                if (npt11 is not None and (te.whitespaces_after_count < 2) and npt11.noun.is_value("ДЕПУТАТ", None)): 
                    ...
                    if (org0_.find_slot(OrganizationReferent.ATTR_TYPE, "посольство", True) is not None or org0_.find_slot(OrganizationReferent.ATTR_TYPE, "консульство", True) is not None): 
                        if (te.next0_ is not None and te.next0_.is_value("В", None)): 
                            r = OrganizationAnalyzer.__is_geo(te.next0_.next0_, False)
                            if (org0_._add_geo_object(r)): 
                                res11.end_token = OrganizationAnalyzer.__get_geo_end_token(r, te.next0_.next0_)
            ...
            if (typ is not None and br is None and BracketHelper.can_be_start_of_sequence(t, False, False)): 
                ...
                if (((typ.typ == "компания" or typ.typ == "предприятие" or typ.typ == "организация") or typ.typ == "компанія" or typ.typ == "підприємство") or typ.typ == "організація"): 
                    if (OrgItemTypeToken.is_decree_keyword(t0.previous, 1)): 
                        return None
                ty2 = OrgItemTypeToken.try_attach(t.next0_, False)
                if (ty2 is not None): 
                    typs2 = list()
                    typs2.append(ty2)
                    rt2 = OrganizationAnalyzer.__try_attach_org_(t.next0_, ty2.end_token.next0_, typs2, True, OrganizationAnalyzer.AttachType.HIGH, None, is_additional_attach)
                    if (rt2 is not None): 
        ...
        if (br is not None and br.is_quote_type): 
            if (br.begin_token.next0_.is_value("О", None) or br.begin_token.next0_.is_value("ОБ", None)): 
                ...
                br = (None)
        if (br is not None and br.is_quote_type and ((br.open_char != '<' or ((typ is not None and typ.root is not None and typ.root.is_pure_prefix)) or (((types is not None and len(types) > 0 and types[0].root is not None) and types[0].root.is_pure_prefix))))): 
            if (t.is_newline_before and ((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP))): 
                if (not br.is_newline_after): 
                    if (typ is None): 
                        return None
                    if (typ.is_newline_before or ((typ.begin_token.previous is not None and typ.begin_token.previous.is_table_control_char))): 
                        pass
                    else: 
                        return None
            ...
            if (na0 is not None and na0.is_empty_word and na0.end_token.next0_ == br.end_token): 
                return None
            rt0 = OrganizationAnalyzer.__try_attach_org(t.next0_, attach_typ, None, is_additional_attach, -1)
            ...
            if (((rt0 is None and t.next0_ is not None and (isinstance(t.next0_, TextToken))) and t.next0_.chars.is_all_upper and t.next0_.length_char > 2) and t.next0_.chars.is_cyrillic_letter): 
                rt0 = OrganizationAnalyzer.__try_attach_org(t.next0_.next0_, attach_typ, None, is_additional_attach, -1)
            ...
            if (ok2 and (isinstance(rt0.referent, OrganizationReferent))): 
                ...
                typ1 = None
                if (tt00 != t.next0_): 
                    typ1 = OrgItemTypeToken.try_attach(t.next0_, False)
                ...
                if (rt0 is not None): 
                    ...
                    t1 = OrganizationAnalyzer.__attach_tail_attributes(org0_, t11.next0_, True, attach_typ, False)
                    if (t1 is None): 
                        t1 = t11
                    if (typ is not None): 
            ...
            if (rt0 is not None and (rt0.end_char < br.end_token.previous.end_char)): 
                rt1 = OrganizationAnalyzer.__try_attach_org(rt0.end_token.next0_, attach_typ, None, is_additional_attach, -1)
                if (rt1 is not None and rt1.end_token.next0_ == br.end_token): 
                    return rt1
            ...
            for step in range(2):
                    ...
                    if (t1.is_char('(')): 
                        ...
                        if (r is not None and t1.next0_.next0_ is not None and t1.next0_.next0_.is_char(')')): 
                            if (r.type_name == OrganizationAnalyzer.GEONAME): 
                                ...
                                break
                        rt = OrganizationAnalyzer.__try_attach_org(t1.next0_, OrganizationAnalyzer.AttachType.HIGH, None, False, -1)
                        ...
                        if ((org0) is not None): 
                            ...
                            if (typ is not None and ((pref or not typ.is_dep))): 
                                if (not_empty > 1): 
                                    rrr = OrganizationAnalyzer.__try_attach_org(typ.begin_token, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
                                    if (rrr is not None): 
                ...
                if (not Utils.isNullOrEmpty(s)): 
                    ...
                    if (typ is not None and typ.root is not None and typ.root.canonic_text.startswith("ИНДИВИДУАЛЬН")): 
                        pers = typ.kit.process_referent("PERSON", tt0, None)
                ...
                if (org0_.number is None and t1.next0_ is not None and (t1.whitespaces_after_count < 2)): 
                    ...
                    if (num1 is not None): 
                        ...
                        t1 = OrganizationAnalyzer.__attach_tail_attributes(org0_, t1.next0_, True, attach_typ, False)
                else: 
                    t1 = OrganizationAnalyzer.__attach_tail_attributes(org0_, t1.next0_, True, attach_typ, False)
                if (t1 is None): 
                    t1 = br.end_token
                ...
                if (ok0): 
                    ...
                    return ReferentToken._new918(org0_, t0, t1, org0_)
        num = None
        _num = None
        epon = None
        _epon = None
        ...
        while True:
            ...
            if (isinstance(t.get_referent(), OrganizationReferent)): 
                pass
            rt = None
            rt = OrganizationAnalyzer.__attach_global_org(t, attach_typ, None)
            if ((rt) is not None): 
                if (t == t0): 
                    if (not t.chars.is_all_lower): 
                        return rt
                    return None
                rt = OrganizationAnalyzer.__try_attach_org(t, attach_typ, mult_typ, is_additional_attach, -1)
                if (rt is not None): 
                    return rt
            ...
            if ((br) is not None): 
                ...
                if (br.is_quote_type): 
                    if (br.open_char == '<' or br.whitespaces_before_count > 1): 
                        break
                    rt = OrganizationAnalyzer.__try_attach_org(t, OrganizationAnalyzer.AttachType.HIGH, None, False, -1)
                    if (rt is None): 
                        break
            ...
            if (na is None and t is not None): 
                if (org0_.kind == OrganizationKind.CHURCH or ((typ is not None and typ.typ is not None and "фермер" in typ.typ))): 
                    prt = t.kit.process_referent("PERSON", t, None)
                    if (prt is not None): 
                        na = OrgItemNameToken._new2990(t, prt.end_token, True)
                        na.value = MiscHelper.get_text_value_of_meta_token(na, GetTextAttr.NO)
                        na.chars = CharsInfo._new2991(True)
                        na.morph = prt.morph
            ...
            if (na is None): 
                ...
                if ((((names is None and t.is_char('/') and (isinstance(t.next0_, TextToken))) and not t.is_whitespace_after and t.next0_.chars.is_all_upper) and t.next0_.length_char >= 3 and (isinstance(t.next0_.next0_, TextToken))) and not t.next0_.is_whitespace_after and t.next0_.next0_.is_char('/')): 
                    na = OrgItemNameToken._new2992(t, t.next0_.next0_, t.next0_.get_source_text().upper(), t.next0_.chars)
            ...
            if (attach_typ == OrganizationAnalyzer.AttachType.MULTIPLE or attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP): 
                if (not na.is_std_tail and not na.chars.is_latin_letter and na.std_org_name_nouns == 0): 
                    if (t.morph.class0_.is_proper_name and not t.chars.is_all_upper): 
                        break
                    ...
                    if (typ is not None and typ.typ == "союз" and not t.morph.case_.is_genitive): 
                        break
                    pit = t.kit.process_referent("PERSONPROPERTY", t, None)
                    if (pit is not None): 
                        if (pit.morph.number == MorphNumber.SINGULAR and pit.begin_token != pit.end_token): 
                            if (typ is not None and typ.typ == "служба" and pit.morph.case_.is_genitive): 
                                pass
                            else: 
                                break
                    pit = t.kit.process_referent("DECREE", t, None)
                    if (pit is not None): 
                        ...
                        if (nptt is not None and nptt.end_token.is_value("РЕШЕНИЕ", None)): 
                            ...
                            break
                    pit = t.kit.process_referent("NAMEDENTITY", t, None)
                    if (pit is not None and pit.end_token != t): 
                        break
            ...
            if (names is None): 
                ...
                if (t.previous is not None and t.is_newline_before and attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                    if (typ is not None and typ.end_token.next0_ == t and ((typ.is_newline_before or BracketHelper.can_be_start_of_sequence(typ.begin_token.previous, False, False)))): 
                        pass
                    else: 
                    ...
                    if (t.whitespaces_before_count > 10): 
                        break
                    if (not t.chars.equals(t.previous.chars)): 
                        break
                ...
                if (typ is not None and ((typ.root is None or not typ.root.is_pure_prefix))): 
                    ...
                    if ((na.is_in_dictionary and na.morph.language.is_cyrillic and na.chars.is_all_lower) and not na.morph.case_.is_undefined): 
                        if (na.preposition is None): 
                            ...
                            if (na.morph.number != MorphNumber.PLURAL): 
                                prr = t.kit.process_referent("PERSONPROPERTY", t, None)
                                if (prr is not None): 
                                    if (OrgItemEponymToken.try_attach(na.end_token.next0_, False) is not None): 
                                        pass
                                    elif (typ is not None and typ.typ == "служба"): 
                                        typ.coef = typ.coef + (3)
                                    else: 
                                        break
                    if (na.preposition is not None): 
                        ...
                        prt = na.kit.process_referent("PERSON", na.begin_token, None)
                        if (prt is not None): 
        ...
        if (typ is not None): 
            if (((attach_typ == OrganizationAnalyzer.AttachType.NORMAL or attach_typ == OrganizationAnalyzer.AttachType.NORMALAFTERDEP)) and typ.morph.number == MorphNumber.PLURAL): 
                if (names is not None and len(names) > 0 and names[len(names) - 1].is_newline_after): 
                    pass
                elif (num is not None and num.is_newline_after): 
                    pass
                else: 
                    return None
            if (LanguageHelper.ends_with_ex(typ.typ, "комитет", "комиссия", "комітет", "комісія")): 
                ...
                if (isinstance(typ.begin_token.previous, NumberToken)): 
                    ...
                    num = OrgItemNumberToken._new2359(t0, t0, typ.begin_token.previous.value)
                ...
                if (names is None and ((typ.name is None or Utils.compareStrings(typ.name, typ.typ, True) == 0))): 
                    if ((attach_typ != OrganizationAnalyzer.AttachType.EXTONTOLOGY and typ.typ != "следственный комитет" and typ.typ != "кабинет министров") and typ.typ != "слідчий комітет"): 
                        if (((typ.typ == "администрация" or typ.typ == "адміністрація")) and (isinstance(typ.end_token.next0_, TextToken))): 
                            rt1 = typ.kit.process_referent("PERSONPROPERTY", typ.end_token.next0_, None)
        ...
        if (((num is not None or org0_.number is not None or epon is not None) or attach_typ == OrganizationAnalyzer.AttachType.HIGH or attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY) or own_org is not None): 
            ...
            if (typ is None): 
                if (names[0].is_std_name and spec_word_before): 
                    ...
                    t = OrganizationAnalyzer.__attach_tail_attributes(org0_, t1.next0_, True, attach_typ, False)
                    if (t is not None): 
                        t1 = t
                    return ReferentToken(org0_, t0, t1)
                return None
            ...
            if (names[0].chars.is_latin_letter): 
                ...
                j = 1
                while j < len(names): 
                    if (names[j].is_newline_before or not names[j].chars.equals(names[0].chars)): 
                        break
                    else: 
                        t1 = names[j].end_token
                    j += 1
                ...
                while True:
                    ...
                    if (names[j].is_newline_before and j > 0): 
                        if (names[j].newlines_before_count > 1): 
                            break
                        if (names[j].chars.is_all_lower): 
                            pass
                        else: 
                            break
                    if (not names[j].chars.equals(names[0].chars) and not names[j].begin_token.chars.equals(names[0].chars)): 
                        break
                    if (names[j].is_noun_phrase != names[0].is_noun_phrase): 
                        break
                ...
                if (koef >= 3 and t1.next0_ is not None): 
                    ...
                    if (r is not None and ((r.type_name == OrganizationAnalyzer.GEONAME or r.type_name == OrganizationReferent.OBJ_TYPENAME))): 
                        koef += (1)
                    elif (OrganizationAnalyzer.__is_geo(t1.next0_, False) is not None): 
                        koef += (1)
                    elif (t1.next0_.is_char('(') and OrganizationAnalyzer.__is_geo(t1.next0_.next0_, False) is not None): 
                        koef += (1)
                    elif (spec_word_before and t1.kit.process_referent("PERSON", t1.next0_, None) is not None): 
                        koef += (1)
                if (koef >= 4): 
                    ok = True
                if (not ok): 
                ...
                if ((not ok and oki == OrganizationKind.FEDERATION and names[0].morph.case_.is_genitive) and koef > 0): 
                    if (OrganizationAnalyzer.__is_geo(names[len(names) - 1].end_token.next0_, False) is not None): 
                        ok = True
                if (not ok and typ is not None and typ.root is not None): 
                    if (len(names) == 1 and ((names[0].chars.is_all_upper or names[0].chars.is_last_lower))): 
                        if ((ki == OrganizationKind.BANK or ki == OrganizationKind.CULTURE or ki == OrganizationKind.HOTEL) or ki == OrganizationKind.MEDIA or ki == OrganizationKind.MEDICAL): 
                            ok = True
                if (((not ok and typ is not None and typ.typ == "компания") and len(names) == 1 and not names[0].chars.is_all_lower) and (typ.whitespaces_after_count < 3)): 
                    ok = True
                if (ok): 
                    tt1 = t1
                    if (br1 is not None): 
                    ...
                    if ((names[0].begin_token == t1 and s is not None and t1.morph.case_.is_genitive) and t1.chars.is_capital_upper): 
                        if (cla.is_undefined or cla.is_proper_geo): 
                            if (ki == OrganizationKind.MEDICAL or ki == OrganizationKind.JUSTICE): 
                                ...
                                rt.data = (OrganizationAnalyzer._get_data(t1))
            ...
            if ((typ.typ == "банк" and (isinstance(t, ReferentToken)) and not t.is_newline_before) and typ.morph.number == MorphNumber.SINGULAR): 
                ...
                r = t.get_referent()
                if (r.type_name == OrganizationAnalyzer.GEONAME and not t.morph.case_.equals(MorphCase.NOMINATIVE)): 
                    org0_._add_geo_object(r)
            ...
            if (typ.root is not None and typ.root.must_has_capital_name): 
                return None
            if (typ.name is None): 
                if (((typ.typ.endswith("университет") or typ.typ.endswith("університет"))) and OrganizationAnalyzer.__is_geo(typ.end_token.next0_, False) is not None): 
                    always = True
                    ...
                    if (typ.end_token.next0_ is not None and ((typ.end_token.next0_.is_hiphen or typ.end_token.next0_.is_char_of(":")))): 
                        ...
                        ad = OrganizationAnalyzer._get_data(t)
                        li = (None if ad is None or len(ad.local_ontology.items) > 1000 else ad.local_ontology.try_attach_by_item(org0_.create_ontology_item()))
                    ...
                    first_pass4033 = True
                    while True:
                        if first_pass4033: first_pass4033 = False
                        else: tt = tt.previous; cou += 1
                        ...
                        if (not org0.can_be_equals(org0_, ReferentsEqualType.WITHINONETEXT)): 
                            continue
                        tt = (Utils.ifNotNull(OrganizationAnalyzer.__attach_tail_attributes(org0_, typ.end_token.next0_, False, attach_typ, False), typ.end_token))
                        if (not org0.can_be_equals(org0_, ReferentsEqualType.WITHINONETEXT)): 
                            break
                        org0_.merge_slots(org0, True)
                    ...
                    if (typ.root is not None and typ.root.can_be_single_geo and t1.next0_ is not None): 
                        ggg = OrganizationAnalyzer.__is_geo(t1.next0_, False)
                        if (ggg is not None): 
                            org0_._add_geo_object(ggg)
                            t1 = OrganizationAnalyzer.__get_geo_end_token(ggg, t1.next0_)
                            return ReferentToken(org0_, t0, t1)
                    return None
        ...
        if (t1.is_newline_after and t0.is_newline_before): 
            ...
            if (typ1 is not None and typ1.is_newline_after): 
                if (OrganizationAnalyzer.__try_attach_org(t1.next0_, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1) is None): 
                    org0_.add_type(typ1, False)
                    t1 = typ1.end_token
            if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                typ1 = OrgItemTypeToken.try_attach(t1.next0_.next0_, False)
                if ((typ1) is not None): 
    ...
    def __try_attach_org_before(self, t : 'Token') -> 'ReferentToken':
        ...
        while t0 is not None: 
            ...
            first_pass4034 = True
            while True:
                if first_pass4034: first_pass4034 = False
                else: ttt = ttt.previous
                if (not (ttt is not None)): break
                if (ttt.whitespaces_after_count > 1): 
                    break
                if (isinstance(ttt, ReferentToken)): 
                    break
                ...
                if (mc.is_verb and not mc.is_preposition): 
                    break
                typ1 = OrgItemTypeToken.try_attach(ttt, True)
                if (typ1 is None or typ1.begin_token != ttt): 
                    if (cou == 1): 
                        break
                    continue
                if (typ1.end_token == tt0): 
                    t0 = ttt
            if ((isinstance(t0.get_referent(), OrganizationReferent)) and org0 is not None): 
                oo = Utils.asObjectOrNull(t0.get_referent(), OrganizationReferent)
                if (oo.higher is None and OrgOwnershipHelper.can_be_higher(org0, oo, False)): 
                    oo.higher = org0
                    return ReferentToken(oo, t0, t)
            if (isinstance(t0, ReferentToken)): 
                break
            rt = OrganizationAnalyzer.__try_attach_org(t0, OrganizationAnalyzer.AttachType.NORMAL, None, False, -1)
            if (rt is not None): 
                if (rt.begin_token != t0): 
                    return None
                ...
                if (mc.is_verb and not mc.is_preposition): 
                    break
                if (mc.is_adverb or mc.is_conjunction): 
                    break
            t0 = t0.previous
        if (res is not None): 
            return None
        ...
        first_pass4035 = True
        while True:
            if first_pass4035: first_pass4035 = False
            else: t0 = t0.previous
            ...
            if (not t0.chars.is_letter): 
                break
            ty = OrgItemTypeToken.try_attach(t0, True)
            if (ty is not None): 
                ...
                if (typ is not None and ty.begin_char > typ.begin_char): 
                    break
        ...
        if (typ is not None and typ.is_dep): 
            res = OrganizationAnalyzer.__try_attach_dep_before_org(typ, None)
        return res
    
    @staticmethod
    def __try_attach_politic_party(t : 'Token', only_abbrs : bool=False) -> 'ReferentToken':
        if (not (isinstance(t, TextToken))): 
            return None
        name_tok = None
        root = None
        ...
        first_pass4036 = True
        while True:
            if first_pass4036: first_pass4036 = False
            else: t = t.next0_
            if (not (t is not None)): break
        ...
        if ((name_tok) is not None and not t.chars.is_all_lower): 
            is_politic = True
            if (name_tok.begin_token == name_tok.end_token and not t.get_morph_class_in_dictionary().is_undefined): 
                coef += 0.1
            else: 
                coef += 0.5
            if ((isinstance(name_tok.end_token, TextToken)) and "ПАРТИ" in name_tok.end_token.term): 
                coef += 0.5
            ...
            if ((br) is not None): 
                ...
                if ((name_tok) is not None): 
                    ...
                    while tt is not None and tt.end_char <= br.end_char: 
                        ...
                        if (tok2 is not None and tok2.termin.tag is None): 
                            ...
                            mc = tt.get_morph_class_in_dictionary()
                            if ((mc.equals(MorphClass.VERB) or mc.equals(MorphClass.ADVERB) or mc.is_pronoun) or mc.is_personal_pronoun): 
                                coef = (0)
                                break
                ...
                first_pass4037 = True
                while True:
                    if first_pass4037: first_pass4037 = False
                    else: tt = tt.next0_
                    if (not (tt is not None)): break
                    if (tt.chars.is_all_lower and tt == t and tt.get_morph_class_in_dictionary().is_preposition): 
                        break
                    if (tt.is_comma_and and tt == t): 
                        break
                    if (isinstance(tt.get_referent(), GeoReferent)): 
                        break
                    if (tt.whitespaces_before_count > 2): 
                    ...
                    if (tt.is_and): 
                        ...
                        if (npt2 is not None and OrganizationAnalyzer.M_POLITIC_SUFFS.try_parse(npt2.end_token, TerminParseAttr.NO) is not None and npt2.end_token.chars.equals(tt.previous.chars)): 
                            continue
                        break
                    ...
                    if (co == 0): 
                        ...
                        if (((words_after > 0 and npt.end_token.chars.equals(tt.previous.chars))) or ((last_suf is not None and last_suf.termin.tag is not None)) or ((tt.previous == root and npt.end_token.chars.is_all_lower and npt.morph.number == MorphNumber.PLURAL) and root.chars.is_capital_upper)): 
                            pp = tt.kit.process_referent("PERSON", tt, None)
                            if (pp is not None): 
                                break
                            words_after += 1
                        else: 
        ...
        first_pass4038 = True
        while True:
            if first_pass4038: first_pass4038 = False
            else: tt = tt.previous
            if (not (tt is not None)): break
            if (not (isinstance(tt, TextToken))): 
                org1 = Utils.asObjectOrNull(tt.get_referent(), OrganizationReferent)
    ...
    def __init_politic() -> None:
        ...
        for s in ["объединенный", "всероссийский", "общероссийский", "христианский", "независимый", "альтернативный"]: 
            OrganizationAnalyzer.M_POLITIC_PREFS.add(Termin._new2962(s.upper(), s))
        for s in ["политический", "правящий", "оппозиционный", "запрешенный", "террористический", "запрещенный", "экстремистский"]: 
            OrganizationAnalyzer.M_POLITIC_PREFS.add(Termin._new92(s.upper(), s))
        for s in ["активист", "член", "руководство", "лидер", "глава", "демонстрация", "фракция", "съезд", "пленум", "террорист", "парламент", "депутат", "парламентарий", "оппозиция", "дума", "рада"]: 
            OrganizationAnalyzer.M_POLITIC_PREFS.add(Termin._new94(s.upper(), s, s))
        ...
        for s in ["реформа", "свобода", "единство", "развитие", "освобождение", "любитель", "поддержка", "возрождение", "независимость"]: 
            OrganizationAnalyzer.M_POLITIC_SUFFS.add(Termin._new92(s.upper(), s))
        ...
        for s in ["Республиканская партия", "Демократическая партия;Демпартия", "Христианско демократический союз;ХДС", "Свободная демократическая партия;СвДП", "ЯБЛОКО", "ПАРНАС", "ПАМЯТЬ", "Движение против нелегальной иммиграции;ДПНИ", "НАЦИОНАЛ БОЛЬШЕВИСТСКАЯ ПАРТИЯ;НБП", "НАЦИОНАЛЬНЫЙ ФРОНТ;НАЦФРОНТ", "Национальный патриотический фронт", "Батькивщина;Батькiвщина", "НАРОДНАЯ САМООБОРОНА", "Гражданская платформа", "Народная воля", "Славянский союз", "ПРАВЫЙ СЕКТОР", "ПЕГИДА;PEGIDA", "Венгерский гражданский союз;ФИДЕС", "БЛОК ЮЛИИ ТИМОШЕНКО;БЮТ", "Аль Каида;Аль Каеда;Аль Кайда;Al Qaeda;Al Qaida", "Талибан;движение талибан", "Бригады мученников Аль Аксы", "Хезболла;Хезбалла;Хизбалла", "Народный фронт освобождения палестины;НФОП", "Организация освобождения палестины;ООП", "Союз исламского джихада;Исламский джихад", "Аль-Джихад;Египетский исламский джихад", "Братья-мусульмане;Аль Ихван альМуслимун", "ХАМАС", "Движение за освобождение Палестины;ФАТХ", "Фронт Аль Нусра;Аль Нусра", "Джабхат ан Нусра"]: 
            pp = Utils.splitString(s.upper(), ';', False)
            t = Termin._new92(pp[0], OrgProfile.POLICY)
            i = 0
            while i < len(pp): 
    ...
    @staticmethod
    def __try_attach_dep_before_org(typ : 'OrgItemTypeToken', rt_org : 'ReferentToken') -> 'ReferentToken':
        if (typ is None): 
            return None
        ...
        if ((isinstance(res.begin_token.previous, ReferentToken)) and org0_ is not None and org0_.can_be_equals(res.begin_token.previous.get_referent(), ReferentsEqualType.WITHINONETEXT)): 
            res.begin_token = res.begin_token.previous
        OrganizationAnalyzer.__correct_dep_attrs(res, typ, False)
    ...
    @staticmethod
    def __try_attach_dep_after_org(typ : 'OrgItemTypeToken') -> 'ReferentToken':
        if (typ is None): 
            return None
    ...
    def __try_attach_dep(typ : 'OrgItemTypeToken', attach_typ : 'AttachType', spec_word_before : bool) -> 'ReferentToken':
        ...
        if (typ.end_token.next0_ is not None and (typ.end_token.whitespaces_after_count < 2)): 
            if (isinstance(typ.end_token.next0_.get_referent(), OrganizationReferent)): 
                after_org = (Utils.asObjectOrNull(typ.end_token.next0_.get_referent(), OrganizationReferent))
            else: 
                ...
                if (na0 is not None and ((na0.std_org_name_nouns > 0 or na0.is_std_name))): 
                    ...
                    rt00 = OrganizationAnalyzer.__try_attach_org(typ.end_token.next0_, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
                    if (rt00 is None and BracketHelper.can_be_start_of_sequence(typ.end_token.next0_, True, False)): 
                        rt00 = OrganizationAnalyzer.__try_attach_org(typ.end_token.next0_.next0_, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
                        if (rt00 is not None): 
                            in_br = True
        ...
        t1 = typ.end_token
        if ((typ.number is None and ((typ.typ == "отдел" or typ.typ == "лаборатория")) and (isinstance(typ.end_token.next0_, NumberToken))) and spec_word_before and (typ.whitespaces_after_count < 3)): 
            t1 = typ.end_token.next0_
            dep.number = t1.value
            typ.coef = typ.coef + (2)
        if ((isinstance(t, TextToken)) and after_org is None and (((LanguageHelper.ends_with(typ.typ, "аппарат") or LanguageHelper.ends_with(typ.typ, "апарат") or LanguageHelper.ends_with(typ.typ, "совет")) or LanguageHelper.ends_with(typ.typ, "рада")))): 
            ...
            if (tt1.is_value("ПРИ", None)): 
                tt1 = tt1.next0_
            pr1 = t.kit.process_referent("PERSON", tt1, None)
        ...
        while True:
            ...
            if ((num) is not None): 
                if (t.is_newline_before or typ.number is not None): 
                    break
                if (typ.root is not None and not typ.root.can_has_number): 
                    break
                if ((isinstance(typ.begin_token.previous, NumberToken)) and (typ.whitespaces_before_count < 2)): 
                    typ2 = OrgItemTypeToken.try_attach(num.end_token.next0_, True)
                ...
                ty0 = OrgItemTypeToken.try_attach(t, True)
                    ...
                    if ((br00) is not None and names is None): 
                        ...
                        if (r is not None): 
                            if (isinstance(r, OrganizationReferent)): 
                                ...
                                after_org_tok = t
                                if (names is None and (t.whitespaces_after_count < 3) and BracketHelper.can_be_start_of_sequence(t.next0_, True, False)): 
                                    continue
                                break
                        ...
                        if ((isinstance(t, NumberToken)) and typ.root is not None and dep.number is None): 
                            if (t.whitespaces_before_count > 1 or not typ.root.can_has_number): 
                                break
                            if ((isinstance(typ.begin_token.previous, NumberToken)) and (typ.whitespaces_before_count < 2)): 
                                typ2 = OrgItemTypeToken.try_attach(t.next0_, True)
                                if (typ2 is not None and typ2.root is not None and ((typ2.root.can_has_number or typ2.is_dep))): 
                                    typ.begin_token = typ.begin_token.previous
                                    typ.number = typ.begin_token.value
                                    dep.number = typ.number
                        ...
                        if (is_pure_dep): 
                            break
                        if (not t.chars.is_all_lower): 
                            rtp = t.kit.process_referent("PERSON", t, None)
                        ...
                        if (typ.typ == "генеральный штаб" or typ.typ == "генеральний штаб"): 
                            rtp = t.kit.process_referent("PERSONPROPERTY", t, None)
                            if (rtp is not None): 
                                break
                        na = OrgItemNameToken.try_attach(t, pr, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, names is None)
                        if (t.is_value("ПО", None) and t.next0_ is not None): 
                            tt = t.next0_
                            if (tt.is_value("ОБСЛУЖИВАНИЕ", None) and tt.next0_ is not None): 
                                tt = tt.next0_
                            if (tt.is_value("РАЙОН", None) or tt.is_value("МИКРОРАЙОН", None)): 
                                na = OrgItemNameToken.try_attach(tt.next0_, pr, attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY, True)
                        if (t.morph.class0_.is_preposition and ((t.is_value("ПРИ", None) or t.is_value("OF", None) or t.is_value("AT", None)))): 
                            if ((isinstance(t.next0_, ReferentToken)) and (isinstance(t.next0_.get_referent(), OrganizationReferent))): 
                                ...
                                break
                            rt0 = OrganizationAnalyzer.__try_attach_org(t.next0_, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
                            if (rt0 is not None): 
                        ...
                        if (names is None): 
                            ...
                            if (NumberHelper.try_parse_roman(t) is not None): 
                                break
                            rt0 = OrganizationAnalyzer.__try_attach_org(t, OrganizationAnalyzer.AttachType.NORMALAFTERDEP, None, False, -1)
                            if (rt0 is not None): 
                                after_org = (Utils.asObjectOrNull(rt0.referent, OrganizationReferent))
                                after_org_temp = True
                                break
                            ...
                            if (t.whitespaces_before_count > 2 and not na.chars.equals(pr.chars)): 
                                break
        ...
        if (after_org is None): 
            ...
            while ttt is not None: 
                if (isinstance(ttt.get_referent(), OrganizationReferent)): 
                    ...
                    break
                elif (ttt.is_char(';') and attach_typ == OrganizationAnalyzer.AttachType.EXTONTOLOGY): 
                    break
                ttt = ttt.next0_
        ...
        if (typ.chars.is_capital_upper): 
            coef += 0.5
        elif (not typ.chars.is_all_lower and typ.begin_token.chars.is_capital_upper): 
            coef += 0.5
        if (br is not None and names is None): 
            ...
            if (after_org is not None or attach_typ == OrganizationAnalyzer.AttachType.HIGH): 
                ...
                j = 0
                while j < len(names): 
                    if (((names[j].is_newline_before and not typ.is_newline_before and not names[j].is_after_conjunction)) or ((not names[j].chars.equals(names[0].chars) and names[j].std_org_name_nouns == 0))): 
                        break
                    else: 
                        if (names[j].chars.equals(typ.chars) and not typ.chars.is_all_lower): 
                            coef += (0.5)
                        if (names[j].is_std_name): 
                            coef += (2)
                    ...
                    j += 1
            if (names[j - 1].end_char > t1.end_char): 
        ...
        if (typ.typ == "курс" or typ.typ == "группа" or typ.typ == "група"): 
            if (dep.number is None): 
                ...
                n = 0
                wrapn2999 = RefOutArgWrapper(0)
                inoutres3000 = Utils.tryParseInt(dep.number, wrapn2999)
                n = wrapn2999.value
                if (inoutres3000): 
                    if (n > 0 and (n < 9)): 
                        coef += (2)
        ...
        OrganizationAnalyzer.__correct_dep_attrs(res, typ, after_org_temp)
        if (after_org is not None and dep.higher is None and not after_org_temp): 
            if (OrgOwnershipHelper.can_be_higher(after_org, dep, False)): 
                dep.higher = after_org
        ...
        if (spec_word_before): 
            if (dep.find_slot(OrganizationReferent.ATTR_NAME, None, True) is not None): 
                coef += (2)
        if ((typ is not None and typ.root is not None and typ.root.can_be_normal_dep) and typ.name is not None and typ.name.find(' ') > 0): 
            coef += (4)
        if (((typ is not None and typ.root is not None and not typ.root.can_be_normal_dep) and dep.higher is None and not after_org_temp) and after_org is None and dep.number is None): 
    ...
    def __correct_dep_attrs(res : 'ReferentToken', typ : 'OrgItemTypeToken', after_temp_org : bool=False) -> None:
        ...
        if (br is not None and (t1.whitespaces_after_count < 2) and br.is_quote_type): 
            g = OrganizationAnalyzer.__is_geo(br.begin_token.next0_, False)
        ...
        for k in range(2):
            ...
            if (geo_ is not None): 
                ...
                rgeo = t1.kit.process_referent("GEO", t1.next0_, None)
                if (rgeo is not None): 
