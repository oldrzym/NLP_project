...
class PullentiNerOrgInternalResourceHelper:
    ...
    def get_bytes(name : str) -> bytearray:
        ...
        names = Utils.getResourcesNames('pullenti.ner.org.internal.properties', '.png;.dat')
        for n in names: 
            if (Utils.endsWithString(n, name, True)): 
                if (len(name) < len(n)): 
                    if (n[len(n) - len(name) - 1] != '.'): 
                        continue
                try: 
                    inf = Utils.getResourceInfo('pullenti.ner.org.internal.properties', n)
                    if (inf is None): 
                        continue
                    with Utils.getResourceStream('pullenti.ner.org.internal.properties', n) as stream: 
    ...
    def get_string(name : str) -> str:
        arr = PullentiNerOrgInternalResourceHelper.get_bytes(name)
        if (arr is None): 
            return None
