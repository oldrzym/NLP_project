...
class TextToken(Token):
    ...
    def __init__(self, source : 'MorphToken', kit_ : 'AnalysisKit', bchar : int=-1, echar : int=-1) -> None:
        ...
        while i < len(self.term): 
            ...
            if (j < self.morph.items_count): 
                break
            if ((i + 2) == len(self.term)): 
                if ((self.term.endswith("ИХ") or self.term.endswith("ЫХ") or self.term.endswith("ОВ")) or self.term.endswith("ОМ")): 
                    break
    ...
    def check_value(self, dict0_ : typing.List[tuple]) -> object:
        ...
        if (dict0_ is None): 
            return None
        res = None
        wrapres3508 = RefOutArgWrapper(None)
        inoutres3509 = Utils.tryGetValue(dict0_, self.term, wrapres3508)
        res = wrapres3508.value
        if (inoutres3509): 
            return res
            ...
            for it in self.morph.items: 
                ...
                if (mf is not None): 
                    if (mf.normal_case is not None): 
                        wrapres3504 = RefOutArgWrapper(None)
                        inoutres3505 = Utils.tryGetValue(dict0_, mf.normal_case, wrapres3504)
                        res = wrapres3504.value
                        if (inoutres3505): 
                            return res
                    if (mf.normal_full is not None and mf.normal_case != mf.normal_full): 
                        wrapres3506 = RefOutArgWrapper(None)
                        inoutres3507 = Utils.tryGetValue(dict0_, mf.normal_full, wrapres3506)
                        res = wrapres3506.value
                        if (inoutres3507): 
                            return res
        return None
    ...
    def is_value(self, term_ : str, termua : str=None) -> bool:
        ...
        if (self.max_length_of_morph_vars >= len(self.term) and (self.max_length_of_morph_vars < len(term_))): 
            return False
        if (term_ == self.term or term_ == self.term0): 
            return True
    ...
    def get_normal_case_text(self, mc : 'MorphClass'=None, num : 'MorphNumber'=MorphNumber.UNDEFINED, gender : 'MorphGender'=MorphGender.UNDEFINED, keep_chars : bool=False) -> str:
        ...
        for it in self.morph.items: 
            ...
            if (wf is not None): 
                ...
                if (num == MorphNumber.SINGULAR and mc is not None and mc.equals(MorphClass.NOUN)): 
                    if (res == "ДЕТИ"): 
                        res = "РЕБЕНОК"
                if (keep_chars): 
        ...
        if (num == MorphNumber.SINGULAR and mc is not None): 
            bi = MorphBaseInfo._new619(MorphClass._new53(mc.value), gender, MorphNumber.SINGULAR, self.morph.language)
            vars0_ = None
            try: 
                vars0_ = MorphologyService.get_wordform(self.term, bi)
            except Exception as ex3512: 
                pass
            ...
            if ((isinstance(it, MorphWordForm)) and it.is_in_dictionary): 
                if (it.class0_.is_verb and it.case_.is_undefined): 
                    ...
                    if (it.class0_.is_adjective and it.contains_attr("к.ф.", None)): 
                        ...
                        return False
        if (short_form): 
            if (self.is_value("ОБЯЗАТЕЛЬНЫЙ", None) or self.is_value("ЖЕЛАТЕЛЬНЫЙ", None)): 
                return True
        return ret
    ...
    @staticmethod
    def _new597(_arg1 : 'MorphToken', _arg2 : 'AnalysisKit', _arg3 : int, _arg4 : int, _arg5 : str) -> 'TextToken':
        res = TextToken(_arg1, _arg2, _arg3, _arg4)
        res.term0 = _arg5
        return res
    
    @staticmethod
    def _new600(_arg1 : 'MorphToken', _arg2 : 'AnalysisKit', _arg3 : int, _arg4 : int, _arg5 : 'CharsInfo', _arg6 : str) -> 'TextToken':
        res = TextToken(_arg1, _arg2, _arg3, _arg4)
        res.chars = _arg5
        res.term0 = _arg6
        return res