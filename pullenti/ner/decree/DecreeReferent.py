...
class DecreeReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang'=None, lev : int=0) -> str:
        ...
        if (self.typ is not None): 
            if ((nam is not None and not nam.startswith("О") and self.typ in nam) and ki != DecreeKind.STANDARD): 
                ...
                nam = (None)
            elif (ki == DecreeKind.STANDARD and (((len(self.typ) < 6) or "ОИ" in self.typ))): 
    ...
    def add_date(self, dt : 'DecreeToken') -> bool:
        if (dt is None): 
            return False
        if (dt.ref is not None and (isinstance(dt.ref.referent, DateReferent))): 
            ...
            s = self.add_slot(DecreeReferent.ATTR_DATE, Utils.toStringStringIO(tmp), False, 0)
            s.add_annotation(dt)
            return True
        if (dt.ref is not None and (isinstance(dt.ref.referent, DateRangeReferent))): 
            s = self.add_slot(DecreeReferent.ATTR_DATE, dt.ref.referent, False, 0)
            s.add_annotation(dt)
            return True
        if (dt.value is not None): 
            s = self.add_slot(DecreeReferent.ATTR_DATE, dt.value, False, 0)
            s.add_annotation(dt)
            return True
        return False
    ...
    def __all_years(self) -> typing.List[int]:
        ...
        for s in self.slots: 
            if (s.type_name == DecreeReferent.ATTR_DATE): 
                ...
                if (i == 4): 
                    str0_ = str0_[0:0+4]
                wrapi1345 = RefOutArgWrapper(0)
                inoutres1346 = Utils.tryParseInt(str0_, wrapi1345)
                i = wrapi1345.value
                if (inoutres1346): 
                    res.append(i)
        return res
    ...
    def add_typ(self, dt : 'DecreeToken') -> None:
        s = self.add_slot(DecreeReferent.ATTR_TYPE, dt.value, True, 0)
        s.add_annotation(dt)
    ...
    def add_number(self, dt : 'DecreeToken') -> None:
        ...
        if (dt.typ == DecreeToken.ItemType.NUMBER): 
            if (dt.num_year > 0): 
                s0 = self.add_slot(DecreeReferent.ATTR_DATE, str(dt.num_year), False, 0)
                s0.add_annotation(dt)
        if (Utils.isNullOrEmpty(dt.value)): 
            return
        value = dt.value
        if (".,".find(value[len(value) - 1]) >= 0): 
            value = value[0:0+len(value) - 1]
        s = self.add_slot(DecreeReferent.ATTR_NUMBER, value, False, 0)
        s.add_annotation(dt)
    ...
    def _add_name(self, dr : 'DecreeReferent') -> None:
        ...
        if (ss is not None): 
            if (ss.tag is None): 
                ss.tag = s.tag
            ss.merge_occurence(s)
    
    def _add_name_str(self, name_ : str, mt : 'MetaToken') -> None:
        ...
        if (Utils.isNullOrEmpty(name_)): 
            return
        if (str.islower(name_[0])): 
            name_ = "{0}{1}".format(str.upper(name_[0]), name_[1:])
        uname = name_.upper()
    ...
    def __can_be_equals(self, obj : 'Referent', typ_ : 'ReferentsEqualType', ignore_geo : bool) -> bool:
        ...
        if (dr.typ0 is not None and self.typ0 is not None): 
            if (dr.typ0 != self.typ0): 
                if (dr.typ0 == "ЗАКОН" and self.typ0 == "КОДЕКС"): 
                    pass
                elif (self.typ0 == "ЗАКОН" and dr.typ0 == "КОДЕКС"): 
                    pass
                else: 
                    return False
        num_eq = 0
        ...
        if (self.case_number is not None and dr.case_number is not None): 
            if (self.case_number != dr.case_number): 
                return False
        if (self.find_slot(DecreeReferent.ATTR_GEO, None, True) is not None or dr.find_slot(DecreeReferent.ATTR_GEO, None, True) is not None): 
            geo1 = self.get_string_value(DecreeReferent.ATTR_GEO)
            geo2 = dr.get_string_value(DecreeReferent.ATTR_GEO)
            if (geo1 is not None and geo1 is not None): 
                if (geo1 != geo2): 
                    return False
            elif (geo1 is not None and dr.find_slot(DecreeReferent.ATTR_SOURCE, None, True) is not None): 
                return False
            elif (geo2 is not None and self.find_slot(DecreeReferent.ATTR_SOURCE, None, True) is not None): 
                return False
        src_eq = False
        src_not_eq = False
        ...
        if (ty is None): 
            return num_eq > 0
        t = DecreeToken.get_kind(ty, None)
    ...
    def _check_correction(self, noun_is_doubtful : bool) -> bool:
        ...
        if ((typ_ == "КОДЕКС" or typ_ == "ОСНОВЫ ЗАКОНОДАТЕЛЬСТВА" or typ_ == "ПРОГРАММА") or typ_ == "ОСНОВИ ЗАКОНОДАВСТВА" or typ_ == "ПРОГРАМА"): 
            if (self.find_slot(DecreeReferent.ATTR_NAME, None, True) is None): 
                if (noun_is_doubtful): 
                    return False
                if (self.date is not None and self.number is not None): 
                    return True
                return False
        ...
        if (ty == DecreeKind.USTAV): 
            if (self.find_slot(DecreeReferent.ATTR_SOURCE, None, True) is not None): 
                return True
            if (self.find_slot(DecreeReferent.ATTR_GEO, None, True) is not None and self.name is not None): 
                return True
        if (ty == DecreeKind.KONVENTION or ty == DecreeKind.CONTRACT): 
            if (self.find_slot(DecreeReferent.ATTR_NAME, None, True) is not None): 
                if ((typ_ != "ДОГОВОР" and typ_ != "ДОГОВІР" and typ_ != "СОГЛАШЕНИЕ") and typ_ != "УГОДА" and typ_ != "ПРОТОКОЛ"): 
                    return True
                if (self.date is not None): 
                    return True
                str0_ = str(self).upper()
                if (("ЕВРО" in str0_ or "МИРО" in str0_ or "ВСЕМИР" in str0_) or "ОБЩЕМИР" in str0_): 
                    return True
        ...
        if (self.number is None): 
            if (self.find_slot(DecreeReferent.ATTR_NAME, None, True) is None or self.find_slot(DecreeReferent.ATTR_SOURCE, None, True) is None or self.find_slot(DecreeReferent.ATTR_DATE, None, True) is None): 
                if (ty == DecreeKind.CONTRACT and self.find_slot(DecreeReferent.ATTR_SOURCE, None, True) is not None and self.find_slot(DecreeReferent.ATTR_DATE, None, True) is not None): 
                    ...
                    pass
                elif (self.kind == DecreeKind.PROJECT and self.find_slot(DecreeReferent.ATTR_NAME, None, True) is not None): 
                    pass
                else: 
                    return False
        else: 
            if (typ_ == "ПАСПОРТ" or typ_ == "ПБУ" or typ_ == "ФОРМА"): 
                return True
            if (self.kind == DecreeKind.STANDARD): 
                return True
