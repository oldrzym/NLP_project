...
class DecreeAnalyzer(Analyzer):
    ...
    class ThisDecree(MetaToken):
        ...
        def try_attach_back(t : 'Token', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (ukaz is not None): 
                if (base_typ is not None and base_typ.value is not None and (("ДОГОВОР" in base_typ.value or "ДОГОВІР" in base_typ.value))): 
                    return DecreeAnalyzer.ThisDecree._new1341(ukaz, ukaz, True, base_typ.value)
            return None
        ...
        def try_attach(dtok : 'PartToken', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (not (isinstance(tt, TextToken))): 
                if ((isinstance(tt, ReferentToken)) and (isinstance(tt.get_referent(), DecreeReferent))): 
                    return DecreeAnalyzer.ThisDecree._new1342(t, tt, Utils.asObjectOrNull(tt.get_referent(), DecreeReferent))
                return None
            ...
            if (not tt.is_newline_after and not res.has_this_ref): 
                ...
                if (dt is not None and dt.typ != DecreeToken.ItemType.MISC): 
                    return None
                if (DecreeToken.try_attach_name(tt.next0_, res.typ, False, False, False) is not None): 
                    return None
            if (base_typ is not None and base_typ.value == res.typ): 
                res.has_this_ref = True
            return res
        ...
        @staticmethod
        def _new1341(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : str) -> 'ThisDecree':
        ...
        @staticmethod
        def _new1342(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeReferent') -> 'ThisDecree':
            res = DecreeAnalyzer.ThisDecree(_arg1, _arg2)
            res.real = _arg3
            return res
    ...
    def __try_attach(dts : typing.List['DecreeToken'], base_typ : 'DecreeToken', after_decree : bool, ad : 'AnalyzerData') -> typing.List['ReferentToken']:
        ...
        if (len(dts) == 2 and dts[0].typ == DecreeToken.ItemType.ORG and dts[1].typ == DecreeToken.ItemType.TYP): 
            return None
        if ((len(dts) == 2 and dts[0].typ == DecreeToken.ItemType.TYP and (dts[0].length_char < 3)) and dts[1].typ == DecreeToken.ItemType.NUMBER): 
            if (not dts[0].is_whitespace_after and dts[0].is_doubtful): 
                return None
        if (len(dts) == 1): 
        ...
        if (dec.find_slot(DecreeReferent.ATTR_NAME, None, True) is None and not rt.is_newline_after): 
            if (rt.is_newline_before or ((rt.begin_token.previous is not None and rt.begin_token.previous.length_char == 1 and ((rt.begin_token.previous.is_newline_before or rt.begin_token.previous.is_table_control_char))))): 
                if (dec.kind == DecreeKind.STANDARD and dec.number is not None): 
                    nam = DecreeToken.try_attach_name_to_end_of_line(rt.end_token.next0_)
                    if (nam is not None): 
        ...
        if (((i + 1) < len(dts)) and dts[i].typ == DecreeToken.ItemType.EDITION and not dts[i].is_newline_before): 
            ...
            if (ed is not None and len(ed) > 0): 
                ...
                for e0_ in ed: 
                    dec.add_slot(DecreeReferent.ATTR_EDITION, e0_.referent, False, 0)
                rt.end_token = ed[len(ed) - 1].end_token
        rt22 = DecreeAnalyzer._try_attach_approved0(rt.end_token.next0_, ad, True, False)
        if (rt22 is not None): 
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (tok is not None and tok.begin_token == tok.end_token and tok.chars.is_all_lower): 
                ...
                if (not ok): 
                    tok = (None)
            if (((dts is not None and len(dts) > 1 and tok is not None) and dts[0].end_token == tok.end_token and dts[0].typ == DecreeToken.ItemType.TYP) and dts[1].typ != DecreeToken.ItemType.TERR): 
                tok = (None)
            if (tok is not None): 
            ...
            if (tok is not None): 
                ...
                last_dec_dist = 0
                continue
            if (dts is None or len(dts) == 0 or ((len(dts) <= 2 and dts[0].typ == DecreeToken.ItemType.TYP))): 
                rt0 = DecreeAnalyzer._try_attach_approved(t, ad)
                if (rt0 is not None): 
            ...
            if (dts[0].typ == DecreeToken.ItemType.TYP and DecreeToken.get_kind(dts[0].value, None) == DecreeKind.PUBLISHER): 
                ...
                if (rts is not None): 
                    ...
                    if (tt1 is not None and tt1.is_char(')')): 
                        tt1 = tt1.next0_
                    mt = DecreeAnalyzer._check_alias_after(tt1)
                    if (mt is not None): 
                        tt = dts[0].begin_token.previous
                        first_pass3787 = True
                        while True:
        ...
        while True:
            ...
            if (dcht is not None and dcht.is_start and not dcht.ignorable): 
            ...
            if (rts is not None): 
                for kp in rts: 
                    ...
                    while i < len(dpr_list): 
                        ...
                        if (dcht is not None and not dcht.ignorable and len(change_stack) > 0): 
            ...
            if (dts is not None and len(dts) > 0 and dts[len(dts) - 1].end_char > t.end_char): 
                t = dts[len(dts) - 1].end_token
            if (dcht is not None and has_start_change > 0 and not dcht.ignorable): 
                if (dcht.end_char > t.end_char): 
                    t = dcht.end_token
                chrt = None
                if (dcht.typ == DecreeChangeTokenTyp.STARTMULTU): 
                    ...
                    if (len(change_stack) > 0 or dcht.decree is not None): 
                        root_change = (Utils.asObjectOrNull(ad.register_referent(DecreeChangeReferent._new1330(DecreeChangeKind.CONTAINER)), DecreeChangeReferent))
                ...
                if (dprs is not None and len(dprs) > 0): 
                    ...
                    if (chrt is None and expire_regime): 
                        chrt = list()
                        dcr = DecreeChangeReferent._new1330(DecreeChangeKind.EXPIRE)
    ...
    def _try_attach_approved(t : 'Token', ad : 'AnalyzerData') -> 'ReferentToken':
        ...
        if (br is not None and br.length_char > 20): 
            rt0 = DecreeAnalyzer._try_attach_approved0(br.end_token.next0_, ad, False, True)
            if (rt0 is not None): 
        ...
        while True:
            ...
            if (DecreeToken.is_keyword(tt, False) is not None and tt.chars.is_capital_upper): 
                break
            rt0 = DecreeAnalyzer._try_attach_approved0(tt, ad, True, False)
            if (rt0 is not None): 
    ...
    def _try_attach_approved0(t : 'Token', ad : 'AnalyzerData', must_be_comma : bool=True, name_in_brackets : bool=False) -> 'ReferentToken':
        ...
        olev = None
        lev = 0
        wrapolev1334 = RefOutArgWrapper(None)
        inoutres1335 = Utils.tryGetValue(kit.misc_data, "dovr", wrapolev1334)
        olev = wrapolev1334.value
        if (not inoutres1335): 
            lev = 1
    ...
    def __try_attach_pulishers(self, dts : typing.List['DecreeToken']) -> typing.List['ReferentToken']:
        ...
        while True:
            ...
            if (pt is not None): 
                ...
                if (pub_part is None): 
                    pub_part = DecreePartReferent._new1336(pub)
            ...
            if (isinstance(t, NumberToken)): 
                ...
                if (rt is not None): 
                    date_ = DecreeToken._new1047(rt.begin_token, rt.end_token, DecreeToken.ItemType.DATE)
                    date_.ref = rt
                    pub0 = (None)
                    pub_part0 = (None)
                    if (t0 is None): 
    ...
    def _try_attach_parts(parts : typing.List['PartToken'], base_typ : 'DecreeToken', _def_owner : 'Referent') -> typing.List['MetaToken']:
        ...
        if (parts[len(parts) - 1].typ != PartToken.ItemType.SUBPROGRAM and parts[len(parts) - 1].typ != PartToken.ItemType.ADDAGREE): 
            ...
            if (this_dec is not None): 
                if ((isinstance(_def_owner, DecreeReferent)) and ((_def_owner.typ0 == this_dec.typ or ((_def_owner.kind == DecreeKind.KODEX and this_dec.typ == "КОДЕКС")) or parts[0].typ == PartToken.ItemType.APPENDIX))): 
                    pass
                else: 
                    _def_owner = (None)
        ...
        while i < len(res): 
            ...
            while et.begin_char > res[i].begin_char:
                if (et.is_char(',') or et.morph.class0_.is_conjunction or et.is_hiphen): 
                    ...
                    break
            res1.append(MetaToken._new630(res[i].begin_token, et, li))
            i = (j - 1)
            i += 1
        return res1