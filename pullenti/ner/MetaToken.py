...
class MetaToken(Token):
    ...
    def __init__(self, begin : 'Token', end : 'Token', kit_ : 'AnalysisKit'=None) -> None:
        ...
        if (begin is None or end is None): 
            return
        self.chars = CharsInfo._new3181(begin.chars.value)
        if (begin != end): 
            t = begin.next0_
            while t is not None and t.end_char <= end.end_char: 
                if (t.chars.is_letter): 
                    if (self.chars.is_capital_upper and t.chars.is_all_lower): 
                        pass
                    else: 
                        self.chars.value = (((self.chars.value) & (t.chars.value)))
                t = t.next0_
    
    def _refresh_chars_info(self) -> None:
        if (self._m_begin_token is None): 
            return
        self.chars = CharsInfo()
        self.chars.value = self._m_begin_token.chars.value
        cou = 0
    ...
    def get_referents(self) -> typing.List['Referent']:
        ...
        first_pass4139 = True
        while True:
            if first_pass4139: first_pass4139 = False
            else: t = t.next0_
    ...
    @staticmethod
    def _new630(_arg1 : 'Token', _arg2 : 'Token', _arg3 : object) -> 'MetaToken':
        res = MetaToken(_arg1, _arg2)
        res.tag = _arg3
        return res
    
    @staticmethod
    def _new647(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection') -> 'MetaToken':
        res = MetaToken(_arg1, _arg2)
        res.morph = _arg3
        return res
    
    @staticmethod
    def _new2951(_arg1 : 'Token', _arg2 : 'Token', _arg3 : object, _arg4 : 'MorphCollection') -> 'MetaToken':
        res = MetaToken(_arg1, _arg2)
        res.tag = _arg3
        res.morph = _arg4
        return res