...
class NumberHelper:
    ...
    def __try_parse(token : 'Token', prev_val : 'NumberToken'=None) -> 'NumberToken':
        ...
        if (val is not None): 
            ...
            if (rr is None or et.whitespaces_after_count > 2): 
                et = tt
            else: 
                mc = rr.morph
            ...
            while (isinstance(et.next0_, TextToken)) and not (isinstance(et.previous, NumberToken)) and et.is_whitespace_before:
                ...
                if (sss == "000"): 
                    ...
                    et = (Utils.asObjectOrNull(et.next0_, TextToken))
                    continue
                elif (sss == "ООО" or sss == "OOO"): 
                    sub = NumberHelper.try_parse_postfix_only(et.next0_.next0_)
                    if (sub is not None): 
                        val = (val + "000")
                        et = (Utils.asObjectOrNull(et.next0_, TextToken))
                        continue
                if (str.isdigit(sss[0]) and len(sss) == 3): 
                    ...
                    while True:
                        ...
                        if (ttt.whitespaces_before_count == 1 and ttt.length_char == 3 and str.isdigit(ss[0])): 
                            ii = 0
                            wrapii637 = RefOutArgWrapper(0)
                            inoutres638 = Utils.tryParseInt(ss, wrapii637)
                            ii = wrapii637.value
                            if (not inoutres638): 
                                break
                            val2 += ss
                        ...
                        if ((ttt.is_char_of(".,") and not ttt.is_whitespace_before and not ttt.is_whitespace_after) and ttt.next0_ is not None and str.isdigit(ttt.next0_.get_source_text()[0])): 
                            if (ttt.next0_.is_whitespace_after and (isinstance(ttt.previous, TextToken))): 
                                ...
                                val = val2
                                break
                        if (((((((ttt.is_value("ТРИЛЛИОН", "ТРИЛЬЙОН") or ttt.is_value("TRILLION", None) or ttt.is_value("ТРЛН", None)) or ttt.is_value("МИЛЛИАРД", "МІЛЬЯРД") or ttt.is_value("BILLION", None)) or ttt.is_value("BN", None) or ttt.is_value("МЛРД", None)) or ttt.is_value("МИЛЛИОН", "МІЛЬЙОН") or ttt.is_value("MILLION", None)) or ttt.is_value("МЛН", None) or ttt.is_value("ТЫСЯЧА", "ТИСЯЧА")) or ttt.is_value("THOUSAND", None) or ttt.is_value("ТЫС", None)) or ttt.is_value("ТИС", None)): 
                            et = (Utils.asObjectOrNull(ttt.previous, TextToken))
                            val = val2
                            break
                        break
            ...
            for k in range(4):
                if ((isinstance(et.next0_, TextToken)) and et.next0_.chars.is_letter): 
                    ...
                    if (k == 0): 
                        coef = "000000000000"
                        if (tt.is_value("ТРИЛЛИОН", "ТРИЛЬЙОН") or tt.is_value("TRILLION", None)): 
                            et = tt
                            val += coef
                        elif (tt.is_value("ТРЛН", None)): 
                            et = tt
                            val += coef
                    ...
                    break
            res = NumberToken._new639(token, et, val, typ, mc)
            if (et.next0_ is not None and (len(res.value) < 4) and ((et.next0_.is_hiphen or et.next0_.is_value("ДО", None)))): 
                ...
                while True:
                    ...
                    if (tt1.is_char_of(",.") or NumberHelper._is_money_char(tt1) is not None): 
                        continue
                    if (tt1.is_value("ТРИЛЛИОН", "ТРИЛЬЙОН") or tt1.is_value("ТРЛН", None) or tt1.is_value("TRILLION", None)): 
                        res.value = res.value + "000000000000"
                    elif (tt1.is_value("МИЛЛИОН", "МІЛЬЙОН") or tt1.is_value("МЛН", None) or tt1.is_value("MILLION", None)): 
        ...
        while t is not None: 
            ...
            if (num is None): 
                break
            wrapj640 = RefOutArgWrapper(0)
            inoutres641 = Utils.tryParseInt(str(num.termin.tag), wrapj640)
            j = wrapj640.value
            if (not inoutres641): 
                break
            ...
            if (jprev > 0): 
                if ((jprev < 20) and (j < 20)): 
                    break
                if (jprev <= j): 
                    if (jprev == j): 
                        break
                    if ((j == 100 or j == 1000 or j == 1000000) or j == 1000000000): 
                        pass
                    else: 
                        break
                elif (len(str(jprev)) == len(str(j))): 
                    break
            if ((int_val + loc_value) > 0 and ((t.is_value("СТО", None) or t.is_value("ДЕСЯТЬ", None))) and t.next0_ is not None): 
                if (t.next0_.is_value("ТЫСЯЧНЫЙ", "ТИСЯЧНИЙ") or t.next0_.is_value("МИЛЛИОННЫЙ", "МІЛЬЙОННИЙ") or t.next0_.is_value("МИЛЛИАРДНЫЙ", "МІЛЬЯРДНИЙ")): 
                    break
            if (is_adj and t != tt): 
                if (((t.is_value("ДЕСЯТЫЙ", "ДЕСЯТИЙ") or t.is_value("СОТЫЙ", "СОТИЙ") or t.is_value("ТЫСЯЧНЫЙ", "ТИСЯЧНИЙ")) or t.is_value("ДЕСЯТИТЫСЯЧНЫЙ", "ДЕСЯТИТИСЯЧНИЙ") or t.is_value("МИЛЛИОННЫЙ", "МІЛЬЙОННИЙ")) or t.is_value("МИЛЛИАРДНЫЙ", "МІЛЬЯРДНИЙ")): 
                    break
                if ((j < 9) and (jprev < 20)): 
                    break
            if (j >= 1000): 
    ...
    def try_parse_roman(t : 'Token') -> 'NumberToken':
        ...
        while t is not None: 
            ...
            if (not (isinstance(t, TextToken))): 
                if ((isinstance(t, NumberToken)) and t.value == "1" and t.typ == NumberSpellingType.DIGIT): 
                    term = "1"
                else: 
                    break
            else: 
        ...
        res.int_value = val
        et = res.end_token
        if (et.whitespaces_after_count > 2): 
            return res
    ...
    def try_parse_age(t : 'Token') -> 'NumberToken':
        ...
        if (nt is not None): 
            if (nt_next is not None): 
                ...
                if (isinstance(t1, TextToken)): 
                    v = t1.term
                    if ((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v == "РІЧЧЯ" or v == "ЖЫЛЖЫГЫ"): 
                        return NumberToken._new639(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
                    if (t1.is_value("ЛЕТНИЙ", "РІЧНИЙ")): 
                        return NumberToken._new639(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
                    if (v == "Л" or ((v == "Р" and nt.morph.language.is_ua))): 
                        if (nt.length_char < 2): 
                            return None
                        res = NumberToken(t, (t1.next0_ if t1.next0_ is not None and t1.next0_.is_char('.') else t1), nt.value, NumberSpellingType.AGE)
                        if (res.end_token.next0_ is not None): 
                            if (res.end_token.next0_.is_char_of("\\/") or res.end_token.next0_.is_value("Д", None)): 
                                return None
                        return res
            return None
        tt = Utils.asObjectOrNull(t, TextToken)
        ...
        if (LanguageHelper.ends_with_ex(s, "ЛЕТИЕ", "ЛЕТИЯ", "РІЧЧЯ", None)): 
            ...
            if (term is not None): 
                return NumberToken._new639(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
        s = tt.lemma
        i = s.find("ЛЕТН")
        if (i < 0): 
            i = s.find("РІЧН")
        if (i > 3): 
            ss = s[0:0+i]
            term = NumberHelper._m_nums.find(ss)
            if (term is not None): 
                return NumberToken._new639(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
            for i in range(len(ss) - 2, 2, -1):
                ss1 = ss[0:0+i]
                term1 = NumberHelper._m_nums.find(ss1)
                if (term1 is None): 
                    continue
                ss2 = ss[i:]
                term2 = NumberHelper._m_nums.find(ss2)
                if (term2 is None): 
                    continue
                num0 = str(term1.tag)
                num1 = str(term2.tag)
                if (len(num0) > 1 and num0[len(num0) - 1] == '0' and len(num1) == 1): 
                    return NumberToken._new639(tt, tt, num0[0:0+len(num0) - 1] + num1, NumberSpellingType.AGE, tt.morph)
            else: i = 2
        return None
    ...
    def try_parse_anniversary(t : 'Token') -> 'NumberToken':
        ...
        if (isinstance(t1, TextToken)): 
            v = t1.term
            if ((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v.startswith("ГОДОВ")): 
                if (t1.next0_ is not None and t1.next0_.is_char('.')): 
                    t1 = t1.next0_
                return NumberToken(t, t1, nt.value, NumberSpellingType.AGE)
            if (v == "ГОД" and t1.next0_ is not None): 
                t1 = t1.next0_
                if (t1.next0_ is not None and t1.is_char('.')): 
                    t1 = t1.next0_
                if (t1.is_value("ОКТЯБРЬ", None) or t1.is_value("ПОБЕДА", None)): 
                    return NumberToken(t, t1.previous, nt.value, NumberSpellingType.AGE)
    ...
    def __analize_number_tail(tt : 'TextToken', val : str) -> 'MetaToken':
        ...
        if (not tt.is_whitespace_before): 
            ok0 = True
        elif (tt.previous is not None and tt.previous.is_hiphen): 
            ok0 = True
        s = tt.term
        mc = None
            ...
            if (((s == "<" or s == "(")) and (isinstance(tt.next0_, TextToken))): 
                ...
                if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
                    if (tt.next0_.next0_ is not None and tt.next0_.next0_.is_char_of(">)")): 
                        ...
                        return MetaToken._new647(tt, tt.next0_.next0_, mc)
            return None
        if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
            mc = MorphCollection()
            mc.class0_ = MorphClass.ADJECTIVE
            mc.language = MorphLang.EN
            return MetaToken._new647(tt, tt, mc)
        ...
        if (not tt.is_whitespace_after): 
            ...
            if (tt.length_char == 1 and ((tt.is_value("X", None) or tt.is_value("Х", None)))): 
                return None
        if (not tt.chars.is_all_lower or not ok0): 
            ss = tt.term
            if (ss == "Я" or ss == "Й" or ss == "Е"): 
                if (ss == "Е" and ((tt.previous is None or not tt.previous.is_hiphen))): 
                    return None
        ...
        if (len(val) == 2 and val[0] == '1'): 
            dig = 0
        samp = NumberHelper.__m_samples[dig]
        if (len(val) > 1 and val[len(val) - 2] == '1'): 
            samp = "ДЕВЯТЫЙ"
        vars0_ = None
        try: 
            vars0_ = MorphologyService.get_all_wordforms(samp, None)
        except Exception as ex649: 
            pass
        ...
        for v in vars0_: 
            if (v.class0_.is_adjective and LanguageHelper.ends_with(v.normal_case, s) and v.number != MorphNumber.UNDEFINED): 
                ...
                ok = False
                for it in mc.items: 
                    if (it.class0_.equals(v.class0_) and it.number == v.number and ((it.gender == v.gender or v.number == MorphNumber.PLURAL))): 
        ...
        if (tt.morph.language.is_ua and mc is None and s == "Ї"): 
            mc = MorphCollection()
            mc.add_item(MorphBaseInfo._new650(MorphClass.ADJECTIVE))
        if (mc is not None): 
            return MetaToken._new647(tt, tt, mc)
        if ((((len(s) < 3) and not tt.is_whitespace_before and tt.previous is not None) and tt.previous.is_hiphen and not tt.previous.is_whitespace_before) and tt.whitespaces_after_count == 1 and s != "А"): 
            if (len(s) == 1 and s != "Й" and not LanguageHelper.is_cyrillic_vowel(s[0])): 
                pass
            else: 
                return MetaToken._new647(tt, tt, MorphCollection._new652(MorphClass.ADJECTIVE))
        return None
    ...
    def __try_parse_float(t : 'NumberToken', d : float, attrs : 'NumberParseAttr') -> 'Token':
        ...
        while True:
            ...
            if (t1.next0_ is None): 
                break
            if (((isinstance(t1.next0_, NumberToken)) and (((t1.whitespaces_after_count < 6) or ((t.previous is not None and t.previous.is_table_control_char)))) and t1.next0_.typ == NumberSpellingType.DIGIT) and t1.next0_.length_char == 3): 
                if (ns is None): 
                    ns = list()
                    ns.append(t)
                    sps = list()
                ...
                sps.append(' ')
                continue
            if ((t1.next0_.is_char_of(",.") and (isinstance(t1.next0_.next0_, NumberToken)) and t1.next0_.next0_.typ == NumberSpellingType.DIGIT) and (t1.whitespaces_after_count < 6) and (t1.next0_.whitespaces_after_count < 2)): 
                if ((((attrs) & (NumberParseAttr.NOWHITESPACES))) != (NumberParseAttr.NO)): 
        ...
        if (len(sps) == 1): 
            if (sps[0] == ' '): 
                is_last_drob = False
            elif ((((attrs) & (NumberParseAttr.COMMAISFLOATPOINT))) != (NumberParseAttr.NO) and sps[0] == ','): 
                is_last_drob = True
    ...
    @staticmethod
    def try_parse_real_number(t : 'Token', attrs : 'NumberParseAttr'=NumberParseAttr.NO) -> 'NumberToken':
        ...
        Args:
            t(Token): начальный токен
            attrs(NumberParseAttr): дополнительные атрибуты выделения
        ...
        t0 = t
        if (t is not None): 
            if (t.is_hiphen or t.is_value("МИНУС", "МІНУС")): 
                t = t.next0_
                is_not = True
        ...
        if ((isinstance(t, TextToken)) and ((t.is_value("НОЛЬ", None) or t.is_value("НУЛЬ", None)))): 
            ...
            if (t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ") and t.next0_.next0_ is not None): 
                t = t.next0_
            if (t.next0_.is_comma_and and t.next0_.next0_ is not None): 
                t = t.next0_
        ...
        if (not (isinstance(t, NumberToken))): 
            ...
            if (t.is_value("ТЫЩА", None) or t.is_value("ТЫСЯЧА", "ТИСЯЧА")): 
                return NumberToken(t, t, "1000", NumberSpellingType.WORDS)
            return None
        if (t.next0_ is not None and t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ")): 
            tt1 = t.next0_.next0_
            if (tt1 is not None and tt1.is_comma_and): 
                tt1 = tt1.next0_
            if ((isinstance(tt1, NumberToken)) or (((isinstance(tt1, TextToken)) and tt1.is_value("НОЛЬ", "НУЛЬ")))): 
                ...
                if (res0.real_value == 0): 
                    ...
                    dd = 0
                    wrapdd656 = RefOutArgWrapper(0)
                    inoutres657 = Utils.tryParseFloat(str0_, wrapdd656)
                    dd = wrapdd656.value
                    if (inoutres657): 
                        pass
                    else: 
                        wrapdd654 = RefOutArgWrapper(0)
                        inoutres655 = Utils.tryParseFloat(str0_.replace('.', ','), wrapdd654)
                        dd = wrapdd654.value
                        if (inoutres655): 
                            pass
                        else: 
                ...
                return res0
        d = 0
        wrapd659 = RefOutArgWrapper(0)
        tt = NumberHelper.__try_parse_float(Utils.asObjectOrNull(t, NumberToken), wrapd659, attrs)
        d = wrapd659.value
        if (tt is None): 
            if ((t.next0_ is None or t.is_whitespace_after or t.next0_.chars.is_letter) or (((attrs) & (NumberParseAttr.CANNOTBEINTEGER))) == (NumberParseAttr.NO)): 
                tt = t
                d = t.real_value
            else: 
                return None
        ...
        if (tt.next0_ is not None and tt.next0_.is_value("ДЕСЯТОК", None)): 
            d *= (10)
            tt = tt.next0_
        return NumberExToken._new658(t0, tt, "", NumberSpellingType.DIGIT, NumberExType.UNDEFINED, d)
    ...
    @staticmethod
    def check_pure_number(t : 'Token') -> bool:
        if (isinstance(t, NumberToken)): 
            return NumberHelper.check_pure_number(t.end_token)
        if (not (isinstance(t, TextToken))): 
            return False
        for w in NumberHelper.__m_1words: 
            if (t.is_value(w, None)): 
                return True
        for w in NumberHelper.__m_10words: 
            if (t.is_value(w, None)): 
                return True
        for w in NumberHelper.__m_100words: 
            if (t.is_value(w, None)): 
                return True
        return False
    ...
    @staticmethod
    def get_number_base_info(num_val : str, cla : 'MorphClass') -> 'MorphBaseInfo':
        ...
        Args:
            num_val(str): число (например, 123)
            cla(MorphClass): часть речи (существительное или прилагательное)
        
        Returns:
            MorphBaseInfo: результат
        """
        ch = num_val[len(num_val) - 1]
        n = 0
        wrapn662 = RefOutArgWrapper(0)
        Utils.tryParseInt(num_val, wrapn662)
        n = wrapn662.value
        res = MorphBaseInfo._new650(cla)
        if (num_val == "1" or ((ch == '1' and n > 20 and ((n % 100)) != 11))): 
            res.number = MorphNumber.SINGULAR
            res.case_ = MorphCase.NOMINATIVE
        elif (((ch == '2' or ch == '3' or ch == '4')) and ((n < 10))): 
            if (cla.is_adjective): 
                res.number = MorphNumber.PLURAL
                res.case_ = MorphCase.NOMINATIVE
            else: 
                res.number = MorphNumber.SINGULAR
                res.case_ = MorphCase.GENITIVE
        else: 
            res.number = MorphNumber.PLURAL
            res.case_ = MorphCase.GENITIVE
        return res
    ...
    def string_to_double(str0_ : str) -> float:
        ...
        if (str0_ == "NaN"): 
            return math.nan
        wrapres667 = RefOutArgWrapper(0)
        inoutres668 = Utils.tryParseFloat(str0_, wrapres667)
        res = wrapres667.value
        if (inoutres668): 
            return res
        wrapres665 = RefOutArgWrapper(0)
        inoutres666 = Utils.tryParseFloat(str0_.replace('.', ','), wrapres665)
        res = wrapres665.value
        if (inoutres666): 
            return res
        wrapres663 = RefOutArgWrapper(0)
        inoutres664 = Utils.tryParseFloat(str0_.replace(',', '.'), wrapres663)
        res = wrapres663.value
        if (inoutres664): 
            return res
        return None
    ...
    def _initialize() -> None:
        ...
        NumberHelper._m_nums.add_string("ТРИЛЛИОН", "1000000000000", None, False)
        NumberHelper._m_nums.add_string("ТРИЛЬЙОН", "1000000000000", MorphLang.UA, False)
        NumberHelper.__m_after_points = TerminCollection()
        t = Termin._new92("ПОЛОВИНА", 0.5)
        t.add_variant("ОДНА ВТОРАЯ", False)
        t.add_variant("ПОЛ", False)
        NumberHelper.__m_after_points.add(t)
        t = Termin._new92("ТРЕТЬ", 0.33)
        t.add_variant("ОДНА ТРЕТЬ", False)
        NumberHelper.__m_after_points.add(t)
        t = Termin._new92("ЧЕТВЕРТЬ", 0.25)
        t.add_variant("ОДНА ЧЕТВЕРТАЯ", False)
        NumberHelper.__m_after_points.add(t)
        t = Termin._new92("ПЯТАЯ ЧАСТЬ", 0.2)
