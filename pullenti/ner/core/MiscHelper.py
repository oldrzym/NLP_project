...
class MiscHelper:
            ...
            while t1 is not None or t2 is not None:
                ...
                if ((((attrs) & (CanBeEqualsAttr.IGNOREUPPERCASE))) == (CanBeEqualsAttr.NO)): 
                    if (t1.previous is None and (((attrs) & (CanBeEqualsAttr.IGNOREUPPERCASEFIRSTWORD))) != (CanBeEqualsAttr.NO)): 
                        pass
                    elif (not t1.chars.equals(t2.chars)): 
                        return False
    ...
    def can_be_start_of_sentence(t : 'Token') -> bool:
        ...
        if (t.previous is None): 
            return True
        if (not t.is_table_control_char and t.previous.is_table_control_char): 
            return True
    ...
    def check_number_prefix(t : 'Token') -> 'Token':
        ...
        s = t.term
        t1 = None
        if (((t.is_value("ПО", None) or t.is_value("ЗА", None))) and t.next0_ is not None): 
            t = t.next0_
        if ((((t.is_value("РЕГИСТРАЦИОННЫЙ", "РЕЄСТРАЦІЙНИЙ") or t.is_value("ГОСУДАРСТВЕННЫЙ", "ДЕРЖАВНИЙ") or t.is_value("ТРАНЗИТНЫЙ", "ТРАНЗИТНИЙ")) or t.is_value("ДЕЛО", None) or t.is_value("СПРАВА", None))) and (isinstance(t.next0_, TextToken))): 
            t = t.next0_
            s = t.term
        elif ((s == "РЕГ" or s == "ГОС" or s == "ТРАНЗ") or s == "ИНВ"): 
            if (t.next0_ is not None and t.next0_.is_char('.')): 
                t = t.next0_
            if (isinstance(t.next0_, TextToken)): 
                ...
                return None
        if (((t.is_value("НОМЕР", None) or s == "№" or s == "N") or s == "NO" or s == "NN") or s == "НР"): 
            t1 = t.next0_
            if (t1 is not None and ((t1.is_char_of("°№") or ((t1.is_value("О", None) and not t1.is_newline_before))))): 
                t1 = t1.next0_
            if (((t1 is not None and t1.is_value("П", None) and t1.next0_ is not None) and ((t1.next0_.is_hiphen or t1.next0_.is_char_of("\\/"))) and t1.next0_.next0_ is not None) and t1.next0_.next0_.is_value("П", None)): 
                t1 = t1.next0_.next0_.next0_
            if (t1 is not None and t1.is_char('.')): 
                t1 = t1.next0_
            if (t1 is not None and t1.is_char(':')): 
                t1 = t1.next0_
            if (s == "N" and t1 is not None and t1.is_hiphen): 
                t1 = t1.next0_
        elif (s == "НОМ"): 
            t1 = t.next0_
            if (t1 is not None and t1.is_char('.')): 
    ...
    def __convert_word(str0_ : str, latin_to_rus : bool) -> typing.List[str]:
        ...
        while True:
            ...
            while i < len(vars0_): 
                if (len(vars0_[i]) > 0): 
                    print(vars0_[i][inds[i]], end="", file=str_tmp)
                i += 1
            res.append(Utils.toStringStringIO(str_tmp))
    ...
    def get_text_value(begin : 'Token', end : 'Token', attrs : 'GetTextAttr'=GetTextAttr.NO) -> str:
        ...
        """
        str0_ = MiscHelper.__get_text_value_(begin, end, attrs, None)
        return str0_
    ...
    def __get_text_value_(begin : 'Token', end : 'Token', attrs : 'GetTextAttr', r : 'Referent') -> str:
        ...
        if ((((attrs) & (GetTextAttr.RESTOREREGISTER))) != (GetTextAttr.NO)): 
            attrs = (Utils.valToEnum((attrs) | (GetTextAttr.KEEPREGISTER), GetTextAttr))
        res = io.StringIO()
                            ...
                            if (it.gender == npt.morph.gender or ((it.gender) & (npt.morph.gender)) != (MorphGender.UNDEFINED)): 
                                if (not ((it.case_) & npt.morph.case_).is_undefined): 
                                    if (it.number == npt.morph.number or ((it.number) & (npt.morph.number)) != (MorphNumber.UNDEFINED)): 
                                        ...
                                        if (isinstance(it, MorphWordForm)): 
                                            var = it.normal_case
                                        bi = MorphBaseInfo._new619(MorphClass.ADJECTIVE, npt.morph.gender, npt.morph.number, npt.morph.language)
                                        try: 
                                            var = MorphologyService.get_wordform(var, bi)
                                        except Exception as ex620: 
                                            pass
                                        if (var is not None): 
    ...
    def get_text_morph_var_by_sample(txt : str, begin_sample : 'Token', use_morph_sample : bool, use_register_sample : bool) -> str:
        ...
        Args:
            ...
            use_morph_sample(bool): использовать именную группу примера для морфологии
    ...
    def get_text_morph_var_by_case(txt : str, cas : 'MorphCase', plural_number : bool=False) -> str:
        ...
        return MiscHelper.get_text_morph_variant(txt, cas, (MorphNumber.PLURAL if plural_number else MorphNumber.UNDEFINED))
    
    @staticmethod
    def get_text_morph_variant(txt : str, cas : 'MorphCase', num : 'MorphNumber'=MorphNumber.UNDEFINED) -> str:
        ...
        Args:
            ...
            cas(MorphCase): падеж
            num(MorphNumber): число (по умолчанию, MorphNumber.Undefined - оставить как есть)
        ...
        res = io.StringIO()
        t0 = ar.first_token
        if ((isinstance(t0, TextToken)) and not t0.chars.is_letter): 
            print(t0.get_source_text(), end="", file=res)
            t0 = t0.next0_
        npt = NounPhraseHelper.try_parse(t0, NounPhraseParseAttr.PARSEVERBS, 0, None)
        if (npt is not None): 
            acc_corr = False
            if (num == MorphNumber.UNDEFINED): 
                if (npt.morph.number == MorphNumber.PLURAL or npt.morph.number == MorphNumber.SINGULAR): 
                    num = npt.morph.number
            if (cas.is_accusative and num == MorphNumber.PLURAL): 
                acc_corr = True
                ...
                if (isinstance(t, NumberToken)): 
                    ...
                    for it in t.morph.items: 
                        ...
                        if (not npt.morph.case_.is_undefined): 
                            if (((npt.morph.case_) & wf.case_).is_undefined): 
                                continue
                        if (num == MorphNumber.PLURAL and wf.number == MorphNumber.SINGULAR and t != npt.anafor): 
                            continue
                        if (num == MorphNumber.SINGULAR and wf.number == MorphNumber.PLURAL and t != npt.anafor): 
                            continue
                        if (is_noun): 
                    ...
                    if (not t.chars.is_letter): 
                        ...
                        if ((t.next0_ is not None and t.next0_.is_hiphen and t.is_value("ГЕНЕРАЛ", None)) or t.is_value("КАПИТАН", None)): 
                            ...
                            mbi = MorphBaseInfo._new622(npt.morph.gender, cas, MorphNumber.SINGULAR)
                            if (num != MorphNumber.UNDEFINED): 
                                mbi.number = num
                            wcas = None
                            try: 
                                wcas = MorphologyService.get_wordform(word, mbi)
                            except Exception as ex623: 
                                pass
                            if (wcas is not None): 
                                word = wcas
    ...
    def get_text_morph_var_by_case_and_number_ex(str0_ : str, cas : 'MorphCase'=None, num : 'MorphNumber'=MorphNumber.SINGULAR, num_val : str=None) -> str:
        ...
        ar = None
        try: 
            ar = ProcessorService.get_empty_processor().process(SourceOfAnalysis._new624(str0_, False), None, None)
        except Exception as ex: 
            pass
        if (ar is None or ar.first_token is None): 
            return str0_
        ...
        adj_bi = MorphBaseInfo._new625((MorphClass.ADJECTIVE) | MorphClass.NOUN, cas, num, npt.morph.gender)
        noun_bi = MorphBaseInfo._new626(npt.noun.morph.class0_, cas, num)
        ...
        if (not Utils.isNullOrEmpty(num_val) and num == MorphNumber.UNDEFINED): 
            ...
            n = 0
            wrapn627 = RefOutArgWrapper(0)
            Utils.tryParseInt(num_val, wrapn627)
            n = wrapn627.value
    ...
    @staticmethod
    def get_pure_referent_token(rt : 'ReferentToken') -> 'MetaToken':
        ...
        Args:
            rt(ReferentToken): метатокен, содержаший сущность
        
        Returns:
            MetaToken: более узкий метатокен или этот же, если не удалось уменьшить
        """
        if (rt is None): 
            return None
        t0 = None
        ...
        while True:
            ...
            if (not (tt is not None and tt.end_char <= rt.end_char)): break
            if (tt.is_char('(')): 
                break
            if (isinstance(tt, ReferentToken)): 
                ...
                if (r is None): 
                    continue
                if (r.type_name == "URI" or r.type_name == "ADDRESS" or r.type_name == "PHONE"): 
                    break
                if (r.type_name == rt.referent.type_name and r.type_name == "ORGANIZATION" and r != rt.referent): 
                    if (t0 is None): 
                        t0 = tt
                    t1 = tt
                continue
            if (tt.is_comma or tt.is_char_of(":;")): 
                continue
            if (t0 is None): 
                t0 = tt
            t1 = tt
        if (t0 is None): 
            t0 = rt.begin_token
            t1 = rt.end_token
        return MetaToken._new630(t0, t1, MiscHelper.get_text_value(t0, t1, Utils.valToEnum((GetTextAttr.KEEPQUOTES) | (GetTextAttr.KEEPREGISTER), GetTextAttr)))