...
class AnalysisKit:
        ...
        if (tokens is not None): 
            ...
            while ii < len(tokens): 
                ...
                if (sofa_.correction_dict is not None): 
                    corw = None
                    wrapcorw598 = RefOutArgWrapper(None)
                    inoutres599 = Utils.tryGetValue(sofa_.correction_dict, mt.term, wrapcorw598)
                    corw = wrapcorw598.value
                    if (inoutres599): 
                        ccc = MorphologyService.process(corw, lang, None)
                        if (ccc is not None and len(ccc) == 1): 
                            tt1 = TextToken._new597(ccc[0], self, tt.begin_char, tt.end_char, tt.term)
                            tt1.chars = tt.chars
                            tt = tt1
                ...
                if (t0 is None): 
                    ...
                    t0.next0_ = tt
                t0 = (tt)
                www = t0.whitespaces_before_count
                ii += 1
        if (sofa_.clear_dust): 
            self.__clear_dust()
    ...
    def correct_word_by_morph(self, tt : 'Token', lang : 'MorphLang') -> 'Token':
        ...
        if (ccc is None or len(ccc) != 1): 
            return None
        tt1 = TextToken._new600(ccc[0], self, tt.begin_char, tt.end_char, tt.chars, tt.term)
    ...
    def __correct_words_by_morph(self, lang : 'MorphLang') -> None:
        ...
        while tt is not None: 
            t = self.correct_word_by_morph(tt, lang)
            if (t is not None): 
                tt = t
            tt = tt.next0_
    ...
    def embed_token(self, mt : 'MetaToken') -> None:
        ...
        if (mt.begin_char > mt.end_char): 
            return
        if (mt.begin_token.begin_char == self.first_token.begin_char): 
            self.first_token = (mt)
        else: 
        ...
        if (isinstance(mt, ReferentToken)): 
            if (mt.referent is not None): 
                mt.referent.add_occurence(TextAnnotation._new601(self.sofa, mt.begin_char, mt.end_char))
    ...
    def get_analyzer_data(self, analyzer : 'Analyzer') -> 'AnalyzerData':
        ...
        if (analyzer is None or analyzer.name is None): 
            return None
        d = None
        wrapd602 = RefOutArgWrapper(None)
        inoutres603 = Utils.tryGetValue(self.__m_datas, analyzer.name, wrapd602)
        d = wrapd602.value
        if (inoutres603): 
            d.kit = self
            return d
    ...
    def remove_analyzer_data_by_analyzer_name(self, analyzer_name : str) -> None:
        if (Utils.ifNotNull(analyzer_name, "") in self.__m_datas): 
            del self.__m_datas[Utils.ifNotNull(analyzer_name, "")]
    ...
    def process_referent(self, analyzer_name : str, t : 'Token', param : str=None) -> 'ReferentToken':
        ...
        res = a.process_referent(t, param)
        self._m_analyzer_stack.remove(analyzer_name)
        if (res is not None): 
            pass
        return res
    
    def fix_analyzer(self, analyzer_name : str, ban : bool) -> None:
        if (ban): 
            self._m_analyzer_stack.append(analyzer_name)
        elif (analyzer_name in self._m_analyzer_stack): 
            self._m_analyzer_stack.remove(analyzer_name)
    ...
    def create_referent(self, type_name : str) -> 'Referent':
        ...
        if (self.processor is not None): 
            for a in self.processor.analyzers: 
                ...
                if (res is not None): 
                    return res
        return ProcessorService.create_referent(type_name)
    
    def refresh_generals(self) -> None:
        GeneralRelationHelper.refresh_generals(self.processor, self)
    
    def serialize(self, stream : Stream, new_format : bool=False) -> None:
        stream.writebyte(0xAA)
        stream.writebyte(((2 if new_format else 1)))
        ...
        for e0_ in self.__m_entities: 
            e0_.serialize(stream, new_format)
    ...
    def deserialize(self, stream : Stream) -> bool:
        ...
        i = 0
        while i < cou: 
            self.__m_entities[i].deserialize(stream, self.__m_entities, self.__m_sofa, (b) > 1)
            i += 1
    ...
    @staticmethod
    def _new3485(_arg1 : 'Processor', _arg2 : 'ExtOntology') -> 'AnalysisKit':
        res = AnalysisKit()
        res.processor = _arg1
        res.ontology = _arg2
        return res
    ...
    @staticmethod
    def _new3486(_arg1 : 'SourceOfAnalysis', _arg2 : bool, _arg3 : 'MorphLang', _arg4 : EventHandler, _arg5 : 'ExtOntology', _arg6 : 'Processor', _arg7 : bool) -> 'AnalysisKit':
