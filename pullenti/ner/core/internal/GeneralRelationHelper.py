...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new521(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new521(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi524 = RefOutArgWrapper(None)
                inoutres525 = Utils.tryGetValue(all0_, a.name, wrapsi524)
                si = wrapsi524.value
                if (not inoutres525): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli522 = RefOutArgWrapper(None)
                    inoutres523 = Utils.tryGetValue(si, s, wrapli522)
                    li = wrapli522.value
                    if (not inoutres523): 
                        li = list()
                        si[s] = li
        ...
        for n in all_refs: 
            if (not n.deleted and n.gen_to is not None and len(n.gen_to) == 1): 
                ...
                if (len(p.gen_from) == 1): 
                    ...
                    p.ref.tag = (n.ref)
                    if (n.ref.general_referent == p.ref): 
                        n.ref.general_referent = None
                    p.replace_values(n)
