...
class NounPhraseItem(MetaToken):
    ...
    def clone(self) -> 'NounPhraseItem':
        res = NounPhraseItem(self.begin_token, self.end_token)
        res.morph = self.morph.clone()
        res.conj_before = self.conj_before
        res.can_be_adj = self.can_be_adj
        res.can_be_noun = self.can_be_noun
        res.multi_nouns = self.multi_nouns
        res.can_be_surname = self.can_be_surname
        res.is_std_adjective = self.is_std_adjective
        res.is_doubt_adjective = self.is_doubt_adjective
        res.adj_morph.extend(self.adj_morph)
        res.noun_morph.extend(self.noun_morph)
        return res
        ...
        for it in self.morph.items: 
            ...
            if (num == MorphNumber.SINGULAR and v.single_number_value is not None): 
                if (mc is not None and ((gender == MorphGender.NEUTER or gender == MorphGender.FEMINIE)) and mc.is_adjective): 
                    bi = MorphBaseInfo._new527(MorphClass._new53(mc.value), gender, MorphNumber.SINGULAR, MorphCase.NOMINATIVE, self.morph.language)
                    str0_ = None
                    try: 
                        str0_ = MorphologyService.get_wordform(v.single_number_value, bi)
                    except Exception as ex528: 
                        pass
            ...
            if (str.isdigit(v.normal_value[0]) and mc is not None and mc.is_adjective): 
                val = 0
                wrapval529 = RefOutArgWrapper(0)
                inoutres530 = Utils.tryParseInt(v.normal_value, wrapval529)
                val = wrapval529.value
                if (inoutres530): 
            ...
            if (self.begin_token != self.end_token and self.end_token.is_value(res1, None)): 
                res1 = (MiscHelper.get_text_value(self.begin_token, self.end_token.previous, GetTextAttr.NO) + res1)
    ...
    def try_parse(t : 'Token', items : typing.List['NounPhraseItem'], attrs : 'NounPhraseParseAttr') -> 'NounPhraseItem':
        ...
        if (isinstance(t, TextToken)): 
                    ...
                    if ((isinstance(wf, MorphWordForm)) and wf.is_in_dictionary): 
                        ...
                        if (wf.class0_.is_adjective): 
                            if (wf.contains_attr("к.ф.", None)): 
                                if (t.get_morph_class_in_dictionary().equals(MorphClass.ADJECTIVE) and not t.morph.contains_attr("неизм.", None)): 
                                    pass
                                else: 
                                    _is_doubt_adj = True
            mc0 = t.morph.class0_
            ...
            if (mc0.is_proper_name and not t.chars.is_all_lower): 
                for wff in t.morph.items: 
                    ...
                    if (wf is None): 
                        continue
                    if (wf.normal_case == "ГОР" or wf.normal_case == "ГОРЫ" or wf.normal_case == "ПОЛ"): 
                        continue
            ...
            if (mc1.equals(MorphClass.VERB) and t.morph.case_.is_undefined): 
                return None
        ...
        for k in range(2):
            ...
            if (k == 0): 
                if (((isinstance(t0, TextToken)) and t0.next0_ is not None and t0.next0_.is_hiphen) and t0.next0_.next0_ is not None and t0.chars.is_cyrillic_letter == t0.next0_.next0_.chars.is_cyrillic_letter): 
                    if (not t0.is_whitespace_after and not t0.morph.class0_.is_pronoun and not (isinstance(t0.next0_.next0_, NumberToken))): 
                        if (not t0.next0_.is_whitespace_after): 
                            ...
                            t = t0.next0_.next0_
            it = NounPhraseItem._new531(t0, t, _can_be_surname)
                        ...
                        if (NounPhraseItem.try_accord_variant(items, (0 if items is None else len(items)), v, False)): 
                            ...
                            if (t.next0_ is not None and t.next0_.is_comma_and and (isinstance(t.next0_.next0_, TextToken))): 
                                ...
                                if (npt2 is not None and npt2.preposition is None and not ((npt2.morph.case_) & v.case_ & items[0].adj_morph[0].case_).is_undefined): 
                                    it.noun_morph.append(NounPhraseItemTextVar(v, t))
                                    it.can_be_noun = True
                        elif ((((attrs) & (NounPhraseParseAttr.PARSENUMERICASADJECTIVE))) != (NounPhraseParseAttr.NO) and len(items) > 0 and (((isinstance(items[0].begin_token, NumberToken)) or (isinstance(items[len(items) - 1].begin_token, NumberToken))))): 
                            it.noun_morph.append(NounPhraseItemTextVar(v, t))
                            it.can_be_noun = True
            if (t0 != t): 
            ...
            if (it.can_be_noun or it.can_be_adj or k == 1): 
                ...
                return it
            if (t0 == t): 
                if (t0.is_value("БИЗНЕС", None) and t0.next0_ is not None and t0.next0_.chars.equals(t0.chars)): 
                    t1 = t0.next0_
                    continue
                return it
        return None
    ...
    @staticmethod
    def _new531(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NounPhraseItem':
