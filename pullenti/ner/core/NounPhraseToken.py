...
class NounPhraseToken(MetaToken):
    ...
    def clone(self) -> 'NounPhraseToken':
        res = NounPhraseToken(self.begin_token, self.end_token)
        res.morph = self.morph.clone()
        res.chars = self.chars
        if (isinstance(self.noun, NounPhraseItem)): 
            res.noun = (self.noun.clone())
        else: 
            res.noun = self.noun
        res.internal_noun = self.internal_noun
        if (self.adverbs is not None): 
            res.adverbs = list(self.adverbs)
        for a in self.adjectives: 
            if (isinstance(a, NounPhraseItem)): 
                res.adjectives.append(a.clone())
            else: 
                res.adjectives.append(a)
        res.anafor = self.anafor
        res.anafora_ref = self.anafora_ref
        res.preposition = self.preposition
        return res
    ...
    def get_multivars(self) -> typing.List['NounPhraseMultivarToken']:
        ...
        while i < len(self.adjectives): 
            v = NounPhraseMultivarToken._new631(self.adjectives[i].begin_token, self.adjectives[i].end_token, self, i, i)
    ...
    def get_morph_variant(self, cas : 'MorphCase', plural : bool) -> str:
        ...
        """
        mi = MorphBaseInfo._new632(cas, MorphLang.RU)
        if (plural): 
    ...
    @staticmethod
    def _new593(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PrepositionToken') -> 'NounPhraseToken':
        res = NounPhraseToken(_arg1, _arg2)
        res.preposition = _arg3
        return res
    
    @staticmethod
    def _new3592(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection') -> 'NounPhraseToken':
        res = NounPhraseToken(_arg1, _arg2)
        res.morph = _arg3
        return res