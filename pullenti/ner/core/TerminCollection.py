...
class TerminCollection:
    ...
    def __add_to_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                nod.children = dict()
            nn = None
            wrapnn724 = RefOutArgWrapper(None)
            inoutres725 = Utils.tryGetValue(nod.children, ch, wrapnn724)
            nn = wrapnn724.value
            if (not inoutres725): 
    ...
    def __remove_from_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                return
            nn = None
            wrapnn726 = RefOutArgWrapper(None)
            inoutres727 = Utils.tryGetValue(nod.children, ch, wrapnn726)
            nn = wrapnn726.value
            if (not inoutres727): 
                return
            nod = nn
    ...
    def __find_in_tree(self, key : str, lang : 'MorphLang') -> typing.List['Termin']:
        ...
        while i < len(key): 
            ...
            if (nod.children is not None): 
                wrapnn728 = RefOutArgWrapper(None)
                Utils.tryGetValue(nod.children, ch, wrapnn728)
                nn = wrapnn728.value
            if (nn is None): 
                if (ch == (32)): 
    ...
    def __add_to_hash1(self, key : int, t : 'Termin') -> None:
        li = None
        wrapli729 = RefOutArgWrapper(None)
        inoutres730 = Utils.tryGetValue(self.__m_hash1, key, wrapli729)
        li = wrapli729.value
        if (not inoutres730): 
            li = list()
    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
            ...
            if ((((pars) & (TerminParseAttr.TERMONLY))) != (TerminParseAttr.NO)): 
                ...
                while i < tt.invariant_prefix_length_of_morph_vars: 
                    ...
                    if (nod.children is None): 
                        ...
                        break
                    nn = None
                    wrapnn731 = RefOutArgWrapper(None)
                    inoutres732 = Utils.tryGetValue(nod.children, ch, wrapnn731)
                    nn = wrapnn731.value
                    if (not inoutres732): 
                        no_vars = True
                        break
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn733 = RefOutArgWrapper(None)
                        inoutres734 = Utils.tryGetValue(nod.children, ch, wrapnn733)
                        nn = wrapnn733.value
                        if (not inoutres734): 
                            no_vars = True
                            break
                        ...
                        i += 1
            if (not no_vars): 
                wrapres739 = RefOutArgWrapper(res)
                inoutres740 = self.__manage_var(token, pars, s, nod, len0, wrapres739)
                res = wrapres739.value
                if (inoutres740): 
                    was_vars = True
                i = 0
                first_pass3731 = True
                while True:
                    ...
                    if (wf.normal_case is None or wf.normal_case == s): 
                        ...
                        if (j < i): 
                            ok = False
                    if (ok): 
                        wrapres735 = RefOutArgWrapper(res)
                        inoutres736 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres735)
                        res = wrapres735.value
                        if (inoutres736): 
                            was_vars = True
                    ...
                    if (j < i): 
                        continue
                    wrapres737 = RefOutArgWrapper(res)
                    inoutres738 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres737)
                    res = wrapres737.value
                    if (inoutres738): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres741 = RefOutArgWrapper(res)
            inoutres742 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres741)
            res = wrapres741.value
            if (inoutres742): 
                was_vars = True
        else: 
            return None
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars743 = RefOutArgWrapper(None)
            inoutres744 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars743)
            vars0_ = wrapvars743.value
            if (inoutres744): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn745 = RefOutArgWrapper(None)
            inoutres746 = Utils.tryGetValue(nod.children, ch, wrapnn745)
            nn = wrapnn745.value
            if (not inoutres746): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ...
                inoutres748 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli747)
                li = wrapli747.value
                if (not inoutres748): 
                    li = list()
                ...
                if (not t in li): 
                    li.append(t)
        res = [ ]
        wrapres749 = RefOutArgWrapper(None)
        inoutres750 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres749)
        res = wrapres749.value
        if (not inoutres750): 
            return None
        else: 
            return res