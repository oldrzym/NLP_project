...
class IntOntologyCollection:
    ...
    class OntologyTermin(Termin):
        ...
        @staticmethod
        def _new616(_arg1 : 'IntOntologyItem', _arg2 : object) -> 'OntologyTermin':
    ...
    def __init__(self) -> None:
        ...
        self.__m_items = list()
        self.m_termins = TerminCollection()
    ...
    def add_item(self, di : 'IntOntologyItem') -> None:
        ...
        while i < len(di.termins): 
            if (isinstance(di.termins[i], IntOntologyCollection.OntologyTermin)): 
                ...
                self.m_termins.add(di.termins[i])
            else: 
                nt = IntOntologyCollection.OntologyTermin._new616(di, di.termins[i].tag)
                di.termins[i].copy_to(nt)
                self.m_termins.add(nt)
                di.termins[i] = (nt)
            i += 1
    ...
    def add_termin(self, di : 'IntOntologyItem', t : 'Termin') -> None:
        nt = IntOntologyCollection.OntologyTermin._new616(di, t.tag)
        t.copy_to(nt)
        self.m_termins.add(nt)
    
    def add(self, t : 'Termin') -> None:
        self.m_termins.add(t)
    
    def find_termin_by_canonic_text(self, text : str) -> typing.List['Termin']:
        return self.m_termins.find_termins_by_canonic_text(text)
    
    def try_attach(self, t : 'Token', referent_type_name : str=None, can_be_geo_object : bool=False) -> typing.List['IntOntologyToken']:
        tts = self.m_termins.try_parse_all(t, (TerminParseAttr.CANBEGEOOBJECT if can_be_geo_object else TerminParseAttr.NO))
        if (tts is None): 
            return None
        res = list()
        dis = list()
        for tt in tts: 
            ...
            if (di is not None): 
                ...
                if (di in dis): 
                    continue
                dis.append(di)
            res.append(IntOntologyToken._new618(tt.begin_token, tt.end_token, di, tt.termin, tt.morph))
    ...
    def try_attach_by_item(self, item : 'IntOntologyItem') -> typing.List['IntOntologyItem']:
        ...
        for t in item.termins: 
            li = self.m_termins.find_termins_by_termin(t)
            if (li is not None): 
                for tt in li: 
    ...
    @staticmethod
    def _new3461(_arg1 : bool) -> 'IntOntologyCollection':
