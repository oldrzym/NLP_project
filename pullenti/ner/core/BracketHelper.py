...
class BracketHelper:
    ...
    def try_parse(t : 'Token', attrs : 'BracketParseAttr'=BracketParseAttr.NO, max_tokens : int=100) -> 'BracketSequenceToken':
        ...
        while True:
            ...
            if (t.is_char_of(BracketHelper.M_OPEN_CHARS) or t.is_char_of(BracketHelper.M_CLOSE_CHARS)): 
                ...
                if (len(br_list) > 20): 
                    break
                if (((len(br_list) == 3 and br_list[1].can_be_open and bb.can_be_close) and bb.char0_ != ')' and BracketHelper.__must_be_close_char(bb.char0_, br_list[1].char0_)) and BracketHelper.__must_be_close_char(bb.char0_, br_list[0].char0_)): 
                    ok = False
                    tt = t.next0_
                        ...
                        if (tt.is_char('.')): 
                            ...
                            while tt is not None: 
                                if (tt.is_newline_before): 
                                    ...
                                    if (BracketHelper.can_be_end_of_sequence(tt, False, None, False) and BracketHelper.__can_be_close_char(bb2.char0_, br_list[0].char0_, False)): 
                                        ok = True
                                    break
                                tt = tt.next0_
                            break
                ...
                if ((((attrs) & (BracketParseAttr.CANCONTAINSVERBS))) == (BracketParseAttr.NO)): 
                    if (t.morph.language.is_cyrillic): 
                        if (t.get_morph_class_in_dictionary().equals(MorphClass.VERB)): 
                        ...
                        if (t.morph.class0_.equals(MorphClass.VERB) and t.chars.is_all_lower): 
                            break
                    r = t.get_referent()
            ...
            if (t.is_char(';') and t.is_newline_after): 
                break
            if ((((attrs) & (BracketParseAttr.INTERNALUSAGE))) == (BracketParseAttr.NO)): 
        ...
        while len(br_list) > 3:
            i = len(br_list) - 1
            if ((br_list[i].can_be_close and br_list[i - 1].can_be_open and not BracketHelper.__can_be_close_char(br_list[i].char0_, br_list[0].char0_, False)) and BracketHelper.__can_be_close_char(br_list[i].char0_, br_list[i - 1].char0_, False)): 
            ...
            while i < (len(br_list) - 2): 
                if ((br_list[i].can_be_open and not br_list[i].can_be_close and br_list[i + 1].can_be_close) and not br_list[i + 1].can_be_open): 
                    ...
                    if (BracketHelper.__must_be_close_char(br_list[i + 1].char0_, br_list[i].char0_) or br_list[i].char0_ != br_list[0].char0_): 
                        ok = True
                        if ((i == 1 and ((i + 2) < len(br_list)) and br_list[i + 2].char0_ == ')') and br_list[i + 1].char0_ != ')' and BracketHelper.__can_be_close_char(br_list[i + 1].char0_, br_list[i - 1].char0_, False)): 
        ...
        if ((len(br_list) >= 4 and br_list[1].can_be_open and br_list[2].can_be_close) and br_list[3].can_be_close and not br_list[3].can_be_open): 
            if (BracketHelper.__can_be_close_char(br_list[3].char0_, br_list[0].char0_, False)): 
        ...
        if ((res is None and len(br_list) >= 3 and br_list[2].can_be_close) and not br_list[2].can_be_open): 
            if ((((attrs) & (BracketParseAttr.NEARCLOSEBRACKET))) != (BracketParseAttr.NO)): 
                if (BracketHelper.__can_be_close_char(br_list[1].char0_, br_list[0].char0_, False)): 
                    return BracketSequenceToken(br_list[0].source, br_list[1].source)
            ok = True
            if (BracketHelper.__can_be_close_char(br_list[2].char0_, br_list[0].char0_, False) and BracketHelper.__can_be_close_char(br_list[1].char0_, br_list[0].char0_, False) and br_list[1].can_be_close): 
                t = br_list[1].source
                ...
                while True:
                    ...
                    if (tt.chars.is_letter or tt.length_char > 1): 
                        continue
                    ch = tt.term[0]
                    if (BracketHelper.__can_be_close_char(ch, br_list[0].char0_, False)): 
                        lev1 += 1
                    elif (BracketHelper.__can_be_close_char(br_list[1].char0_, ch, False)): 
                        lev1 -= 1
                        if (lev1 < 0): 
                            return BracketSequenceToken(br_list[0].source, br_list[1].source)
            if (ok and BracketHelper.__can_be_close_char(br_list[2].char0_, br_list[0].char0_, False)): 
            ...
            elif (ok and BracketHelper.__can_be_close_char(br_list[2].char0_, br_list[1].char0_, False) and br_list[0].can_be_open): 
                if (BracketHelper.__can_be_close_char(br_list[2].char0_, br_list[0].char0_, False)): 
                    intern = BracketSequenceToken(br_list[1].source, br_list[2].source)
                    res = BracketSequenceToken(br_list[0].source, br_list[2].source)
                    res.internal.append(intern)
                elif (len(br_list) == 3): 
                    return None
        if (res is None and len(br_list) > 1): 
            has_cr = False
            tt = br_list[0].source.next0_
            while tt.end_char < br_list[1].source.begin_char: 
                if (tt.is_newline_after): 
                    has_cr = True
                tt = tt.next0_
            if (BracketHelper.__can_be_close_char(br_list[1].char0_, br_list[0].char0_, has_cr)): 
            ...
            while True:
                ...
                if (isinstance(mt.end_token, TextToken)): 
                    if (mt.end_token.is_char_of(BracketHelper.M_CLOSE_CHARS)): 
                        ...
                        if (bb.can_be_close and BracketHelper.__can_be_close_char(bb.char0_, br_list[0].char0_, False)): 
    ...
    @staticmethod
    def __can_be_close_char(close0_ : 'char', open0_ : 'char', strict : bool=False) -> bool:
        ...
        if (i == j): 
            return True
        if (not strict): 
            if (open0_ == '"' and ((close0_ == '“' or close0_ == '”' or close0_ == '»'))): 
                return True
            if (open0_ == '”' and ((close0_ == '“' or close0_ == '"'))): 
                return True
            if (open0_ == '“' and ((close0_ == '"' or close0_ == '”'))): 
                return True
        return False
