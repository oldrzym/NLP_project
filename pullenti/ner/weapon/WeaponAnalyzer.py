...
class WeaponAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        first_pass4134 = True
        while True:
            if first_pass4134: first_pass4134 = False
            else: t = t.next0_
                        ...
                        if (s.type_name == WeaponReferent.ATTR_MODEL): 
                            ...
                            for k in range(2):
                                if (not str.isdigit(mod[0])): 
                                    li = [ ]
                                    wrapli3450 = RefOutArgWrapper(None)
                                    inoutres3451 = Utils.tryGetValue(objs_by_model, mod, wrapli3450)
                                    li = wrapli3450.value
                                    if (not inoutres3451): 
                                        li = list()
        ...
        first_pass4135 = True
        while True:
            if first_pass4135: first_pass4135 = False
            else: t = t.next0_
            if (not (t is not None)): break
            if (t.is_ignored): 
                continue
    ...
    def __try_attach(self, its : typing.List['WeaponItemToken'], attach : bool) -> typing.List['ReferentToken']:
        ...
        first_pass4136 = True
        while True:
            if first_pass4136: first_pass4136 = False
            else: i += 1
        ...
        if (noun is None and model is not None): 
            ...
            first_pass4137 = True
            while True:
                if first_pass4137: first_pass4137 = False
