...
class MorphCollection(MorphBaseInfo):
    ...
    def __remove_items_morph_case(self, cas : 'MorphCase') -> None:
        ...
        for i in range(len(self.__m_items) - 1, -1, -1):
            if (((self.__m_items[i].case_) & cas).is_undefined): 
                del self.__m_items[i]
                self.__m_need_recalc = True
            elif (not ((self.__m_items[i].case_) & cas).equals(self.__m_items[i].case_)): 
    ...
    def __remove_items_morph_base_info(self, inf : 'MorphBaseInfo') -> None:
        ...
        for i in range(len(self.__m_items) - 1, -1, -1):
            ...
            if (not ok): 
                ...
                if (not inf.case_.is_undefined): 
                    if (not it.case_.equals((inf.case_) & it.case_)): 
    ...
    @staticmethod
    def _new652(_arg1 : 'MorphClass') -> 'MorphCollection':
        res = MorphCollection()
        res.class0_ = _arg1
        return res
    
    @staticmethod
    def _new2950(_arg1 : 'MorphGender') -> 'MorphCollection':
        res = MorphCollection()
        res.gender = _arg1
        return res
    
    @staticmethod
    def _new3014(_arg1 : 'MorphCase') -> 'MorphCollection':
        res = MorphCollection()
        res.case_ = _arg1
        return res
