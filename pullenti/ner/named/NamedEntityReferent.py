...
class NamedEntityReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang', lev : int=0) -> str:
        ...
        if (re is not None): 
            if (res.tell() > 0): 
                print("; ", end="", file=res)
            print(re.to_string_ex(short_variant, lang, lev + 1), end="", file=res)
    ...
    def to_sort_string(self) -> str:
        return Utils.enumToString(self.kind) + self.to_string_ex(True, MorphLang.UNKNOWN, 0)
    ...
    @staticmethod
    def _new2297(_arg1 : 'NamedEntityKind') -> 'NamedEntityReferent':
