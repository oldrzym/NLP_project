        ...
        if (res.typ == MailLine.Types.UNDEFINED): 
            ...
            while True:
                ...
                if (isinstance(t, TextToken)): 
                    ...
                    if (tok is not None and (isinstance(t, TextToken)) and t.is_value("ДОРОГОЙ", None)): 
                        if (t.term == "ДОРОГОЙ" or t.term == "ДОРОГАЯ" or t.term == "ДОРОГИЕ"): 
                            pass
                        else: 
                            tok = (None)
                    if (tok is not None): 
                    ...
                    if (t.is_value("ВСЕ", None) or t.is_value("ALL", None) or t.is_value("TEAM", None)): 
                        nams += 1
                        continue
                    pit = PersonItemToken.try_attach(t, PersonItemToken.ParseAttr.NO, None)
                    if (pit is not None): 
                        nams += 1
                        t = pit.end_token
                        continue
            ...
            if (t is not None): 
                ...
                if (((t.is_value("ПЕРЕСЫЛАЕМОЕ", None) or t.is_value("ПЕРЕАДРЕСОВАННОЕ", None))) and t.next0_ is not None and t.next0_.is_value("СООБЩЕНИЕ", None)): 
                    ...
                    while True:
                        ...
                        if (not (t is not None and t.end_char <= tmax)): break
