...
class InstrumentParticipantReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang', lev : int=0) -> str:
        ...
        if (org0_ is not None): 
            print(": {0}".format(org0_.to_string_ex(short_variant, lang, 0)), end="", file=res, flush=True)
            if (not short_variant and del0_ is not None): 
                print(" (в лице {0})".format(del0_.to_string_ex(True, lang, lev + 1)), end="", file=res, flush=True)
        elif (del0_ is not None): 
            print(": в лице {0}".format(del0_.to_string_ex(short_variant, lang, lev + 1)), end="", file=res, flush=True)
    ...
    @staticmethod
    def _new1936(_arg1 : str) -> 'InstrumentParticipantReferent':
