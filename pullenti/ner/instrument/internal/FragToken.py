...
class FragToken(MetaToken):
    ...
    def fill_by_content_children(self) -> None:
        self.sort_children()
        if (len(self.children) == 0): 
            self.children.append(FragToken._new1850(self.begin_token, self.end_token, InstrumentKind.CONTENT))
            return
        if (self.begin_char < self.children[0].begin_char): 
            self.children.insert(0, FragToken._new1850(self.begin_token, self.children[0].begin_token.previous, InstrumentKind.CONTENT))
        i = 0
        while i < (len(self.children) - 1): 
            if (self.children[i].end_token.next0_ != self.children[i + 1].begin_token and (self.children[i].end_token.next0_.end_char < self.children[i + 1].begin_char)): 
                self.children.insert(i + 1, FragToken._new1850(self.children[i].end_token.next0_, self.children[i + 1].begin_token.previous, InstrumentKind.CONTENT))
            i += 1
        if (self.children[len(self.children) - 1].end_char < self.end_char): 
            self.children.append(FragToken._new1850(self.children[len(self.children) - 1].end_token.next0_, self.end_token, InstrumentKind.CONTENT))
    ...
    def create_document(t : 'Token', max_char : int, root_kind : 'InstrumentKind'=InstrumentKind.UNDEFINED) -> 'FragToken':
        ...
        while True:
            ...
            if (tt.is_newline_before and tt.is_newline_after): 
                if (isinstance(tt.get_referent(), DecreePartReferent)): 
                    if (tt.next0_ is not None and tt.next0_.is_value("ВВЕСТИ", None)): 
                        pass
                    else: 
                        tt = t.kit.debed_token(tt)
                    continue
            if (tt.is_newline_before and (isinstance(tt.get_referent(), DecreeChangeReferent))): 
                dch = Utils.asObjectOrNull(tt.get_referent(), DecreeChangeReferent)
                if (dch.kind == DecreeChangeKind.CONTAINER and t.begin_token.is_value("ИЗМЕНЕНИЕ", None)): 
        ...
        if (res is not None): 
            return res
        res = FragToken._new1850(t, t, InstrumentKind.DOCUMENT)
        ...
        while True:
            ...
            if (li[i].typ == ILTypes.APPENDIX or li[i].typ == ILTypes.APPROVED): 
                if (li[i].typ == ILTypes.APPROVED): 
                    ...
                    if (i == 0): 
                        continue
                    if (not li[i].is_newline_after): 
                        if (((i + 1) < len(li)) and li[i + 1].is_newline_after and li[i + 1].typ == ILTypes.UNDEFINED): 
                            pass
                        else: 
                            continue
        ...
        if (max_char == 0 and i1 == len(li)): 
            i = 0
            while i < len(li): 
                if (li[i].typ == ILTypes.PERSON and li[i].is_newline_before and ((not li[i].has_table_chars or li[i].is_pure_person))): 
                    j = 0
                    dat = False
                    num = False
                    geo_ = False
        ...
        for i in range(i1 - 1, 0, -1):
            ...
            if ((lii.typ == ILTypes.PERSON or lii.typ == ILTypes.REGNUMBER or lii.typ == ILTypes.DATE) or lii.typ == ILTypes.GEO): 
                if (not lii.is_newline_before and li[i - 1].typ == ILTypes.UNDEFINED and li[i - 1].has_verb): 
                    break
                if (len(pers_list) > 0): 
                ...
                if (not lii.is_newline_before and not lii.begin_token.is_table_control_char and ((lii.typ == ILTypes.GEO or li[i].typ == ILTypes.PERSON))): 
                    ...
                    if (not lii.is_newline_after): 
                        if (lii.end_token.next0_ is not None and not lii.end_token.next0_.is_table_control_char): 
                            break
                if (tail is None): 
                    tail = FragToken._new1850(li[i].begin_token, li[i1 - 1].end_token, InstrumentKind.TAIL)
                    if ((i1 - 1) > i): 
                        pass
                ...
                if (li[i].is_podpis_storon and tail is not None): 
                    ...
                    tail.children.insert(0, FragToken._new1901(li[i].begin_token, li[i].end_token, InstrumentKind.NAME, ss))
                    cmax = (i - 1)
                    break
                jj = 0
                jj = 0
        ...
        if (cmax < 0): 
            ...
            content = FragToken._new1850(li[0].begin_token, li[cmax].end_token, InstrumentKind.CONTENT)
        ...
        if (tail is not None): 
            ...
            while i1 < len(li): 
                if (li[i1].begin_token == li[i1].end_token and (isinstance(li[i1].begin_token.get_referent(), DecreeReferent)) and li[i1].begin_token.get_referent().kind == DecreeKind.PUBLISHER): 
                    ap = FragToken._new1850(li[i1].begin_token, li[i1].end_token, InstrumentKind.APPROVED)
                    ap.referents = list()
            ...
            if (len(tail.children) > 0 and (tail.children[len(tail.children) - 1].end_char < tail.end_char)): 
                unkw = FragToken._new1850(tail.children[len(tail.children) - 1].end_token.next0_, tail.end_token, InstrumentKind.UNDEFINED)
        ...
        while i < len(li): 
            ...
            if (li[j - 1].typ == ILTypes.PERSON and li[j - 1].is_newline_before and li[j - 1].is_newline_after): 
                tail = FragToken._new1850(li[j - 1].begin_token, li[j - 1].end_token, InstrumentKind.TAIL)
                for jj in range(j - 1, i, -1):
                    if (li[jj].typ != ILTypes.PERSON or not li[jj].is_newline_before or not li[jj].is_newline_after): 
                        break
                    else: 
                        fr = FragToken._new1850(li[jj].begin_token, li[jj].end_token, InstrumentKind.SIGNER)
            ...
            if (title is None): 
                ...
                if (is_app and app.kind == InstrumentKind.APPENDIX): 
                    ...
                    res0 = FragToken._new1907(res.begin_token, res.end_token, res._m_doc, InstrumentKind.DOCUMENT)
                    res._m_doc = (None)
                ...
                if (title.end_token.next0_ is not None): 
                    if (title.end_token.end_char < app.end_token.begin_char): 
                        acontent = FragToken._new1850(title.end_token.next0_, app.end_token, InstrumentKind.CONTENT)
    ...
    def __create_case_info(t : 'Token') -> 'FragToken':
        ...
        if ((not has_date and t1.next0_ is not None and (isinstance(t1.next0_.get_referent(), DateReferent))) and t1.next0_.is_newline_after): 
            t1 = t1.next0_
        return FragToken._new1850(t, t1, InstrumentKind.CASEINFO)
    ...
    def __create_approved(t : 'Token') -> 'FragToken':
        ...
        if (((isinstance(t, ReferentToken)) and t.begin_token.is_char('(') and t.end_token.is_char(')')) and t.begin_token.next0_.is_value("ПРОТОКОЛ", None)): 
            res = FragToken._new1850(t, t, InstrumentKind.APPROVED)
            res.referents = list()
            res.referents.append(t.get_referent())
            return res
        tt = InstrToken._check_approved(t)
        if (tt is not None): 
            res = FragToken._new1850(t, tt, InstrumentKind.APPROVED)
        ...
        if (dts is not None and len(dts) > 0): 
            ...
            while i < len(dts): 
                ...
                if (dt.typ == DecreeToken.ItemType.ORG): 
                    res.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.ORGANIZATION, dt))
                elif (dt.typ == DecreeToken.ItemType.OWNER): 
                    res.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.INITIATOR, dt))
                elif (dt.typ == DecreeToken.ItemType.DATE): 
                    res.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.DATE, dt))
                elif (dt.typ == DecreeToken.ItemType.NUMBER and i > 0 and dts[i - 1].typ == DecreeToken.ItemType.DATE): 
                    res.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.NUMBER, dt))
                    ...
                    res.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.PLACE, dt))
                else: 
                    break
    ...
    def _create_misc(t : 'Token') -> 'FragToken':
        ...
        if (t.is_value("ФОРМА", None) and t.next0_.is_value("ДОКУМЕНТА", None)): 
            ...
            if (num is not None and num.typ == DecreeToken.ItemType.NUMBER): 
                return FragToken._new1850(t, num.end_token, InstrumentKind.UNDEFINED)
            if ((isinstance(t.next0_.next0_, NumberToken)) and t.next0_.next0_.is_newline_after): 
                return FragToken._new1850(t, t.next0_.next0_, InstrumentKind.UNDEFINED)
        if (t.is_value("С", None) and t.next0_.is_value("ИЗМЕНЕНИЕ", None) and t.next0_.next0_ is not None): 
            ...
            if (isinstance(tt.get_referent(), DateReferent)): 
                if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                    tt = tt.next0_
                return FragToken._new1850(t, tt, InstrumentKind.UNDEFINED)
        t0 = t
        ...
        if (t.is_value("ЗАКАЗ", None)): 
            ...
            if (itt is not None): 
                return FragToken._new1850(t, itt.end_token, InstrumentKind.UNDEFINED)
        nums = 0
        spec = 0
        chars_ = 0
        words = 0
        ...
        while True:
            ...
            ddd = DecreeToken.try_attach(t1, None, False)
            if (ddd is not None): 
                break
            if (t1.is_table_control_char): 
                ...
                nums += 1
            elif (not (isinstance(t1, TextToken)) or t1.length_char > 7): 
                break
            elif (not t1.chars.is_letter): 
                spec += 1
            elif (t1.length_char < 3): 
                chars_ += 1
            elif (t1.get_morph_class_in_dictionary().is_undefined): 
                chars_ += 1
            else: 
                words += 1
            if (not t1.is_newline_after): 
                continue
            if ((nums + spec + chars_) <= 1): 
                break
            if ((nums + spec + chars_) > (words * 3)): 
                pass
            elif ((words < 2) and (nums + spec + chars_) > 3): 
                pass
            else: 
                break
            return FragToken._new1850(t0, t1, InstrumentKind.UNDEFINED)
        if (isinstance(t.get_referent(), StreetReferent)): 
            lin = InstrToken1.parse(t0, True, None, 0, None, False, 0, False, False)
            if (lin is not None and (lin.length_char < 70)): 
                return FragToken._new1850(t0, lin.end_token, InstrumentKind.CONTACT)
        if (t0.is_char('(')): 
            br = BracketHelper.try_parse(t0, BracketParseAttr.NO, 100)
            if (br is not None and br.is_newline_after): 
                return FragToken._new1850(t0, br.end_token, InstrumentKind.UNDEFINED)
        return None
    ...
    def _create_editions(t : 'Token') -> 'FragToken':
        ...
        if ((t.is_newline_before and t.is_value("С", None) and t.next0_ is not None) and t.next0_.is_value("ИЗМЕНЕНИЕ", None)): 
            eee = FragToken._new1850(t, t, InstrumentKind.EDITIONS)
            t = t.next0_.next0_
            while t is not None: 
                if (t.is_comma_and or (isinstance(t.get_referent(), DateReferent))): 
                    ...
                    dd = DateItemToken.try_parse(t, None, False)
                    if (dd is not None): 
                        t = dd.end_token
                        eee.end_token = t
                    else: 
        ...
        if (not t.is_char('(') and not t.is_value("ПРИЛОЖЕНИЕ", "ДОДАТОК")): 
            if (t.is_value("В", "У") and t.next0_ is not None and ((t.next0_.is_value("РЕДАКЦИЯ", "РЕДАКЦІЯ") or t.next0_.is_value("РЕД", None)))): 
                ...
                if (dtt0 is not None): 
                    return FragToken._new1850(t, dtt0.end_token, InstrumentKind.EDITIONS)
            elif (t.is_char('{') and t.is_newline_before): 
                is_in_fig = True
            else: 
                return None
        else: 
        ...
        elif (is_in_bracks and t is not None and (isinstance(t.get_referent(), DecreePartReferent))): 
            t = t.next0_
        if (t is None): 
            return None
        is_doubt = False
        ...
        if (t.is_value("РЕДАКЦИЯ", "РЕДАКЦІЯ")): 
            ok = True
        elif (t.is_value("ОТФОРМАТИРОВАН", None)): 
            ok = True
        ...
        while True:
            ...
            if (is_in_bracks): 
                ...
                if (t.is_char(')')): 
                    break
            if (is_in_fig and t.is_char('}') and t.is_newline_after): 
                break
            dr = Utils.asObjectOrNull(t.get_referent(), DecreeReferent)
            if (dr is not None): 
                decrs.append(dr)
        ...
        if (ok and not is_doubt): 
            eds = FragToken._new1850(t0, t, InstrumentKind.EDITIONS)
            eds.referents = list()
            for d in decrs: 
    ...
    def __create_owner(t : 'Token') -> 'FragToken':
        ...
        while True:
            ...
            if ((isinstance(r, GeoReferent)) and tt.is_newline_before): 
                break
            t1 = tt
            oo = tt.kit.process_referent("ORGANIZATION", tt, None)
            if (oo is not None): 
                tt = oo.end_token
                t1 = tt
                continue
            ...
            if (npt is not None): 
                if (tt == t): 
                    typ = OrgItemTypeToken.try_attach(tt, False)
                    if (typ is not None): 
                        keyword = True
                        tt = typ.end_token
                        t1 = tt
        ...
        if (t1 is None): 
            return None
        fr = FragToken._new1869(t, t1, InstrumentKind.ORGANIZATION, True)
        return fr
            ...
            if (isinstance(t.get_referent(), DecreeChangeReferent)): 
                ...
                while True:
                    ...
                    if (t0 is not None and ((t0.end_char + 50) < t1.end_char)): 
                        return MetaToken._new630(t0, t1, dcr)
                    return None
            if (t.end_char > self.end_char): 
                break
            t = t.next0_
        ...
        while t is not None and (cou < 30): 
            ...
            if (t.is_value("ОТВЕТ", None)): 
                if (t.is_newline_after): 
                    ...
                    ok = True
            if (ok): 
                res = FragToken._new1850(t0, li.end_token, InstrumentKind.HEAD)
                if (li.begin_token != t0): 
                    hh = FragToken._new1850(t0, li.begin_token.previous, InstrumentKind.APPROVED)
                    res.children.append(hh)
                res.children.append(FragToken._new1872(li.begin_token, li.end_token, InstrumentKind.KEYWORD, True))
                return res
            t = li.end_token
            t = t.next0_; cou += 1
        return None
    ...
    def __create_contract_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (t1 is None): 
            return None
        p1 = InstrumentParticipantReferent._new1936(par1.typ)
        ...
        if (par1.kind == ParticipantToken.Kinds.NAMEDASPARTS): 
            ...
            while ii < len(par1.parts): 
                pp = InstrumentParticipantReferent._new1936("СТОРОНА {0}".format(ii + 1))
        ...
        title = FragToken._new1850(t0, t0, InstrumentKind.HEAD)
        add = False
        nam_beg = None
        nam_end = None
        dttyp = None
            ...
            if ((dttyp is not None and nam_beg is None and t.chars.is_cyrillic_letter) and (isinstance(t, TextToken))): 
                if (t.is_value("МЕЖДУ", "МІЖ")): 
                    ...
                    if (pp is not None and pp.end_token.next0_ is not None and pp.end_token.next0_.is_and): 
                        ...
                        if (pp2 is not None): 
                            fr = FragToken._new1850(t, pp2.end_token, InstrumentKind.PLACE)
        ...
        if (nam_end is not None and nam_beg is not None): 
            ...
            if (val is not None and len(val) > 3): 
                nam = FragToken._new1901(nam_beg, nam_end, InstrumentKind.NAME, val)
                ...
                if (nam_beg2 is not None and nam_end2 is not None): 
                    ...
                    if (val2 is not None and len(val2) > 3): 
                        nam = FragToken._new1901(nam_beg2, nam_end2, InstrumentKind.NAME, val2)
                        title.children.append(nam)
                        title.sort_children()
        ...
        if (len(title.children) > 0 and title.children[0].begin_char > title.begin_char): 
            title.children.insert(0, FragToken._new1850(title.begin_token, title.children[0].begin_token.previous, InstrumentKind.UNDEFINED))
    ...
    def __create_project_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        while True:
            ...
            if ((isinstance(t, TextToken)) and ((t.term == "ПРОЕКТ" or t.term == "ЗАКОНОПРОЕКТ"))): 
                if ((t.is_value("ПРОЕКТ", None) and t == t0 and (isinstance(t.next0_, ReferentToken))) and (isinstance(t.next0_.get_referent(), OrganizationReferent))): 
                    return None
                is_project = True
                title.children.append(FragToken._new1869(t, t, InstrumentKind.KEYWORD, True))
                doc.add_slot(InstrumentReferent.ATTR_TYPE, "ПРОЕКТ", False, 0)
                continue
            tt = FragToken.__attach_project_enter(t)
            if (tt is not None): 
                is_entered = True
                title.children.append(FragToken._new1850(t, tt, InstrumentKind.APPROVED))
                t = tt
                continue
            tt = FragToken.__attach_project_misc(t)
            if (tt is not None): 
                title.children.append(FragToken._new1850(t, tt, (InstrumentKind.EDITIONS if tt.is_value("ЧТЕНИЕ", "ЧИТАННЯ") else InstrumentKind.UNDEFINED)))
                t = tt
                continue
        ...
        while True:
            ...
            if (t.whitespaces_after_count > 15): 
                ...
                if (t.next0_.get_morph_class_in_dictionary().equals(MorphClass.VERB)): 
                    t11 = t
                    break
        ...
        if (t11 is None): 
            return None
        nam = FragToken._new1869(t00, t11, InstrumentKind.NAME, True)
    ...
    def __create_justice_participants(title : 'FragToken', doc : 'InstrumentReferent') -> None:
        ...
        while t is not None:
            ...
            if (tmp.tell() > 10 and t1 is not None): 
                art = InstrumentArtefactReferent._new1951("предмет")
    ...
    def __create_just_participant(t : 'Token', typ : str) -> 'ReferentToken':
        ...
        while True:
            ...
            if (npt is not None): 
                if ((npt.noun.is_value("УЧРЕЖДЕНИЕ", "УСТАНОВА") or npt.noun.is_value("ПРЕДПРИЯТИЕ", "ПІДПРИЄМСТВО") or npt.noun.is_value("ОРГАНИЗАЦИЯ", "ОРГАНІЗАЦІЯ")) or npt.noun.is_value("КОМПЛЕКС", None)): 
                    ...
                    ok = True
                    continue
            ty = OrgItemTypeToken.try_attach(t, True)
            if (ty is not None): 
                t = ty.end_token
                t1 = t
                ok = True
            ...
            if ((isinstance(t, TextToken)) and t.chars.is_cyrillic_letter and t.chars.is_all_lower): 
                if (t.morph.class0_.equals(MorphClass.VERB) or t.morph.class0_.equals(MorphClass.ADVERB)): 
                    break
        ...
        if (not ok): 
            return None
        pat = InstrumentParticipantReferent._new1936(typ)
        for r in refs: 
            ...
            while True:
                ...
                if (t.is_value("ВЗЫСКАТЬ", "СТЯГНУТИ")): 
                    ...
                    if (sum0_ is not None): 
                        art = InstrumentArtefactReferent._new1951("РЕЗОЛЮЦИЯ")
                        if (gosposh): 
            ...
            if (self.children[i].kind == InstrumentKind.DIRECTIVE and ((i + 1) < len(self.children))): 
                ...
                if ((((s == "РЕШЕНИЕ" or s == "ОПРЕДЕЛЕНИЕ" or s == "ПОСТАНОВЛЕНИЕ") or s == "ПРИГОВОР" or s == "РІШЕННЯ") or s == "ВИЗНАЧЕННЯ" or s == "ПОСТАНОВА") or s == "ВИРОК"): 
                    ...
                    if (ii == (i + 1)): 
                        ...
                        res.append(FragToken._new1850(self.children[i + 1].begin_token, self.children[ii].end_token, InstrumentKind.CONTENT))
                else: 
                    dir0_ = True
        if (res is not None): 
            return res
    ...
    def __create_action_question(t : 'Token', max_char : int) -> 'FragToken':
        ...
        if (res is None): 
            return None
        ques = FragToken._new1850(t, t2.previous, InstrumentKind.QUESTION)
        res.children.insert(0, ques)
        ques.children.append(FragToken._new1850(li[len(li) - 1].begin_token, li[len(li) - 1].end_token, InstrumentKind.KEYWORD))
        content = FragToken._new1850(li[len(li) - 1].end_token.next0_, t2.previous, InstrumentKind.CONTENT)
        ques.children.append(content)
        content._analize_content(res, max_char > 0, InstrumentKind.UNDEFINED)
        if (len(li) > 1): 
            fr = FragToken._new1872(t, li[len(li) - 2].end_token, InstrumentKind.NAME, True)
    ...
    def _analize_tables(self) -> bool:
        ...
        while True:
            ...
            if (t0.end_char < rows[0].begin_char): 
                self.children.append(FragToken._new1850(t0, rows[0].begin_token.previous, InstrumentKind.CONTENT))
            tab = FragToken._new1850(rows[0].begin_token, rows[len(rows) - 1].end_token, InstrumentKind.TABLE)
            self.children.append(tab)
            i = 0
            while i < len(rows): 
                rr = FragToken._new1867(rows[i].begin_token, rows[i].end_token, InstrumentKind.TABLEROW, i + 1)
                ...
                for ce in rows[i].cells: 
                    no += 1
                    cell = FragToken._new1867(ce.begin_token, ce.end_token, InstrumentKind.TABLECELL, no)
                    if (ce.col_span > 1): 
        ...
        if ((t0 is not None and (t0.end_char < self.end_char) and tabs) and t0 != self.end_token): 
            self.children.append(FragToken._new1850(t0, self.end_token, InstrumentKind.CONTENT))
        return tabs
    ...
    def __create_doc_title_(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (t is not None and t.is_value("ДЕЛО", "СПРАВА")): 
            ...
            if (dt is not None and dt.typ == DecreeToken.ItemType.NUMBER): 
                dt.begin_token = t
                title.children.append(FragToken._new1901(t, t, InstrumentKind.KEYWORD, "ДЕЛО"))
                FragToken.__add_title_attr(doc, title, dt)
                t = dt.end_token.next0_
        ...
        while True:
            ...
            if (t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))): 
                ...
                if (iii is not None and iii.typ == InstrToken1.Types.COMMENT): 
                    cmt = FragToken._new1850(iii.begin_token, iii.end_token, InstrumentKind.COMMENT)
                ...
                if (iii is not None and iii.end_token.is_char('?')): 
                    cmt = FragToken._new1850(iii.begin_token, iii.end_token, InstrumentKind.NAME)
                ...
                if ((((t.is_value("ЗАЯВИТЕЛЬ", "ЗАЯВНИК") or t.is_value("ИСТЕЦ", "ПОЗИВАЧ") or t.is_value("ОТВЕТЧИК", "ВІДПОВІДАЧ")) or t.is_value("ДОЛЖНИК", "БОРЖНИК") or t.is_value("КОПИЯ", "КОПІЯ"))) and t.next0_ is not None and ((t.next0_.is_char(':') or t.next0_.is_table_control_char))): 
                    ...
                    if (ptt is not None): 
                        ...
                        if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                t1 = br.end_token
                        ft = FragToken._new1850(t, t1, InstrumentKind.INITIATOR)
                ...
                if (t.is_value("ЦЕНА", "ЦІНА") and t.next0_ is not None and t.next0_.is_value("ИСК", "ПОЗОВ")): 
                    ...
                    if (tt is not None and has_money): 
                        ...
                        if (tt.next0_ is not None and tt.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                tt = br.end_token
                        title.children.append(FragToken._new1850(t, tt, InstrumentKind.CASEINFO))
                        t1 = tt
                        title.end_token = t1
                        t = title.end_token
                        continue
                if (t.is_value("В", "У")): 
                    ...
                    if (tt is not None and (isinstance(tt.get_referent(), OrganizationReferent))): 
                        ...
                        if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                t1 = br.end_token
                        ooo = FragToken._new1850(t, t1, InstrumentKind.ORGANIZATION)
                        ooo.referents = list()
                ...
                if (t.length_char == 1 and t.chars.is_letter and t.is_whitespace_after): 
                    ...
                    while ii < len(InstrToken._m_directives_norm): 
                        ...
                        if (ee is not None and ee.is_newline_after): 
                            ooo = FragToken._new1901(t, ee, InstrumentKind.KEYWORD, InstrToken._m_directives_norm[ii])
                            title.children.append(ooo)
            ...
            if (dr0 is not None): 
                if (doc.typ is None or doc.typ == dr0.typ): 
                    ...
                    if (li is not None and len(li) > 0 and li[len(li) - 1].is_newline_after): 
                        ...
                        if (doc.typ == "КОДЕКС"): 
                            ...
                            if (pt is not None and len(pt.values) > 0): 
                                ...
                                title.children.append(FragToken._new1901(pt.begin_token, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
                                t = pt.end_token
                                continue
                    ...
                    t1 = t
                    title.children.append(FragToken._new1901(t, t, InstrumentKind.NAME, nam))
                    if (pt is not None and len(pt.values) > 0): 
                        title.children.append(FragToken._new1901(pt.begin_token, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
                        t1 = pt.end_token
                    t = t1
            ...
            if (doc.typ is None): 
                ...
                if (ttt1 is not None and ttt1.is_newline_after): 
                    ...
                    if (doc.typ is None): 
                        doc.typ = typ
                    title.children.append(FragToken._new1901(t, ttt1, InstrumentKind.TYP, typ))
                    dt0 = DecreeToken._new1046(t, ttt1, DecreeToken.ItemType.TYP, typ)
                    can_be_orgs = False
                    t = ttt1
                    t1 = t
                    continue
            ...
            if (dt is None and dt0 is not None and ((dt0.typ == DecreeToken.ItemType.OWNER or dt0.typ == DecreeToken.ItemType.ORG))): 
                if ((isinstance(t, NumberToken)) and t.is_newline_after and t.is_newline_before): 
                    dt = DecreeToken._new1046(t, t, DecreeToken.ItemType.NUMBER, str(t.value))
            ...
            if ((dt is None and (isinstance(t, NumberToken)) and t.is_newline_before) and t.is_newline_after): 
                if (dt0 is not None and dt0.typ == DecreeToken.ItemType.ORG and ((t.typ == NumberSpellingType.DIGIT))): 
                    dt = DecreeToken._new1046(t, t, DecreeToken.ItemType.NUMBER, str(t.value))
            if ((dt is not None and dt0 is not None and dt0.end_token.next0_ != dt.begin_token) and ((dt.typ == DecreeToken.ItemType.TYP or dt.typ == DecreeToken.ItemType.OWNER or dt.typ == DecreeToken.ItemType.ORG))): 
            ...
            if (dt is not None and dt.typ == DecreeToken.ItemType.DATE and dt0 is not None): 
                if (dt.is_newline_before or dt.is_newline_after): 
                    ...
                    pass
                elif (dt0.end_token.next0_ == dt.begin_token): 
                    pass
                else: 
                    dt = (None)
            if (dt is None): 
                ...
                if (t.is_value("ДАТА", None)): 
                    ...
                    while True:
                        ...
                        if (da is not None): 
                            frdt = FragToken._new1850(t, tt, InstrumentKind.DATE)
                ...
                if (((isinstance(r, AddressReferent)) or (isinstance(r, UriReferent)) or (isinstance(r, PhoneReferent))) or (isinstance(r, PersonIdentityReferent)) or (isinstance(r, BankDataReferent))): 
                    cnt = FragToken._new1850(t, t, InstrumentKind.CONTACT)
                    cnt.referents = list()
            ...
            if (dt.typ == DecreeToken.ItemType.ORG or dt.typ == DecreeToken.ItemType.OWNER): 
                ...
                while ttt is not None: 
                    if (ttt.whitespaces_before_count > 15): 
                        break
                    if (ttt.get_morph_class_in_dictionary().equals(MorphClass.VERB)): 
                        dt = (None)
                        break
                    ...
                    elif (not ttt.chars.equals(dt.begin_token.chars) and ttt.is_newline_before): 
                        break
                    else: 
                        dt.end_token = ttt
                    ttt = ttt.next0_
                if (dt is None): 
                    break
            if (dt.typ == DecreeToken.ItemType.TYP): 
                typ = DecreeToken.get_kind(dt.value, None)
        ...
        if (t is None or (((doc.typ is None and doc.reg_number is None and appr0 is None) and not start_of_name))): 
            if (t == t0): 
                nam = DecreeToken.try_attach_name(t0, None, True, False, False)
                if (nam is not None): 
                    ...
                    if (not Utils.isNullOrEmpty(name_)): 
                        ...
                        title.children.append(FragToken._new1901(t0, t1, InstrumentKind.NAME, name_.strip()))
                        ...
                        while True:
                            ...
                            break
                        return title
            if (t is not None and t.is_value("О", None)): 
                nam = DecreeToken.try_attach_name(t, None, True, False, False)
                if (nam is not None): 
                    name_ = FragToken._get_restored_name(t, nam.end_token, False)
                    if (not Utils.isNullOrEmpty(name_)): 
                        t1 = nam.end_token
                        doc.add_slot(InstrumentBlockReferent.ATTR_NAME, name_.strip(), True, 0)
                        title.children.append(FragToken._new1901(t, t1, InstrumentKind.NAME, name_.strip()))
            if (attrs > 0): 
                title.end_token = t1
                return title
            return None
        ...
        if (end_empty_lines is not None and doc.find_slot(InstrumentReferent.ATTR_SOURCE, None, True) is None): 
            ...
            title.children.insert(0, FragToken._new1901(t0, end_empty_lines, InstrumentKind.ORGANIZATION, val))
        is_case = False
        for ch in title.children: 
        ...
        if ((((name_ is not None or t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))) or ((not t.is_newline_before and len(title.children) > 0 and title.children[len(title.children) - 1].kind == InstrumentKind.TYP)))) and not is_case): 
            ...
            if (val is not None): 
                ...
                title.children.append(FragToken._new1901(tt0, t1, InstrumentKind.NAME, val.strip()))
                if ("КОДЕКС" in val): 
                    npt = NounPhraseHelper.try_parse(tt0, NounPhraseParseAttr.NO, 0, None)
        ...
        while True:
            ...
            if (t1.is_newline_before and (isinstance(t1.get_referent(), DecreeReferent)) and t1.is_newline_after): 
                dr = Utils.asObjectOrNull(t1.get_referent(), DecreeReferent)
                title.children.append(FragToken._new1850(t1, t1, InstrumentKind.IGNORED))
                continue
            if (t1.is_newline_before and t1.is_value("ЧАСТЬ", "ЧАСТИНА")): 
                ...
                if (pt is not None and pt.is_newline_after): 
                    ...
                    if (pt2 is not None and (((pt2.typ == PartToken.ItemType.SECTION or pt2.typ == PartToken.ItemType.SUBSECTION or pt2.typ == PartToken.ItemType.CHAPTER) or pt2.typ == PartToken.ItemType.CLAUSE))): 
                        ...
                        doc.add_slot(InstrumentReferent.ATTR_PART, pt.values[0].value, False, 0)
                        title.children.append(FragToken._new1901(t1, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
            ...
            if (t1.is_newline_before): 
                ...
                if (iii is not None and iii.typ == InstrToken1.Types.COMMENT): 
                    title.children.append(FragToken._new1850(t1, iii.end_token, InstrumentKind.COMMENT))
                    t1 = iii.end_token
                    continue
            ...
            if ((isinstance(t1.get_referent(), DecreeReferent)) and t1.get_referent().kind == DecreeKind.PUBLISHER and t1.is_newline_after): 
                pub = FragToken._new1850(t1, t1, InstrumentKind.APPROVED)
                pub.referents = list()
            ...
            if (tt.is_char_of("([") and tt.is_newline_before): 
                ...
                if (br is not None): 
                    ...
                    t1 = title.end_token
                    title.children.append(FragToken._new1850(br.begin_token, br.end_token, (InstrumentKind.NAME if tt.is_char('[') else InstrumentKind.COMMENT)))
                    continue
            if (tt.is_table_control_char): 
                title.end_token = tt
                continue
    ...
    def __create_appendix_title(t0 : 'Token', app : 'FragToken', doc : 'InstrumentReferent', is_app : bool, start : bool) -> 'FragToken':
        ...
        if (rr is not None): 
            if (rr.type_name == "PERSON"): 
                return None
        title = FragToken._new1850(t0, t, InstrumentKind.HEAD)
        ...
        while True:
            ...
            if (t.is_value("ОСОБЫЙ", "ОСОБЛИВИЙ") and t.next0_ is not None): 
                t2 = t.next0_
            if (isinstance(t, TextToken)): 
                title.children.append(FragToken._new1869(t, t2, InstrumentKind.KEYWORD, True))
            t = fr.end_token
            title.end_token = t
            if (fr.typ == InstrToken1.Types.APPENDIX and fr.num_begin_token is None): 
                ...
                if (fr1 is not None and fr1.typ == InstrToken1.Types.APPROVED): 
                    t = fr1.begin_token
                    title.children.append(FragToken._new1901(t, t, InstrumentKind.KEYWORD, t.get_source_text().upper()))
                    t = fr1.end_token
                    title.end_token = t
                    fr = fr1
            appr0 = FragToken.__create_approved(t)
            ...
            if (fr.num_begin_token is not None and fr.num_end_token is not None): 
                num = FragToken._new1901(fr.num_begin_token, fr.num_end_token, InstrumentKind.NUMBER, MiscHelper.get_text_value(fr.num_begin_token, fr.num_end_token, GetTextAttr.KEEPREGISTER))
                ...
                if (t.get_referent().kind == DecreeKind.PUBLISHER): 
                    ff = FragToken._new1850(t, t, InstrumentKind.APPROVED)
                    ff.referents = list()
                    ...
                    appr = FragToken._new1850(kw.begin_token, t, InstrumentKind.APPROVED)
                    del title.children[len(title.children) - 1]
                    appr.children.append(kw)
                    appr.children.append(FragToken._new1850(t, t, InstrumentKind.DOCREFERENCE))
                    title.children.append(appr)
                else: 
                    title.children.append(FragToken._new1850(t, t, InstrumentKind.DOCREFERENCE))
            elif (fr.typ == InstrToken1.Types.APPROVED and fr.length_char > 15 and fr.begin_token != fr.end_token): 
                title.children.append(FragToken._new1850(fr.begin_token.next0_, t, InstrumentKind.DOCREFERENCE))
            else: 
                ...
                if (dts is not None and len(dts) > 0 and dts[0].typ == DecreeToken.ItemType.TYP): 
                    dref = FragToken._new1850(dts[0].begin_token, dts[0].end_token, InstrumentKind.DOCREFERENCE)
                    i = 1
                    while i < len(dts): 
        ...
        if (t.is_value("К", "ДО")): 
            ...
            while tt is not None: 
                ...
                if (tok.typ == ILTypes.TYP and not tt.is_newline_after): 
                    nn = DecreeToken.try_attach_name(tt.next0_, None, False, True, False)
                    if (nn is not None): 
            ...
            if (max_ind >= 0): 
                ...
                if (not te.is_newline_after): 
                    nn = DecreeToken.try_attach_name(te.next0_, None, False, True, False)
                    if (nn is not None): 
                        te = nn.end_token
            elif (t.next0_ is not None and (isinstance(t.next0_.get_referent(), DecreeReferent))): 
                te = t.next0_
            if (te is not None): 
                dr = FragToken._new1850(t, te, InstrumentKind.DOCREFERENCE)
        ...
        while True:
            ...
            if (t.is_newline_before or t.previous.is_table_control_char): 
                ...
                if (ltt.typ == InstrToken1.Types.APPROVED): 
                    title.children.append(FragToken._new1850(ltt.begin_token, ltt.begin_token, InstrumentKind.APPROVED))
                    if (ltt.begin_token != ltt.end_token): 
                        title.children.append(FragToken._new1850(ltt.begin_token.next0_, ltt.end_token, InstrumentKind.DOCREFERENCE))
                    t = ltt.end_token
    ...
    def __add_title_attr(doc : 'InstrumentReferent', title : 'FragToken', dt : 'DecreeToken') -> bool:
        if (dt.typ == DecreeToken.ItemType.TYP): 
            ...
            if (title is not None): 
                title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.TYP, Utils.ifNotNull(dt.full_value, dt.value)))
            ...
            if (dt.is_delo): 
                if (doc is not None): 
                    ...
                    if (doc.reg_number == dt.value): 
                        doc.reg_number = None
                if (title is not None): 
                    title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.CASENUMBER, dt.value))
            else: 
                ...
                if (title is not None): 
                    title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.NUMBER, dt.value))
            ...
            if (title is not None): 
                title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.NAME, dt.value))
        elif (dt.typ == DecreeToken.ItemType.DATE): 
            if (doc is None or doc._add_date(dt)): 
                if (title is not None): 
                    title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.DATE, dt))
        elif (dt.typ == DecreeToken.ItemType.TERR): 
            if (title is not None): 
                title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, InstrumentKind.PLACE, dt))
            if (doc is not None and dt.ref is not None): 
                ...
                if (geo_ == "Россия"): 
                    ...
                    geo_ = (None)
                if (geo_ is None): 
                    ss = doc.add_slot(InstrumentReferent.ATTR_GEO, str(dt.ref.referent), False, 0)
                    ss.add_annotation(dt)
        elif (dt.typ == DecreeToken.ItemType.OWNER or dt.typ == DecreeToken.ItemType.ORG): 
            if (title is not None): 
                title.children.append(FragToken._new1901(dt.begin_token, dt.end_token, (InstrumentKind.ORGANIZATION if dt.typ == DecreeToken.ItemType.ORG else InstrumentKind.INITIATOR), dt))
            if (doc is not None): 
                if (dt.ref is not None): 
                    ss = doc.add_slot(InstrumentReferent.ATTR_SOURCE, dt.ref.referent, False, 0)
                    ss.tag = dt.get_source_text()
                    ss.add_annotation(dt)
                    if (isinstance(dt.ref.referent, PersonPropertyReferent)): 
                        doc.add_ext_referent(dt.ref)
                else: 
                    ss = doc.add_slot(InstrumentReferent.ATTR_SOURCE, MiscHelper.convert_first_char_upper_and_other_lower(dt.value), False, 0)
                    ss.tag = dt.get_source_text()
                    ss.add_annotation(dt)
        else: 
            return False
    ...
    def __createtztitle(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (tz is None): 
            return None
        title = FragToken._new1850(t0, tz.end_token, InstrumentKind.HEAD)
        t = t0
        first_pass3934 = True
        while True:
            ...
            if (dt is not None): 
                ...
                if (br is not None and BracketHelper.is_bracket(t.next0_, True)): 
                    nam = FragToken._new1872(br.begin_token, br.end_token, InstrumentKind.NAME, True)
                    title.children.append(nam)
                    t = br.end_token
                    title.end_token = t
                    continue
                if (t.next0_ is not None and t.next0_.is_value("НА", None)): 
                    ...
                    while True:
                        ...
                        if (npt is not None): 
                            tt = npt.end_token
                        t1 = tt
                    nam = FragToken._new1872(t.next0_, t1, InstrumentKind.NAME, True)
                    title.children.append(nam)
                    t = t1
                    title.end_token = t
                    continue
    ...
    @staticmethod
    def _new1850(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        return res
    
    @staticmethod
    def _new1851(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrToken1', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._itok = _arg3
    ...
    @staticmethod
    def _new1852(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val2 = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _new1859(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._itok = _arg3
        return res
    
    @staticmethod
    def _new1860(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._itok = _arg4
        return res
    ...
    @staticmethod
    def _new1867(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : int) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.number = _arg4
        return res
    ...
    @staticmethod
    def _new1869(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val2 = _arg4
        return res
    ...
    @staticmethod
    def _new1872(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val = _arg4
        return res
    ...
    @staticmethod
    def _new1884(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : object, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.value = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _new1889(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : int, _arg5 : bool, _arg6 : typing.List['Referent']) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.number = _arg4
    ...
    @staticmethod
    def _new1901(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : object) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new1907(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentReferent', _arg4 : 'InstrumentKind') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._m_doc = _arg3
        res.kind = _arg4
        return res
    ...
    @staticmethod
    def _new2073(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val = _arg4
        res._itok = _arg5
