...
class InstrToken1(MetaToken):
    ...
    def parse(t : 'Token', ignore_directives : bool, cur : 'FragToken'=None, lev : int=0, prev : 'InstrToken1'=None, is_citat : bool=False, max_char : int=0, can_be_table_cell : bool=False, is_in_index : bool=False) -> 'InstrToken1':
        ...
        if (t.is_char_of("({")): 
            ...
            if (fr is not None): 
                edt = InstrToken1._new2047(fr.begin_token, fr.end_token, InstrToken1.Types.EDITIONS)
            else: 
                t2 = InstrToken1._create_edition(t)
                if (t2 is not None): 
                    edt = InstrToken1._new2047(t, t2, InstrToken1.Types.EDITIONS)
            if (edt is not None): 
        ...
        t0 = t
        t00 = None
        res = InstrToken1._new2049(t0, t, True)
        while t is not None: 
                ...
                if (t.is_char(chr(0x1E))): 
                    ...
                    if (rows is not None and len(rows) > 0): 
                        ...
                        if (len(rows[0].cells) > 2 or len(rows[0].cells) == 0): 
                            ...
                            it11 = InstrToken1.parse(rows[0].begin_token, True, None, lev + 1, None, False, max_char, can_be_table_cell, False)
        ...
        if (dt is None and (((isinstance(t.get_referent(), PersonReferent)) or (isinstance(t.get_referent(), InstrumentParticipantReferent))))): 
            dt = DecreeToken._new1047(t, t, DecreeToken.ItemType.OWNER)
                ...
                while True:
                    ...
                    if (t.chars.is_letter): 
                        ...
                        if (lat is not None and not t.is_value("C", None) and not t.is_value("С", None)): 
                            ...
                            res.end_token = t
                            continue
                        if (t.length_char == 1 and t.chars.is_all_upper and (isinstance(t, TextToken))): 
                            res.num_begin_token = t
        ...
        if (t.is_value("КОНСУЛЬТАНТПЛЮС", None) or t.is_value("ГАРАНТ", None) or ((t.is_value("ИНФОРМАЦИЯ", None) and t.is_newline_before))): 
            ...
            if (t1 is not None and ((t1.is_value("ПРИМЕЧАНИЕ", None) or ok))): 
                if (t1.next0_ is not None and t1.next0_.is_char('.')): 
                    t1 = t1.next0_
                re = InstrToken1._new2047(t, t1, InstrToken1.Types.COMMENT)
                hiph = False
                t1 = t1.next0_
                first_pass3947 = True
                while True:
        ...
        if (check_comment > 0 or t.is_value("О", "ПРО")): 
            ...
            while ttt is not None: 
                ...
                ttt = ttt.next0_
            if (ok): 
                cmt = InstrToken1._new2047(t, t1, InstrToken1.Types.COMMENT)
        ...
        if (tt is not None): 
            ...
            if (dt1 is not None and dt1.typ == DecreeToken.ItemType.TYP): 
                ...
                return res
        val = None
        wrapval2058 = RefOutArgWrapper(None)
        tt2 = InstrToken1._check_directive(t, wrapval2058)
        val = wrapval2058.value
        if (tt2 is not None): 
            if (tt2.is_newline_after or ((tt2.next0_ is not None and ((tt2.next0_.is_char_of(":") or ((tt2.next0_.is_char('.') and tt2 != t)))) and ((tt2.next0_.is_newline_after or t.chars.is_all_upper))))): 
                return InstrToken1._new2053(t, (tt2 if tt2.is_newline_after else tt2.next0_), InstrToken1.Types.DIRECTIVE, val)
            ...
            if (tt is not None and tt.is_char_of(".)]")): 
                ...
                while tt is not None: 
                    ...
                    if (tt.is_newline_after): 
                        if (ok): 
                            return InstrToken1._new2047(t, tt, InstrToken1.Types.COMMENT)
                        break
                    tt = tt.next0_
        ...
        while True:
            ...
            if (t.is_newline_before and t != res.begin_token): 
                if (len(res.numbers) == 2): 
                    if (res.numbers[0] == "3" and res.numbers[1] == "4"): 
                        pass
                is_new_line = True
                if ((t.newlines_before_count == 1 and t.previous is not None and t.previous.chars.is_letter) and lev == 0): 
                    npt = NounPhraseHelper.try_parse(t.previous, NounPhraseParseAttr.NO, 0, None)
                    if (npt is not None and npt.end_char > t.begin_char): 
                        ...
                        if (npt is not None and npt.morph.check_accord(t.previous.morph, False, False)): 
                            is_new_line = False
                    if (not is_new_line): 
                        tes = InstrToken1.parse(t, True, cur, lev + 1, None, False, 0, False, False)
                ...
                if (is_new_line and t.chars.is_letter): 
                    if (not MiscHelper.can_be_start_of_sentence(t)): 
                        if (t.previous is not None and t.previous.is_char_of(":;.")): 
                            ...
                            vvv = InstrToken1.parse(t, True, cur, lev + 1, None, False, 0, False, False)
            ...
            if ((isinstance(t, TextToken)) and t.chars.is_letter): 
                ...
                if (t.chars.is_capital_upper and res.length_char > 20): 
                    if (t.is_value("РУКОВОДСТВУЯСЬ", None)): 
                        ...
                        ttt = InstrToken1.parse(t, True, None, lev + 1, None, False, 0, False, False)
            ...
            if (t != res.begin_token and InstrToken1.__is_first_line(t)): 
                break
            tmp = None
            wraptmp2055 = RefOutArgWrapper(None)
            tt2 = InstrToken1._check_directive(t, wraptmp2055)
            tmp = wraptmp2055.value
            if (tt2 is not None): 
        ...
        if (res.num_typ != NumberTypes.UNDEFINED and res.begin_token == res.num_begin_token and res.end_token == res.num_end_token): 
            ...
            if (prev is not None): 
                if (NumberingHelper.calc_delta(prev, res, True) == 1): 
                    ok = True
            if (not ok): 
                res1 = InstrToken1.parse(res.end_token.next0_, True, None, lev + 1, None, False, 0, False, False)
                if (res1 is not None): 
        ...
        if (res.end_token.is_char('>') and res.begin_token.is_value("ПУТЕВОДИТЕЛЬ", None)): 
            ...
            while True:
                ...
                if (not (ttt is not None)): break
                li2 = InstrToken1.parse(ttt, True, None, lev + 1, None, False, 0, False, False)
        ...
        if (res.typ == InstrToken1.Types.LINE): 
            if (res.num_typ != NumberTypes.UNDEFINED): 
                ...
                for nn in res.numbers: 
                    vv = 0
                    wrapvv2056 = RefOutArgWrapper(0)
                    inoutres2057 = Utils.tryParseInt(nn, wrapvv2056)
                    vv = wrapvv2056.value
                    if (inoutres2057): 
    ...
    @staticmethod
    def _new2047(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types') -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new2049(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.all_upper = _arg3
        return res
    
    @staticmethod
    def _new2053(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types', _arg4 : str) -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new2066(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : 'Types') -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
