...
class NumberingHelper:
        ...
        if ((len(prev.numbers) - 1) == len(next0_.numbers) and len(prev.numbers) > 1): 
            ...
            while i < (len(prev.numbers) - 2): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12067 = RefOutArgWrapper(0)
            inoutres2068 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 2], wrapn12067)
            n1 = wrapn12067.value
            if (not inoutres2068): 
        ...
        if ((len(prev.numbers) - 2) == len(next0_.numbers) and len(prev.numbers) > 2): 
            ...
            while i < (len(prev.numbers) - 3): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12069 = RefOutArgWrapper(0)
            inoutres2070 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 3], wrapn12069)
            n1 = wrapn12069.value
            if (not inoutres2070): 
                return 0
            if ((n1 + 1) != n2): 
        ...
        if ((len(prev.numbers) - 3) == len(next0_.numbers) and len(prev.numbers) > 3): 
            ...
            while i < (len(prev.numbers) - 4): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12071 = RefOutArgWrapper(0)
            inoutres2072 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 4], wrapn12071)
            n1 = wrapn12071.value
            if (not inoutres2072): 
                return 0
            if ((n1 + 1) != n2): 
    ...
    def create_number(owner : 'FragToken', itok : 'InstrToken1') -> None:
        ...
        if (itok.num_begin_token is None or itok.num_end_token is None): 
            return
        num = FragToken._new2073(itok.num_begin_token, itok.num_end_token, InstrumentKind.NUMBER, True, itok)
    ...
    def create_diap(s1 : str, s2 : str) -> typing.List[str]:
        ...
        if (s2.startswith(s1)): 
            ...
            if (((i + 1) < len(s2)) and s2[i] == '.' and str.isdigit(s2[i + 1])): 
                wrapn22074 = RefOutArgWrapper(0)
                inoutres2075 = Utils.tryParseInt(s2[i + 1:], wrapn22074)
                n2 = wrapn22074.value
                if (inoutres2075): 
                    res0 = list()
                    res0.append(s1)
        ...
        if (((i)) > 0): 
            pref = s1[0:0+i + 1]
            wrapn12078 = RefOutArgWrapper(0)
            inoutres2079 = Utils.tryParseInt(s1[i + 1:], wrapn12078)
            n1 = wrapn12078.value
            if (not inoutres2079): 
                return None
            if (not s2.startswith(pref)): 
                return None
            wrapn22076 = RefOutArgWrapper(0)
            inoutres2077 = Utils.tryParseInt(s2[i + 1:], wrapn22076)
            n2 = wrapn22076.value
            if (not inoutres2077): 
                return None
        else: 
            wrapn12082 = RefOutArgWrapper(0)
            inoutres2083 = Utils.tryParseInt(s1, wrapn12082)
            n1 = wrapn12082.value
            if (not inoutres2083): 
                return None
            wrapn22080 = RefOutArgWrapper(0)
            inoutres2081 = Utils.tryParseInt(s2, wrapn22080)
            n2 = wrapn22080.value
            if (not inoutres2081): 
                return None
        if (n2 <= n1): 
