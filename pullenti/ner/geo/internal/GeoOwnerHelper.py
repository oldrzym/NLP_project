    ...
    def _can_be_higher_token(rhi : 'Token', rlo : 'Token') -> bool:
        ...
        if (rhi.end_char <= rlo.begin_char): 
            ...
            if (rhi.is_newline_after or rlo.is_newline_before): 
                if (not citi_in_reg): 
                    if (hi.find_slot(GeoReferent.ATTR_NAME, "МОСКВА", True) is not None or hi.find_slot(GeoReferent.ATTR_NAME, "САНКТ-ПЕТЕРБУРГ", True) is not None): 
                        pass
                    else: 
                        return False
            ...
            if (lo.is_city): 
                ...
                if ("поселение" in hit or "поселок" in hit or "улус" in hit): 
                    if ("деревня" in lot and "коттеджный поселок" in hit): 
                        return False
                    if ((((("село;" in lot or "деревня" in lot or "хутор" in lot) or "аул" in lot or "кишлак" in lot) or "пункт" in lot or "слобода" in lot) or "станица" in lot or "починок" in lot) or "заимка" in lot or "местечко" in lot): 
                        return True
                    if (lot == "поселение;"): 
                        if ("поселок" in hit): 
                            return False
