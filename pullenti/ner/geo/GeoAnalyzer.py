...
class GeoAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while t is not None: 
            t.inner_bool = False
            t = t.next0_
        if (MiscLocationHelper.is_user_param_address(kit.first_token)): 
            t = kit.first_token
            while t is not None: 
                if (isinstance(t.get_referent(), DateReferent)): 
                    rt = Utils.asObjectOrNull(t, ReferentToken)
                    if ((rt.begin_token.chars.is_cyrillic_letter and rt.begin_token.next0_ is not None and rt.begin_token.next0_.is_comma) and (isinstance(rt.begin_token.next0_.next0_, NumberToken))): 
                        t = kit.debed_token(t)
                t = t.next0_
        sw = Stopwatch()
        sw.reset()
            ...
            while True:
                ...
                if (step > 0 and (isinstance(t, ReferentToken))): 
                    ...
                    if (((geo_ is not None and t.next0_ is not None and t.next0_.is_char('(')) and t.next0_.next0_ is not None and geo_.can_be_equals(t.next0_.next0_.get_referent(), ReferentsEqualType.WITHINONETEXT)) and t.next0_.next0_.next0_ is not None and t.next0_.next0_.next0_.is_char(')')): 
                        rt0 = ReferentToken._new916(geo_, t, t.next0_.next0_.next0_, t.morph)
                        kit.embed_token(rt0)
                        t = (rt0)
                        continue
                    if ((geo_ is not None and t.next0_ is not None and t.next0_.is_hiphen) and t.next0_.next0_ is not None and geo_.can_be_equals(t.next0_.next0_.get_referent(), ReferentsEqualType.WITHINONETEXT)): 
                        rt0 = ReferentToken._new916(geo_, t, t.next0_.next0_, t.morph)
                        kit.embed_token(rt0)
                        t = (rt0)
                        continue
                ok = False
                        ...
                        if (rt is None and cli[0].onto_item.referent.is_state): 
                            if ((rt is None and tt is not None and (isinstance(tt.get_referent(), GeoReferent))) and tt.whitespaces_before_count == 1): 
                                ...
                                if (GeoOwnerHelper.can_be_higher(Utils.asObjectOrNull(cli[0].onto_item.referent, GeoReferent), geo2, None, None)): 
                                    ...
                                    cl.occurrence.clear()
                                    rt = ReferentToken._new916(cl, cli[0].begin_token, cli[0].end_token, cli[0].morph)
                            if (rt is None and step == 0 and tt is not None): 
                ...
                if (rt is not None): 
                    ...
                    if ((not MiscLocationHelper.is_user_param_address(rt) and not geo_.is_city and not geo_.is_state) and geo_.find_slot(GeoReferent.ATTR_TYPE, "республика", True) is None): 
                        non_registered.append(geo_)
                    else: 
                        rt.referent = ad.register_referent(geo_)
                    tt2 = rt.begin_token.previous
                    if (tt2 is not None and tt2.is_comma): 
                        tt2 = tt2.previous
                    if (tt2 is not None and tt2.get_referent() == rt.referent): 
                        rt.begin_token = tt2
                        tt2 = rt.end_token.next0_
                        if (tt2 is not None and tt2.is_char(')')): 
                            if (rt.get_source_text().find('(') >= 0): 
                                rt.end_token = tt2
                    kit.embed_token(rt)
                    t = (rt)
                    if (step == 0): 
        ...
        while True:
            ...
            if (g is None): 
                continue
            if (t.next0_ is not None and t.next0_.is_char_of("(/\\") and (isinstance(t.next0_.next0_, ReferentToken))): 
                g2 = Utils.asObjectOrNull(t.next0_.next0_.get_referent(), GeoReferent)
                if (g2 is not None and g2 == g): 
                    rt2 = ReferentToken(g, t, t.next0_.next0_)
                    if (rt2.end_token.next0_ is not None and rt2.end_token.next0_.is_char_of(")/\\")): 
                        rt2.end_token = rt2.end_token.next0_
                    t.kit.embed_token(rt2)
                    t = (rt2)
        ...
        if (new_cities and len(ad.local_ontology.items) > 0): 
            ...
            while True:
                ...
                if (geo_ is not None): 
                    ...
                    rt = ReferentToken._new916(geo_, li[0].begin_token, li[0].end_token, t.morph)
        ...
        while True:
            ...
            while True:
                ...
                if (tt.is_value("IN", None) or tt.is_value("В", None)): 
                    ...
                    continue
                if (tt.is_hiphen): 
                    if (((tt.is_whitespace_before and tt.is_whitespace_after)) or comma1): 
                        continue
            ...
            if (tt.whitespaces_before_count > 15): 
                continue
            elif ((tt is not None and tt.next0_ is not None and tt.next0_.is_hiphen) and MiscLocationHelper.is_user_param_address(tt)): 
                sit = StreetItemToken._try_parse_spec(tt, None)
            ...
            while True:
                ...
                if (ttt.is_comma_and): 
                    ...
                    continue
                if (ttt.is_hiphen): 
                    if (((ttt.is_whitespace_before and ttt.is_whitespace_after)) or comma2): 
                        continue
            ...
            if (ttt is not None and ttt.whitespaces_before_count > 15): 
                geo2 = (None)
            elif ((ttt is not None and ttt.next0_ is not None and ttt.next0_.is_hiphen) and MiscLocationHelper.is_user_param_address(ttt)): 
                sit = StreetItemToken._try_parse_spec(ttt, None)
                if (sit is not None and sit[0].typ == StreetItemType.NAME): 
                    geo2 = (None)
            if (geo2 is not None): 
                ok2 = comma1 and comma2
                if (comma1 != comma2): 
                    if (geo_.is_region and geo1.is_region and geo2.is_city): 
                        ok2 = True
                if (ok2 and GeoOwnerHelper._can_be_higher_token(t, tt) and GeoOwnerHelper._can_be_higher_token(tt, ttt)): 
                    geo2.higher = geo1
                    geo1.higher = geo_
                    rt0 = ReferentToken._new916(geo1, t, tt, tt.morph)
                    kit.embed_token(rt0)
                    rt = ReferentToken._new916(geo2, rt0, ttt, ttt.morph)
                    kit.embed_token(rt)
                    t = (rt)
                    go_back = True
                    continue
                    ...
                    if (GeoOwnerHelper._can_be_higher_token(t, ttt)): 
                        if (geo2.find_slot(GeoReferent.ATTR_TYPE, "город", True) is not None and geo1.find_slot(GeoReferent.ATTR_TYPE, "район", True) is not None and geo_.is_region): 
                            ...
                            geo2.higher = geo_
                            geo1.higher = geo2
                            rt = ReferentToken._new916(geo1, t, ttt, tt.morph)
                            kit.embed_token(rt)
                            t = (rt)
                            go_back = True
                            continue
                    ...
                    if (GeoOwnerHelper._can_be_higher_token(tt, t)): 
                        geo_.higher = geo1
                        geo1.higher = geo2
                        rt0 = ReferentToken._new916(geo1, tt, ttt, tt.morph)
                        kit.embed_token(rt0)
                        rt = ReferentToken._new916(geo_, t, rt0, t.morph)
                        kit.embed_token(rt)
                        t = (rt)
                        go_back = True
                        continue
            ...
            if (GeoOwnerHelper._can_be_higher_token(t, tt) and ((not GeoOwnerHelper._can_be_higher_token(tt, t) or adj))): 
                geo1.higher = geo_
                rt = ReferentToken._new916(geo1, t, tt, tt.morph)
            ...
            if (GeoOwnerHelper._can_be_higher_token(tt, t) and ((not GeoOwnerHelper._can_be_higher_token(t, tt) or inp))): 
                ...
                if (bra and tt.next0_ is not None and tt.next0_.is_char(')')): 
                    tt = tt.next0_
                if ((bra and geo1.is_region and geo1.higher is None) and geo_.is_city and MiscLocationHelper.is_user_param_address(tt)): 
                    tt2 = tt.next0_
                    ...
                    if (isinstance(tt2, TextToken)): 
                        ter = TerrItemToken.try_parse(tt2, None, None)
                        if (ter is not None and ter.onto_item is None and ter.termin_item is None): 
                            ...
                            if (tt2 is not None and (isinstance(tt2.get_referent(), GeoReferent))): 
                                if (GeoOwnerHelper.can_be_higher(Utils.asObjectOrNull(tt2.get_referent(), GeoReferent), geo1, None, None)): 
                                    geo1.higher = Utils.asObjectOrNull(tt2.get_referent(), GeoReferent)
                                    tt = tt2
                rt = ReferentToken._new916(geo_, t, tt, t.morph)
                kit.embed_token(rt)
                t = (rt)
                go_back = True
                continue
    ...
    def __try_attach_territory_after_city(self, t : 'Token', ad : 'AnalyzerDataWithOntology') -> 'ReferentToken':
        ...
        if (ait is None): 
            return None
        if (ait.typ == AddressItemType.STREET and ait.ref_token is None): 
            reg = GeoReferent()
        ...
        if (ait.typ == AddressItemType.REGION and (isinstance(ait.referent, GeoReferent))): 
            reg = GeoReferent()
            reg._add_typ("район")
            reg._add_name(MiscHelper.get_text_value(t0, t1, GetTextAttr.NO))
            reg.higher = Utils.asObjectOrNull(ait.referent, GeoReferent)
            return ReferentToken(reg, t0, t1)
        return None
    ...
    def process_referent(self, begin : 'Token', param : str) -> 'ReferentToken':
        ...
        if (toks is not None): 
            for tok in toks: 
                ...
                res1 = ReferentToken._new1780(city, tok.begin_token, tok.end_token, tok.morph, begin.kit.get_analyzer_data(self))
                break
        if (not begin.morph.class0_.is_adjective): 
            ...
            if ((te.chars.is_all_upper and te.chars.is_cyrillic_letter and te.length_char == 2) and te.get_morph_class_in_dictionary().is_undefined): 
                ...
                if (cou == 1 and res1 is None): 
                    res1 = ReferentToken._new1781(geo0, begin, begin, ad)
            ad.level += 1
                ...
                while True:
                    ...
                    for g in geos: 
                        ...
                        if (gg is not None): 
                            res = None
                            if (gg.is_city): 
                                res = ReferentToken._new1780(gg, begin, ctoks[0].end_token, ctoks[0].morph, ad)
                            if (gg.higher is not None and gg.higher.is_city): 
                                res = ReferentToken._new1780(gg.higher, begin, ctoks[0].end_token, ctoks[0].morph, ad)
        ...
        if (tt is None or tt.onto_item is None): 
            ...
            if ((tok is not None and tok[0].item is not None and (isinstance(tok[0].item.referent, GeoReferent))) and tok[0].item.referent.is_state): 
                tt = TerrItemToken._new1668(tok[0].begin_token, tok[0].end_token, tok[0].item)
        if (tt is None): 
            return res1
