...
class MeasureReferent(Referent):
    ...
    def double_values(self) -> typing.List[float]:
        ...
        for s in self.slots: 
            if (s.type_name == MeasureReferent.ATTR_VALUE and (isinstance(s.value, str))): 
                d = 0
                wrapd2275 = RefOutArgWrapper(0)
                inoutres2276 = MeasureHelper.try_parse_double(Utils.asObjectOrNull(s.value, str), wrapd2275)
                d = wrapd2275.value
                if (inoutres2276): 
                    res.append(d)
        return res
        ...
        for s in self.slots: 
            if (s.type_name == MeasureReferent.ATTR_VALUE): 
                if (isinstance(s.value, str)): 
                    ...
                    vals.append(s.value.to_string_ex(True, lang, 0))
    ...
    def out_units(self, lang : 'MorphLang'=None) -> str:
        ...
        if (len(uu) == 0): 
            return ""
        res = io.StringIO()
        print(uu[0].to_string_ex(True, lang, 0), end="", file=res)
        i = 1
        while i < len(uu): 
            pow0_ = uu[i].get_string_value(UnitReferent.ATTR_POW)
            if (not Utils.isNullOrEmpty(pow0_) and pow0_[0] == '-'): 
                print("/{0}".format(uu[i].to_string_ex(True, lang, 1)), end="", file=res, flush=True)
                if (pow0_ != "-1"): 
                    print("<{0}>".format(pow0_[1:]), end="", file=res, flush=True)
            else: 
                print("*{0}".format(uu[i].to_string_ex(True, lang, 0)), end="", file=res, flush=True)
            i += 1
        return Utils.toStringStringIO(res)
