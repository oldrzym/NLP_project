...
class NumbersWithUnitToken(MetaToken):
    ...
    def to_string_ex(self, ignore_units : bool=False) -> str:
        res = io.StringIO()
    ...
    @staticmethod
    def try_parse_multi(t : 'Token', add_units : 'TerminCollection', attrs : 'NumberWithUnitParseAttr'=NumberWithUnitParseAttr.NO) -> typing.List['NumbersWithUnitToken']:
        ...
        if (tt0.is_char('(')): 
            ...
            if (whd is not None): 
                tt0 = whd.end_token
            res0 = NumbersWithUnitToken.try_parse_multi(tt0.next0_, add_units, attrs)
            if (res0 is not None): 
                res0[0].whl = whd
                ...
                if (tt2 is not None and tt2.is_char(')')): 
                    ...
                    return res0
        mt = NumbersWithUnitToken.try_parse(t, add_units, attrs)
        if (mt is None): 
            return None
        res = list()
        nnn = None
        ...
        if (nnn is not None): 
            mt2 = NumbersWithUnitToken.try_parse(nnn, add_units, NumberWithUnitParseAttr.NO)
            if (mt2 is not None): 
                mt3 = None
                nnn = (None)
                ...
                if (nnn is not None): 
                    mt3 = NumbersWithUnitToken.try_parse(nnn, add_units, NumberWithUnitParseAttr.NO)
                if (mt3 is None): 
        ...
        if ((not mt.is_whitespace_after and MeasureHelper.is_mult_char_end(mt.end_token.next0_) and (isinstance(mt.end_token.next0_.next0_, NumberToken))) and len(mt.units) == 0): 
            ...
            if (terms is not None and len(terms) > 0): 
                mt.units.append(UnitToken._new2137(mt.end_token.next0_, mt.end_token.next0_, Utils.asObjectOrNull(terms[0].tag, Unit)))
                mt.end_token = mt.end_token.next0_
                res1 = NumbersWithUnitToken.try_parse_multi(mt.end_token.next0_, add_units, NumberWithUnitParseAttr.NO)
                if (res1 is not None): 
                    res1.insert(0, mt)
                    return res1
        res.append(mt)
    ...
    @staticmethod
    def try_parse(t : 'Token', add_units : 'TerminCollection', attrs : 'NumberWithUnitParseAttr'=NumberWithUnitParseAttr.NO) -> 'NumbersWithUnitToken':
        if (t is None): 
            return None
        res = NumbersWithUnitToken._try_parse(t, add_units, attrs)
        if (res is not None and (((attrs) & (NumberWithUnitParseAttr.NOT))) == (NumberWithUnitParseAttr.NOT)): 
            res.not0_ = True
        return res
    ...
    @staticmethod
    def _try_parse(t : 'Token', add_units : 'TerminCollection', attrs : 'NumberWithUnitParseAttr') -> 'NumbersWithUnitToken':
        if (t is None): 
            return None
        while t is not None:
        ...
        is_diap_keyw = False
        min_max = 0
        wrapmin_max2145 = RefOutArgWrapper(min_max)
        ttt = NumbersWithUnitToken._is_min_or_max(t, wrapmin_max2145)
        min_max = wrapmin_max2145.value
        if (ttt is not None): 
            t = ttt.next0_
            if (t is None): 
                return None
        ...
        if (t.is_value("В", None) and t.next0_ is not None): 
            if (t.next0_.is_value("ПРЕДЕЛ", None) or t.next0_.is_value("ДИАПАЗОН", None) or t.next0_.is_value("ТЕЧЕНИЕ", None)): 
                t = t.next0_.next0_
                if (t is None): 
                    return None
                is_diap_keyw = True
        if (t0.is_char('(')): 
            mt0 = NumbersWithUnitToken._try_parse(t.next0_, add_units, NumberWithUnitParseAttr.NO)
            if (mt0 is not None and mt0.end_token.next0_ is not None and mt0.end_token.next0_.is_char(')')): 
                if ((((attrs) & (NumberWithUnitParseAttr.ISSECOND))) != (NumberWithUnitParseAttr.NO)): 
        ...
        if (tok is not None): 
            if (t.is_value("С", None)): 
                npt = NounPhraseHelper.try_parse(t.next0_, NounPhraseParseAttr.PARSENUMERICASADJECTIVE, 0, None)
                ...
                if (uni is not None): 
                    ...
                    while t is not None:
                        if (t.is_char_of("):")): 
                            ...
                            break
                    mt0 = NumbersWithUnitToken._try_parse(t, add_units, Utils.valToEnum((attrs) & (NumberWithUnitParseAttr.CANOMITNUMBER), NumberWithUnitParseAttr))
            ...
            dty = NumbersWithUnitToken.DiapTyp.GE
            t = t.next0_
        while t is not None and (((t.is_char_of(":,") or t.is_value("ЧЕМ", None) or ((t.get_morph_class_in_dictionary().is_preposition and t != t0))) or t.is_table_control_char)):
            t = t.next0_
        second = (((attrs) & (NumberWithUnitParseAttr.ISSECOND))) != (NumberWithUnitParseAttr.NO)
        if (t is not None): 
        ...
        if (num is None): 
            ...
            if (uni is not None): 
                ...
                if (not delim): 
                    if (t is None): 
                        if (has_keyw and (((attrs) & (((NumberWithUnitParseAttr.CANBENON) | (NumberWithUnitParseAttr.CANOMITNUMBER))))) != (NumberWithUnitParseAttr.NO)): 
                            pass
                        else: 
                            return None
                    elif (not t.is_whitespace_before): 
                        return None
                    if ((t is not None and t.next0_ is not None and t.is_hiphen) and t.is_whitespace_after): 
                        delim = True
                        t = t.next0_
                num = NumberHelper.try_parse_real_number(t, NumberParseAttr.NO)
                if (num is not None and num.is_newline_before): 
                    return None
        res = None
        rval = 0
        if (num is None): 
            ...
            if (tt is not None): 
                ...
                for u in UnitsHelper.UNITS: 
                    if (u.fullname_cyr == unam): 
                        uni = list()
                        uni.append(UnitToken._new2137(t, t, u))
                        break
                if (uni is None): 
                    return None
                res = NumbersWithUnitToken._new2139(t0, tt.end_token, about_)
                t = tt.end_token.next0_
            else: 
                if ((((attrs) & (NumberWithUnitParseAttr.CANOMITNUMBER))) == (NumberWithUnitParseAttr.NO) and not has_keyw and (((attrs) & (NumberWithUnitParseAttr.CANBENON))) == (NumberWithUnitParseAttr.NO)): 
                    return None
                if ((uni is not None and len(uni) == 1 and uni[0].begin_token == uni[0].end_token) and uni[0].length_char > 3): 
                    rval = (1)
                    res = NumbersWithUnitToken._new2139(t0, uni[len(uni) - 1].end_token, about_)
                    t = res.end_token.next0_
                elif (has_keyw and (((attrs) & (NumberWithUnitParseAttr.CANBENON))) != (NumberWithUnitParseAttr.NO)): 
                    rval = math.nan
                    res = NumbersWithUnitToken._new2139(t0, t0, about_)
                    if (t is not None): 
            ...
            if ((t == t0 and t0.is_hiphen and not t.is_whitespace_before) and not t.is_whitespace_after and (num.real_value < 0)): 
                num = NumberHelper.try_parse_real_number(t.next0_, NumberParseAttr.NO)
                if (num is None): 
                    return None
                ...
                if ((norm.endswith("Ь") or norm == "ЧЕТЫРЕ" or norm == "ТРИ") or norm == "ДВА"): 
                    ...
                    try: 
                        ...
                        if (mi.class0_.is_adjective): 
                            return None
                    except Exception as ex2142: 
                        pass
            t = num.end_token.next0_
            res = NumbersWithUnitToken._new2139(t0, num.end_token, about_)
            rval = num.real_value
            if (res.end_token.next0_ is not None and res.end_token.next0_.is_char('(') and (isinstance(res.end_token.next0_.next0_, NumberToken))): 
                nn = Utils.asObjectOrNull(res.end_token.next0_.next0_, NumberToken)
                if (nn.value == num.value and nn.end_token.next0_ is not None and nn.end_token.next0_.is_char(')')): 
                    res.end_token = nn.end_token.next0_
            if (t is not None and t.is_value("И", None) and t.next0_ is not None): 
                if (t.next0_.is_value("БОЛЕЕ", None) or t.next0_.is_value("БОЛЬШЕ", None)): 
                    dty = NumbersWithUnitToken.DiapTyp.GE
                    res.end_token = t.next0_
                    t = res.end_token.next0_
                elif (t.next0_.is_value("МЕНЕЕ", None) or t.next0_.is_value("МЕНЬШЕ", None)): 
                    dty = NumbersWithUnitToken.DiapTyp.LE
                    res.end_token = t.next0_
                    t = res.end_token.next0_
        if (uni is None): 
            ...
            if (uni is not None): 
                if ((plusminus and second and len(uni) >= 1) and uni[0].unit == UnitsHelper.UPERCENT): 
                    ...
                    res.end_token = uni[len(uni) - 1].end_token
                t = res.end_token.next0_
                if ((t is not None and t.is_value("И", None) and t.next0_ is not None) and dty == NumbersWithUnitToken.DiapTyp.UNDEFINED): 
                    if (NumbersWithUnitToken._try_parse(t.next0_, add_units, attrs) is None): 
                        if (t.next0_.is_value("БОЛЕЕ", None) or t.next0_.is_value("БОЛЬШЕ", None)): 
                            dty = NumbersWithUnitToken.DiapTyp.GE
                            res.end_token = t.next0_
                            t = res.end_token.next0_
            ...
            if (len(uni) > 1): 
                ...
                if (((uni1 is not None and uni1[0].unit == uni[0].unit and (len(uni1) < len(uni))) and uni[len(uni1)].pow0_ == -1 and uni1[len(uni1) - 1].end_token.next0_ is not None) and uni1[len(uni1) - 1].end_token.next0_.is_char_of("/\\")): 
                    num2 = NumbersWithUnitToken._try_parse(uni1[len(uni1) - 1].end_token.next0_.next0_, add_units, NumberWithUnitParseAttr.NO)
        ...
        if (not second): 
            iii = 0
            wrapiii2144 = RefOutArgWrapper(iii)
            ttt = NumbersWithUnitToken._is_min_or_max(t, wrapiii2144)
            iii = wrapiii2144.value
            if (ttt is not None and iii > 0): 
                is_second_max = True
                t = ttt.next0_
        next0__ = None
        if (second or plusminus or ((t is not None and ((t.is_table_control_char or t.is_newline_before))))): 
            pass
        else: 
            a = NumberWithUnitParseAttr.ISSECOND
            if ((((attrs) & (NumberWithUnitParseAttr.CANBENON))) != (NumberWithUnitParseAttr.NO)): 
                a = (Utils.valToEnum((a) | (NumberWithUnitParseAttr.CANBENON), NumberWithUnitParseAttr))
            next0__ = NumbersWithUnitToken._try_parse(t, add_units, a)
        ...
        if (next0__ is not None and ((next0__.to_val is not None or next0__.single_val is not None)) and next0__.from_val is None): 
            if ((((next0__.begin_token.is_char('+') and next0__.single_val is not None and not math.isnan(next0__.single_val)) and next0__.end_token.next0_ is not None and next0__.end_token.next0_.is_char_of("\\/")) and next0__.end_token.next0_.next0_ is not None and next0__.end_token.next0_.next0_.is_hiphen) and not has_keyw and not math.isnan(rval)): 
                next2 = NumbersWithUnitToken._try_parse(next0__.end_token.next0_.next0_.next0_, add_units, NumberWithUnitParseAttr.ISSECOND)
    ...
    def _try_parsewhl(t : 'Token') -> 'MetaToken':
        ...
        if (len(txt) == 5 and ((txt[1] == 'Х' or txt[1] == 'X')) and ((txt[3] == 'Х' or txt[3] == 'X'))): 
            ...
            for i in range(3):
                ...
                if (ch == 'Г'): 
                    ...
                    return None
            return MetaToken._new630(t, t, nams)
        t0 = t
        t1 = t
        while t is not None: 
        ...
        if (nams is None or (len(nams) < 2)): 
            return None
        return MetaToken._new630(t0, t1, nams)
    ...
    def _initialize() -> None:
        ...
        lss = ["МЕНЬШЕ", "МЕНЕ", "КОРОЧЕ", "МЕДЛЕННЕЕ", "НИЖЕ", "МЛАДШЕ", "ДЕШЕВЛЕ", "РЕЖЕ", "РАНЬШЕ", "РАНЕЕ"]
        gts = ["БОЛЬШЕ", "ДЛИННЕЕ", "БЫСТРЕЕ", "БОЛЕ", "ЧАЩЕ", "ГЛУБЖЕ", "ВЫШЕ", "СВЫШЕ", "СТАРШЕ", "ДОЛЬШЕ", "ПОЗДНЕЕ", "ПОЗЖЕ", "ДОРОЖЕ", "ПРЕВЫШАТЬ"]
        t = Termin._new92("МЕНЕЕ", NumbersWithUnitToken.DiapTyp.LS)
        for s in lss: 
            t.add_variant(s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new92("НЕ МЕНЕЕ", NumbersWithUnitToken.DiapTyp.GE)
        for s in lss: 
            t.add_variant("НЕ " + s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new92("БОЛЕЕ", NumbersWithUnitToken.DiapTyp.GT)
        for s in gts: 
            t.add_variant(s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new92("НЕ БОЛЕЕ", NumbersWithUnitToken.DiapTyp.LE)
        for s in gts: 
            t.add_variant("НЕ " + s, False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new92("ОТ", NumbersWithUnitToken.DiapTyp.FROM)
        t.add_variant("С", False)
        t.add_variant("C", False)
        t.add_variant("НАЧИНАЯ С", False)
        t.add_variant("НАЧИНАЯ ОТ", False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new92("ДО", NumbersWithUnitToken.DiapTyp.TO)
        t.add_variant("ПО", False)
        t.add_variant("ЗАКАНЧИВАЯ", False)
        NumbersWithUnitToken.M_TERMINS.add(t)
        t = Termin._new92("НЕ ХУЖЕ", NumbersWithUnitToken.DiapTyp.UNDEFINED)
        NumbersWithUnitToken.M_TERMINS.add(t)
        NumbersWithUnitToken.M_SPEC = TerminCollection()
        t = Termin._new94("ПОЛЛИТРА", 0.5, "литр")
        t.add_variant("ПОЛУЛИТРА", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        t = Termin._new94("ПОЛКИЛО", 0.5, "килограмм")
        t.add_variant("ПОЛКИЛОГРАММА", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        t = Termin._new94("ПОЛМЕТРА", 0.5, "метр")
        t.add_variant("ПОЛУМЕТРА", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        t = Termin._new94("ПОЛТОННЫ", 0.5, "тонна")
        t.add_variant("ПОЛУТОННЫ", False)
        NumbersWithUnitToken.M_SPEC.add(t)
        NumbersWithUnitToken.M_SPEC.add(t)
    
    @staticmethod
    def _new2130(_arg1 : 'Token', _arg2 : 'Token', _arg3 : float) -> 'NumbersWithUnitToken':
        res = NumbersWithUnitToken(_arg1, _arg2)
        res.single_val = _arg3
        return res
    
    @staticmethod
    def _new2139(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NumbersWithUnitToken':
        res = NumbersWithUnitToken(_arg1, _arg2)
        res.about = _arg3
        return res