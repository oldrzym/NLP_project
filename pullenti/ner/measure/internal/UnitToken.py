...
class UnitToken(MetaToken):
    ...
    def try_parse_list(t : 'Token', add_units : 'TerminCollection', parse_unknown_units : bool=False) -> typing.List['UnitToken']:
        ...
        while tt is not None: 
            ...
            if (ut is None): 
                if (tt.is_char_of("\\/") and tt.next0_ is not None and tt.next0_.is_char('(')): 
                    li = UnitToken.try_parse_list(tt.next0_.next0_, add_units, parse_unknown_units)
                    if (li is not None and li[len(li) - 1].end_token.next0_ is not None and li[len(li) - 1].end_token.next0_.is_char(')')): 
                        for v in li: 
                            v.pow0_ = (- v.pow0_)
                            res.append(v)
                        li[len(li) - 1].end_token = li[len(li) - 1].end_token.next0_
                if (tt.is_char_of("\\/") or tt.is_hiphen): 
                    if (not tt.is_whitespace_after and not tt.is_whitespace_before and len(res) == 1): 
                        if (res[0].is_doubt or res[0].length_char == 1 or res[0].unit is None): 
                            if (not (isinstance(tt.next0_, NumberToken))): 
                                return None
                break
            if (ut.unit is None and ut.is_whitespace_before): 
                break
        ...
        for i in range(len(res) - 1, 0, -1):
            r = res[i]
            if ((r.is_doubt and r.begin_token == r.end_token and r.unit is None) and not r.begin_token.get_morph_class_in_dictionary().is_undefined): 
                del res[i]
            else: 
                break
        if (len(res) > 1): 
    ...
    def try_parse(t : 'Token', add_units : 'TerminCollection', prev : 'UnitToken', parse_unknown_units : bool=False) -> 'UnitToken':
        ...
        if (toks is not None): 
            if ((prev is not None and tt == t0 and len(toks) == 1) and t.is_whitespace_before): 
                return None
            if ((toks[0].length_char == 1 and tt.chars.is_all_upper and not toks[0].is_whitespace_after) and (isinstance(toks[0].end_token.next0_, NumberToken))): 
                return None
            if (tt.is_value("BA", None) and tt.kit.base_language.is_en): 
                return None
            if (toks[0].begin_token == toks[0].end_token and tt.morph.class0_.is_preposition and (tt.whitespaces_after_count < 3)): 
                ...
                if (nex is not None): 
                    return None
            if (toks[0].begin_token.length_char == 1 and toks[0].begin_token.chars.is_all_lower): 
                if (toks[0].begin_token == toks[0].end_token or ((toks[0].end_token.is_char('.') and toks[0].begin_token.next0_ == toks[0].end_token))): 
                    if (toks[0].begin_token.is_value("М", None) or toks[0].begin_token.is_value("M", None)): 
                        if (prev is not None and prev.unit is not None and prev.unit.kind == MeasureKind.LENGTH): 
                            res = UnitToken._new2137(t0, toks[0].end_token, UnitsHelper.UMINUTE)
                            res.pow0_ = pow0__
                            if (is_neg): 
                                res.pow0_ = (- pow0__)
                            res.__correct()
                            return res
                    if (toks[0].begin_token.is_value("S", None) or toks[0].begin_token.is_value("С", None)): 
                        if (prev is not None and prev.unit is not None and ((prev.unit.kind == MeasureKind.LENGTH or is_neg))): 
                            res = UnitToken._new2137(t0, toks[0].end_token, UnitsHelper.USEC)
                            res.pow0_ = pow0__
                            if (is_neg): 
                                res.pow0_ = (- pow0__)
                            res.__correct()
            ...
            uts = list()
            for tok in toks: 
                res = UnitToken._new2137(t0, tok.end_token, Utils.asObjectOrNull(tok.termin.tag, Unit))
                res.pow0_ = pow0__
                if (is_neg): 
                    res.pow0_ = (- pow0__)
        ...
        if (t.is_char_of("º°")): 
            t1 = t
        elif ((t.is_char('<') and t.next0_ is not None and t.next0_.next0_ is not None) and t.next0_.next0_.is_char('>') and (((t.next0_.is_value("О", None) or t.next0_.is_value("O", None) or t.next0_.is_char_of("º°")) or (((isinstance(t.next0_, NumberToken)) and t.next0_.value == "0"))))): 
            t1 = t.next0_.next0_
        if (t1 is not None): 
            res = UnitToken._new2137(t0, t1, UnitsHelper.UGRADUS)
            res.__check_doubt()
            t = t1.next0_
        ...
        if ((isinstance(t, TextToken)) and ((t.is_value("ОС", None) or t.is_value("OC", None)))): 
            ...
            if (str0_ == "оС" or str0_ == "oC"): 
                res = UnitToken._new2269(t, t, UnitsHelper.UGRADUSC, False)
                return res
        if (t.is_char('%')): 
            tt1 = t.next0_
            if (tt1 is not None and tt1.is_char('(')): 
                tt1 = tt1.next0_
            if ((isinstance(tt1, TextToken)) and ((tt1.is_value("ОБ", None) or tt1.is_value("ОБОРОТ", None)))): 
                re = UnitToken._new2137(t, tt1, UnitsHelper.UALCO)
                ...
                if (re.end_token.next0_ is not None and re.end_token.next0_.is_char(')') and t.next0_.is_char('(')): 
                    re.end_token = re.end_token.next0_
                return re
            return UnitToken._new2137(t, t, UnitsHelper.UPERCENT)
        if (add_units is not None): 
            tok = add_units.try_parse(t, TerminParseAttr.NO)
            if (tok is not None): 
                res = UnitToken._new2272(t0, tok.end_token, Utils.asObjectOrNull(tok.termin.tag, UnitReferent))
        ...
        if (mc.is_undefined): 
            ...
            return None
        res1 = UnitToken._new2273(t0, t1, pow0__, True)
        if (is_neg): 
            res1.pow0_ = (- pow0__)
        res1.unknown_name = t.get_source_text()
        if (prev is not None and res1.unknown_name == "d"): 
    ...
    def __check_doubt(self) -> None:
        ...
        if (self.pow0_ != 1): 
            return
        if ((self.begin_token.length_char < 3) or self.begin_token.is_value("ARE", None)): 
            self.is_doubt = True
            if ((self.begin_token.chars.is_capital_upper or self.begin_token.chars.is_all_upper or self.begin_token.chars.is_last_lower) or self.begin_token.chars.is_all_lower): 
                if (self.begin_token.length_char == 2 and (isinstance(self.begin_token.previous, NumberToken)) and (self.begin_token.whitespaces_before_count < 2)): 
                    self.is_doubt = False
        ...
        first_pass3991 = True
        while True:
            if first_pass3991: first_pass3991 = False
    ...
    @staticmethod
    def _new2137(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Unit') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.unit = _arg3
        return res
    
    @staticmethod
    def _new2269(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Unit', _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.unit = _arg3
        res.is_doubt = _arg4
        return res
    ...
    @staticmethod
    def _new2272(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'UnitReferent') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.ext_onto = _arg3
        return res
    
    @staticmethod
    def _new2273(_arg1 : 'Token', _arg2 : 'Token', _arg3 : int, _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.pow0_ = _arg3
        res.is_doubt = _arg4
        return res