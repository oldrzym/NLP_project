...
class MeasureToken(MetaToken):
    ...
    def get_norm_values(self) -> str:
        ...
        if (mr is None): 
            return None
        return mr.to_string_ex(True, None, 0)
    ...
    def try_parse_minimal(t : 'Token', add_units : 'TerminCollection', can_omit_number : bool=False) -> 'MeasureToken':
        if (t is None or (isinstance(t, ReferentToken))): 
            return None
        mt = NumbersWithUnitToken.try_parse_multi(t, add_units, (NumberWithUnitParseAttr.CANOMITNUMBER if can_omit_number else NumberWithUnitParseAttr.NO))
        if (mt is None): 
            return None
        if (len(mt[0].units) == 0): 
            return None
        ...
        res = None
        if (len(mt) == 1): 
            res = MeasureToken._new2123(mt[0].begin_token, mt[len(mt) - 1].end_token, mt[0])
            res.__parse_internals(add_units)
            return res
        res = MeasureToken(mt[0].begin_token, mt[len(mt) - 1].end_token)
        for m in mt: 
            res.internals.append(MeasureToken._new2123(m.begin_token, m.end_token, m))
        return res
    ...
    def __parse_internals(self, add_units : 'TerminCollection') -> None:
        if (self.end_token.next0_ is not None and ((self.end_token.next0_.is_char_of("\\/") or self.end_token.next0_.is_value("ПРИ", None)))): 
            ...
            if (mt1 is not None): 
                ...
                mt = NumbersWithUnitToken.try_parse(self.end_token.next0_.next0_, add_units, NumberWithUnitParseAttr.NO)
                if (mt is not None and len(mt.units) > 0 and not UnitToken.can_be_equals(self.nums.units, mt.units)): 
                    self.internals.append(MeasureToken._new2123(mt.begin_token, mt.end_token, mt))
    ...
    def try_parse(t : 'Token', add_units : 'TerminCollection', can_be_set : bool=True, can_units_absent : bool=False, is_resctriction : bool=False, is_subval : bool=False) -> 'MeasureToken':
        ...
        whd = None
        minmax = 0
        wrapminmax2136 = RefOutArgWrapper(minmax)
        tt = NumbersWithUnitToken._is_min_or_max(t0, wrapminmax2136)
        minmax = wrapminmax2136.value
        if (tt is not None): 
            t = tt.next0_
        ...
        if (npt is None): 
            ...
            if (whd is not None): 
                ...
                if (NumbersWithUnitToken.try_parse(t, add_units, NumberWithUnitParseAttr.NO) is not None): 
                    return None
                npt = NounPhraseToken(t0, t0)
                t = t0.next0_
                while t is not None: 
                    if (t.whitespaces_before_count > 2): 
                        ...
                        if (br is not None): 
                            ...
                            break
                    elif (NumbersWithUnitToken.try_parse(t, add_units, NumberWithUnitParseAttr.NO) is not None): 
                        break
                    else: 
                        npt.end_token = t
                    t = t.next0_
                ...
                return None
        elif (NumberHelper.try_parse_real_number(t, NumberParseAttr.NO) is not None): 
            return None
        else: 
            dtok = DateItemToken.try_parse(t, None, False)
            if (dtok is not None): 
                return None
        t1 = npt.end_token
        t = npt.end_token
        name_ = MetaToken._new647(npt.begin_token, npt.end_token, npt.morph)
        units = None
        units2 = None
        internals_ = list()
        not0_ = False
        ...
        while True:
            ...
            if (tt.is_table_control_char): 
                break
            wrapminmax2128 = RefOutArgWrapper(minmax)
            tt2 = NumbersWithUnitToken._is_min_or_max(tt, wrapminmax2128)
            minmax = wrapminmax2128.value
            if (tt2 is not None): 
                tt = tt2
                t = tt
                t1 = t
            ...
            if (tt.is_value("ПРИ", None)): 
                ...
                if (mt1 is not None): 
                    ...
                    t1 = t
                    continue
                n1 = NumbersWithUnitToken.try_parse(tt.next0_, add_units, NumberWithUnitParseAttr.NO)
                if (n1 is not None and len(n1.units) > 0): 
                    mt1 = MeasureToken._new2123(n1.begin_token, n1.end_token, n1)
                    internals_.append(mt1)
                    tt = mt1.end_token
                    t = tt
                    t1 = t
            ...
            if ((isinstance(tt, NumberToken)) and tt.previous is not None): 
                if (tt.previous.is_value("USB", None)): 
                    ...
                    while ttt is not None: 
                        ...
                        ttt = ttt.next0_
                    continue
            mt0 = NumbersWithUnitToken.try_parse(tt, add_units, NumberWithUnitParseAttr.NO)
            if (mt0 is not None): 
        ...
        first_pass3989 = True
        while True:
            if first_pass3989: first_pass3989 = False
            else: t1 = t1.next0_
            if (not (t1 is not None)): break
            if (t1.is_newline_before and t1.is_table_control_char): 
                return None
            elif (t1.is_table_control_char): 
                pass
                ...
                if (t1.is_char(':')): 
                    ...
                    first_pass3990 = True
                    while True:
                        if first_pass3990: first_pass3990 = False
                        else: ttt = ttt.next0_
                        ...
                        if (ttt.is_char(';')): 
                            ...
                            break
                    if (len(li) > 1): 
                        res0 = MeasureToken._new2129(t0, li[len(li) - 1].end_token, li, True)
        ...
        if (t1 is None): 
            return None
        attrs = NumberWithUnitParseAttr.CANBENON
        if (not0_): 
            attrs = (Utils.valToEnum((attrs) | (NumberWithUnitParseAttr.NOT), NumberWithUnitParseAttr))
        if (is_resctriction): 
            attrs = (Utils.valToEnum((attrs) | (NumberWithUnitParseAttr.ISSECOND), NumberWithUnitParseAttr))
        mts = NumbersWithUnitToken.try_parse_multi(t1, add_units, attrs)
        if (mts is None): 
            if (units is not None and len(units) > 0): 
                if (t1 is None or t1.previous.is_char(':')): 
                    ...
                    if (t1 is None): 
                        ...
                        t1 = t1.previous
                    mts.append(NumbersWithUnitToken._new2130(t0, t1, math.nan))
            if (mts is None): 
                return None
        mt = mts[0]
        ...
        if (len(mts) > 1 and len(internals_) == 0): 
            if (len(mt.units) == 0): 
                if (units is not None): 
                    for m in mts: 
                        m.units = units
            res1 = MeasureToken._new2131(t0, mts[len(mts) - 1].end_token, name_.morph, True)
            res1.name = MiscHelper.get_text_value_of_meta_token(name_, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
            k = 0
            while k < len(mts): 
                ttt = MeasureToken._new2123(mts[k].begin_token, mts[k].end_token, mts[k])
                if (whd is not None): 
            ...
            if (tt1 is not None and tt1.is_char('±')): 
                nn = NumbersWithUnitToken._try_parse(tt1, add_units, NumberWithUnitParseAttr.ISSECOND)
        ...
        if (len(mt.units) == 0): 
            ...
            if (units is None): 
                ...
                mt.units = units
        res = MeasureToken._new2133(t0, mt.end_token, name_.morph, internals_)
        ...
        if (t1 is not None and t1.is_comma_and): 
            t1 = t1.next0_
        mts1 = NumbersWithUnitToken.try_parse_multi(t1, add_units, NumberWithUnitParseAttr.NO)
        if ((mts1 is not None and len(mts1) == 1 and (t1.whitespaces_before_count < 3)) and len(mts1[0].units) > 0 and not UnitToken.can_be_equals(mts[0].units, mts1[0].units)): 
            res.is_set = True
            res.nums = (None)
            res.internals.append(MeasureToken._new2123(mt.begin_token, mt.end_token, mt))
            res.internals.append(MeasureToken._new2123(mts1[0].begin_token, mts1[0].end_token, mts1[0]))
            res.end_token = mts1[0].end_token
        return res
    
    @staticmethod
    def _new2123(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'NumbersWithUnitToken') -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.nums = _arg3
        return res
    
    @staticmethod
    def _new2129(_arg1 : 'Token', _arg2 : 'Token', _arg3 : typing.List['MeasureToken'], _arg4 : bool) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.internals = _arg3
        res.is_empty = _arg4
        return res
    ...
    @staticmethod
    def _new2131(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : bool) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.reliable = _arg4
        return res
    ...
    @staticmethod
    def _new2133(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : typing.List['MeasureToken']) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.internals = _arg4
        return res