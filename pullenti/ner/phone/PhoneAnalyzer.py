...
class PhoneAnalyzer(Analyzer):
    ...
    class PhoneAnalizerData(AnalyzerData):
        ...
        def register_referent(self, referent : 'Referent') -> 'Referent':
            ...
            if (len(key) >= 10): 
                key = key[3:]
            ph_li = [ ]
            wrapph_li3279 = RefOutArgWrapper(None)
            inoutres3280 = Utils.tryGetValue(self.__m_phones_hash, key, wrapph_li3279)
            ph_li = wrapph_li3279.value
            if (not inoutres3280): 
                ph_li = list()
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        first_pass4082 = True
        while True:
            if first_pass4082: first_pass4082 = False
            else: t = t.next0_
            ...
            if (rts is None): 
                ...
                if (ki == PhoneKind.UNDEFINED and prev_phone is not None): 
                    if (isinstance(prev_phone.tag, PhoneKind)): 
                        ki = (Utils.valToEnum(prev_phone.tag, PhoneKind))
                    elif (not is_pref and prev_phone.kind != PhoneKind.MOBILE and kkk == 0): 
                        ki = prev_phone.kind
                for rt in rts: 
        ...
        if (num.tell() < 7): 
            if (city_code is not None and (len(city_code) + num.tell()) > 7): 
                ...
                if (pli[0].item_type == PhoneItemToken.PhoneItemType.PREFIX and pli[0].kind == PhoneKind.HOME): 
                    ok = False
                elif ((len(part_length) == 1 and (num.tell() < 7) and pli[0].kind != PhoneKind.UNDEFINED) and (pli[0].length_char < 3)): 
                    ok = False
        ...
        if ((ok and city_code is None and country_code is None) and prev_phone is None and not is_phone_before): 
            if (not end.is_whitespace_after and end.next0_ is not None): 
                ...
                if (not tt.is_whitespace_before): 
                    ok = False
        if (ok): 
            stempl = Utils.toStringStringIO(templ).strip()
            if (stempl == "4 3.4" or ((((stempl == "2.4" or stempl == "3.4")) and pli[0].length_char == 4))): 
                if (pli[0].begin_token.previous is not None and pli[0].begin_token.previous.is_char('.') and (isinstance(pli[0].begin_token.previous.previous, NumberToken))): 
                    ok = False
            if (stempl == "4 3,4" or ((((stempl == "2,4" or stempl == "3,4")) and pli[0].length_char == 4))): 
                if (pli[0].begin_token.previous is not None and pli[0].begin_token.previous.is_char(',') and (isinstance(pli[0].begin_token.previous.previous, NumberToken))): 
                    ok = False
