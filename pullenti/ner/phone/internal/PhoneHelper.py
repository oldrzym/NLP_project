...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                nn = None
                wrapnn3248 = RefOutArgWrapper(None)
                inoutres3249 = Utils.tryGetValue(tn.children, dig, wrapnn3248)
                nn = wrapnn3248.value
                if (not inoutres3249): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            nn = None
            wrapnn3250 = RefOutArgWrapper(None)
            inoutres3251 = Utils.tryGetValue(nod.children, dig, wrapnn3250)
            nn = wrapnn3250.value
            if (not inoutres3251): 
                break
