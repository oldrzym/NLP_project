...
class AddressAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (li is None or len(li) == 0): 
                continue
            if ((len(li) == 1 and li[0].typ == AddressItemType.STREET and li[0].referent.kind == StreetKind.RAILWAY) and li[0].referent.numbers is None): 
                t = li[0].end_token
                continue
            tt = AddressDefineHelper.try_define(li, t, ad, False)
            if (tt is not None): 
                t = tt
        sw.stop()
        sli = list()
    ...
    def process_ontology_item(self, begin : 'Token') -> 'ReferentToken':
        ...
        if (li is None or (len(li) < 2)): 
            return None
        rt = StreetDefineHelper._try_parse_street(li, True, False, False, None)
        if (rt is None): 
            return None
        ...
        while True:
            ...
            if (t is None): 
                break
            li = StreetItemToken.try_parse_list(begin, 10, None)
            rt1 = StreetDefineHelper._try_parse_street(li, True, False, False, None)
            if (rt1 is not None): 
