...
class ShortNameHelper:
    ...
    class ShortnameVar:
        ...
        @staticmethod
        def _new3221(_arg1 : str, _arg2 : 'MorphGender') -> 'ShortnameVar':
    ...
    def get_names_for_shortname(shortname : str) -> typing.List['ShortnameVar']:
        res = [ ]
        wrapres3219 = RefOutArgWrapper(None)
        inoutres3220 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, shortname, wrapres3219)
        res = wrapres3219.value
        if (not inoutres3220): 
            return None
        else: 
            return res
        ...
        if (obj is not None): 
            ...
            while t is not None: 
                if (t.is_newline_before): 
                    ...
                    for s in shos: 
                        li = None
                        wrapli3222 = RefOutArgWrapper(None)
                        inoutres3223 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, s, wrapli3222)
                        li = wrapli3222.value
                        if (not inoutres3223): 
                            li = list()
                            ShortNameHelper.M_SHORTS_NAMES[s] = li
                        li.append(ShortNameHelper.ShortnameVar._new3221(nam, g))
                    if (t is None): 
                        break
                    t = t.previous
                t = t.next0_
