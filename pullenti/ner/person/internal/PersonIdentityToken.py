...
class PersonIdentityToken(MetaToken):
    ...
    @staticmethod
    def try_attach_onto_for_duble(pit0 : 'PersonItemToken', pit1 : 'PersonItemToken') -> 'PersonReferent':
        if ((pit0 is None or pit0.firstname is None or pit1 is None) or pit1.middlename is None): 
            return None
        ad = PersonAnalyzer._get_data(pit0)
        if (len(ad.local_ontology.items) > 100): 
            return None
        p0 = None
        cou = 0
        for p in ad.local_ontology.items: 
            ...
            for ot in otl: 
                if (ot.end_token == pits[ind].end_token): 
                    ...
                    if (inf.gender == MorphGender.MASCULINE): 
                        ...
                        if (pers.is_male): 
                            continue
                    inf0 = MorphBaseInfo._new3100(inf.case_, inf.gender)
                    if (not ot.morph.case_.is_undefined and inf0.case_.equals(MorphCase.ALL_CASES) and ot.begin_token == ot.end_token): 
                    ...
                    for v in vars0_: 
                        ...
                        if (not pers.can_be_equals(p, ReferentsEqualType.WITHINONETEXT)): 
                            ...
                            pit = v
                    if ((pit is None and len(pits) == 1 and len(vars0_) == 0) and pers.find_slot(None, pits[0].value, True) is not None): 
                        p = PersonReferent()
                        p.add_slot(PersonReferent.ATTR_UNDEFNAME, pits[0].value, False, 0)
                        pit = PersonIdentityToken._new3101(pits[0].begin_token, pits[0].end_token, 3, FioTemplateType.UNDEFINED)
                        pit.ontology_person = p
                        ...
                        if (pit is None or (pit.coef < 2)): 
                            continue
                        if (pit.end_token != pits[len(pits) - 1].end_token and len(vars0_) > 0): 
                            continue
                        p = PersonReferent()
    ...
    def try_attach_for_ext_onto(pits : typing.List['PersonItemToken']) -> typing.List['PersonIdentityToken']:
        ...
        if (len(pits) == 3): 
            if (pits[0].typ == PersonItemToken.ItemType.VALUE and pits[1].typ == PersonItemToken.ItemType.INITIAL and pits[2].typ == PersonItemToken.ItemType.VALUE): 
                pit = PersonIdentityToken._new3102(pits[0].begin_token, pits[2].end_token, FioTemplateType.NAMEISURNAME)
                ...
                if (pits[0].firstname is None and pits[1].middlename is None and ((pits[1].firstname is not None or pits[2].middlename is not None))): 
                    ...
                    ok = True
                if (ok): 
                    pit = PersonIdentityToken._new3102(pits[0].begin_token, pits[2].end_token, FioTemplateType.SURNAMENAMESECNAME)
            ...
            for i in range(2):
                if (((pits[i].firstname is not None and pits[i].firstname.is_in_dictionary)) or ((pits[i ^ 1].lastname is not None and ((pits[i ^ 1].lastname.is_in_dictionary or pits[i ^ 1].lastname.is_lastname_has_std_tail))))): 
                    ...
                    break
            if (nam is not None): 
                pit = PersonIdentityToken._new3102(pits[0].begin_token, pits[1].end_token, (FioTemplateType.NAMESURNAME if nam == pits[0] else FioTemplateType.SURNAMENAME))
    ...
    def try_attach(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', first_tok : 'Token', king : bool, is_attr_before : bool) -> typing.List['PersonIdentityToken']:
        ...
        if ((pit) is not None): 
            ...
            pit = PersonIdentityToken.__try_attach_arabic(pits, ind, inf, ty == FioTemplateType.ARABICLONG)
            if ((pit) is not None): 
                res.append(pit)
            else: 
                ...
                if ((pit1) is not None): 
                    ...
                    if (pit is not None and (pit.coef + (1)) >= pit1.coef and ty != FioTemplateType.SURNAMENAME): 
                        pit1.coef -= (0.5)
                pit1 = PersonIdentityToken.__try_attach_name_secname_surname(pits, ind, inf, ty == FioTemplateType.NAMESECNAMESURNAME)
                if ((pit1) is not None): 
                    if (pit is not None and pit1.coef > 2 and pit.coef > pit1.coef): 
                        if (ind == 0 and len(pits) == 3): 
        ...
        for r in res: 
            ...
            if (ttt is not None and ttt.morph.class0_.equals(MorphClass.VERB)): 
    ...
    def manage_lastname(res : 'PersonIdentityToken', pit : 'PersonItemToken', inf : 'MorphBaseInfo') -> None:
        if (pit.lastname is None): 
            ...
            if (len(res.lastname.items) == 0 and pit.value is not None): 
                res.lastname.items.append(PersonMorphCollection.PersonMorphVariant._new3105(pit.value))
    ...
    def manage_firstname(res : 'PersonIdentityToken', pit : 'PersonItemToken', inf : 'MorphBaseInfo') -> None:
        if (pit.firstname is None): 
            ...
            if (pit.value.find('-') > 0 and pit.end_token.get_morph_class_in_dictionary().is_proper_name): 
                if (pit.lastname is not None): 
                    PersonIdentityToken.__set_value2(res.firstname, pit.lastname, inf)
                else: 
                    res.firstname.items.append(PersonMorphCollection.PersonMorphVariant._new3105(pit.value))
            else: 
                if (pit.lastname is not None): 
                    res.coef -= 1
                if (pit.is_in_dictionary): 
    ...
    def __try_attach_name_surname(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', prev_has_this_typ : bool=False, is_attr_before : bool=False) -> 'PersonIdentityToken':
        if ((ind + 1) >= len(pits) or pits[ind + 1].typ != PersonItemToken.ItemType.VALUE or pits[ind].typ != PersonItemToken.ItemType.VALUE): 
            return None
        if (pits[ind].begin_token.chars.is_all_upper and not pits[ind + 1].begin_token.chars.is_all_upper): 
            if (pits[ind].firstname is None or not pits[ind].firstname.is_in_dictionary): 
                return None
        ...
        if (PersonIdentityToken.__is_both_surnames(pits[ind], pits[ind + 1])): 
            if (len(pits) == 2 and ind == 0 and is_attr_before): 
                ...
                return None
        res = PersonIdentityToken._new3102(pits[ind].begin_token, pits[ind + 1].end_token, FioTemplateType.NAMESURNAME)
        res.coef -= (ind)
        ...
        if (pits[ind].firstname is not None): 
            if (isinstance(pits[ind + 1].begin_token, ReferentToken)): 
                res.coef += 1
            elif (((ind == 0 and pits[ind + 1].firstname is None and pits[ind + 1].middlename is None) and not pits[ind + 1].is_in_dictionary and pits[ind].chars.is_capital_upper) and pits[ind + 1].chars.is_capital_upper): 
                mc1 = pits[ind].begin_token.get_morph_class_in_dictionary()
                if (not mc1.is_noun): 
                    res.coef += 1
        ...
        if (mc.is_verb): 
            if (pits[ind].begin_token.chars.is_capital_upper and not MiscHelper.can_be_start_of_sentence(pits[ind].begin_token)): 
                ...
                res.coef -= (1)
        if (mc.is_pronoun and pits[ind].firstname is None): 
            return None
                    ...
                    if (pits[ind + 1].lastname is not None and ((pits[ind + 1].lastname.is_in_dictionary or pits[ind + 1].lastname.is_in_ontology))): 
                        ...
                        for v in pits[ind + 1].middlename.vars0_: 
                            if (((v.gender) & (res.morph.gender)) != (MorphGender.UNDEFINED)): 
                                res.coef -= (1)
                                break
        if (not pits[ind].chars.equals(pits[ind + 1].chars)): 
            if (pits[ind].chars.is_capital_upper and pits[ind + 1].chars.is_all_upper): 
                ...
                res.coef -= (10)
            elif (pits[ind + 1].end_token.chars.is_capital_upper and pits[ind].chars.is_capital_upper): 
                pass
            else: 
                res.coef -= (1)
        ...
        if (pits[ind].is_in_dictionary): 
            if (pits[ind + 1].is_in_dictionary): 
                ...
                if (pits[ind].is_in_dictionary and pits[ind].firstname is None): 
                    res.coef -= 1
            elif (((pits[ind].firstname is None or not pits[ind].firstname.is_in_dictionary)) and (pits[ind].value.find('-') < 0)): 
        ...
        if (ind > 0 and res.coef > 0 and pits[ind].is_hiphen_before): 
            ...
            if (b1 is not None and b1.second_count == b1.pair_count): 
                res0 = PersonIdentityToken._new3102(pits[ind].begin_token, pits[ind + 1].end_token, FioTemplateType.NAMESURNAME)
        ...
        if (ind == 0 and len(pits) == 2 and pits[0].chars.is_latin_letter): 
            ...
            if (pits[0].chars.is_all_upper and pits[1].chars.is_capital_upper): 
                res.coef = (0)
        elif (ind == 0 and len(pits) == 2 and pits[1].is_in_dictionary): 
            if (pits[0].is_in_dictionary): 
                res.coef -= (1)
            elif (pits[1].lastname is None or not pits[1].lastname.is_in_dictionary): 
                mc = pits[1].get_morph_class_in_dictionary()
                if (mc.is_verb and not mc.is_adjective): 
                    res.coef -= (1)
        if (ind == 0 and len(pits) == 2 and pits[1].middlename is not None): 
            t0 = pits[0].begin_token
            if (t0.previous is not None and t0.previous.is_comma): 
                t0 = t0.previous
            if ((isinstance(t0.previous, TextToken)) and t0.previous.is_newline_before): 
                li = MailLine.parse(t0.previous, 0, 0)
                if (li is not None and li.typ == MailLine.Types.HELLO): 
                    return None
        if ((ind == 0 and len(pits) == 3 and pits[2].middlename is not None) and pits[1].firstname is not None): 
            res.coef -= 1
        return res
    ...
    def __try_attach_name_secname_surname(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', prev_has_this_typ : bool=False) -> 'PersonIdentityToken':
        ...
        if (pits[ind + 2].lastname is None and not prev_has_this_typ and not pits[ind].morph.language.is_en): 
            return None
        if (pits[ind].begin_token.chars.is_all_upper and not pits[ind + 2].begin_token.chars.is_all_upper): 
            if (pits[ind].firstname is None or not pits[ind].firstname.is_in_dictionary): 
                return None
        if (pits[ind + 1].begin_token.chars.is_all_upper and not pits[ind + 2].begin_token.chars.is_all_upper and pits[ind + 1].typ == PersonItemToken.ItemType.VALUE): 
            if (pits[ind + 1].middlename is None or not pits[ind + 1].middlename.is_in_dictionary): 
                return None
        ok = False
        ...
        res.coef -= (ind)
        if (pits[ind + 1].middlename is not None and not pits[ind + 1].middlename.morph.case_.is_undefined): 
            if (((inf.case_) & pits[ind + 1].middlename.morph.case_).is_undefined): 
                inf = MorphBaseInfo()
        ...
        if (pits[ind + 1].middlename is not None and len(pits[ind + 1].middlename.vars0_) > 0): 
            ...
            res.coef += (0.5)
        if (not pits[ind].chars.equals(pits[ind + 2].chars)): 
            res.coef -= (1)
            if (pits[ind].chars.is_all_upper): 
                res.coef -= (1)
        elif (pits[ind + 1].typ != PersonItemToken.ItemType.INITIAL and not pits[ind].chars.equals(pits[ind + 1].chars)): 
            res.coef -= (1)
    ...
    def __correct_coef_after_lastname(res : 'PersonIdentityToken', pits : typing.List['PersonItemToken'], ind : int) -> None:
        if (not pits[ind - 1].is_newline_after): 
            pat = PersonAttrToken.try_attach(pits[ind - 1].begin_token, PersonAttrToken.PersonAttrAttachAttrs.ONLYKEYWORD)
            if (pat is not None): 
                res.coef -= (1)
        if (ind >= len(pits)): 
    ...
    def __try_attach_surname_name(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', prev_has_this_typ : bool=False) -> 'PersonIdentityToken':
        ...
        if (PersonIdentityToken.__is_both_surnames(pits[ind], pits[ind + 1])): 
            return None
        if (pits[ind + 1].begin_token.chars.is_all_upper and not pits[ind].begin_token.chars.is_all_upper): 
            if (pits[ind + 1].firstname is None or not pits[ind + 1].firstname.is_in_dictionary): 
                return None
        res = PersonIdentityToken._new3102(pits[ind].begin_token, pits[ind + 1].end_token, FioTemplateType.SURNAMENAME)
        res.coef -= (ind)
        if (pits[ind].is_newline_after): 
            res.coef -= 1
        ...
        if (((len(pits) == 2 and ind == 0 and pits[0].chars.is_all_upper) and pits[1].chars.is_capital_upper and not pits[1].is_in_dictionary) and (res.coef < 0)): 
            if (pits[1].lastname is not None and ((pits[1].lastname.is_has_std_postfix or pits[1].lastname.is_in_dictionary or pits[1].lastname.is_lastname_has_std_tail))): 
                pass
            else: 
                res.coef = (0)
        if (len(pits) == 2 and ind == 0 and pits[1].begin_token.is_value("ЛАВРА", None)): 
            if (pits[0].end_token.morph.class0_.is_adjective): 
                return None
        return res
        ...
        if (pits[0].lastname is None or not pits[0].lastname.is_lastname_has_std_tail): 
            ...
            if (stat is not None and stata is not None and statb is not None): 
                if (stat.like_chars_after_words is not None and stat.like_chars_before_words is not None): 
                    coua = 0
                    coub = 0
                    wrapcoua3111 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat.like_chars_after_words, stata, wrapcoua3111)
                    coua = wrapcoua3111.value
                    wrapcoub3110 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat.like_chars_before_words, statb, wrapcoub3110)
                    coub = wrapcoub3110.value
    ...
    def __try_attach_surname_name_secname(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', prev_has_this_typ : bool=False, always : bool=False) -> 'PersonIdentityToken':
        ...
        if (pits[ind].lastname is None and not prev_has_this_typ): 
            ...
            if (len(pits) == 3 and not always): 
                ...
                if (tt1 is not None and tt1.is_comma): 
                    tt1 = tt1.next0_
                if (tt1 is not None and not tt1.is_newline_before and PersonAttrToken.try_attach(tt1, PersonAttrToken.PersonAttrAttachAttrs.ONLYKEYWORD) is not None): 
                    pass
                else: 
                    return None
        if (not always): 
        ...
        if (pits[ind + 1].begin_token.chars.is_all_upper and not pits[ind].begin_token.chars.is_all_upper): 
            if (pits[ind + 1].firstname is None or not pits[ind + 1].firstname.is_in_dictionary): 
                return None
        if (pits[ind + 2].begin_token.chars.is_all_upper and not pits[ind].begin_token.chars.is_all_upper and pits[ind + 2].typ == PersonItemToken.ItemType.VALUE): 
            if (pits[ind + 2].middlename is None or not pits[ind + 2].middlename.is_in_dictionary): 
                return None
        res = PersonIdentityToken._new3102(pits[ind].begin_token, pits[ind + 2].end_token, FioTemplateType.SURNAMENAMESECNAME)
        ...
        if (pits[ind].is_newline_after): 
            if (pits[ind].is_newline_before and pits[ind + 2].is_newline_after): 
                pass
            elif (pits[ind].begin_token.previous is not None and pits[ind].begin_token.previous.is_table_control_char): 
                pass
            else: 
                res.coef -= 1
                if (pits[ind].whitespaces_after_count > 15): 
        ...
        if (pits[ind + 1].is_newline_after): 
            if (pits[ind].is_newline_before and pits[ind + 2].is_newline_after): 
                ...
                if (pits[ind + 1].whitespaces_after_count > 15): 
                    res.coef -= 1
        if (pits[ind + 2].middlename is not None and not pits[ind + 2].middlename.morph.case_.is_undefined): 
            if (((inf.case_) & pits[ind + 2].middlename.morph.case_).is_undefined): 
                inf = MorphBaseInfo()
        ...
        if (not pits[ind].chars.equals(pits[ind + 1].chars) or not pits[ind].chars.equals(pits[ind + 2].chars)): 
            res.coef -= (1)
        ...
        if (pits[ind].is_in_dictionary and pits[ind + 1].is_in_dictionary and pits[ind + 2].is_in_dictionary): 
            res.coef -= 1
        elif (pits[ind].is_in_dictionary and pits[ind].chars.is_capital_upper and MiscHelper.can_be_start_of_sentence(pits[ind].begin_token)): 
            mc = pits[ind].begin_token.get_morph_class_in_dictionary()
            if ((mc.is_adverb or mc.is_verb or mc.is_preposition) or mc.is_conjunction): 
                res.coef -= 1
        return res
    ...
    def __correct_coef_after_name(res : 'PersonIdentityToken', pits : typing.List['PersonItemToken'], ind : int) -> None:
        ...
        if (t is not None and t.next0_ is not None and t.next0_.is_char(',')): 
            t = t.next0_
        if (t is not None): 
            if (PersonAttrToken.try_attach(t, PersonAttrToken.PersonAttrAttachAttrs.ONLYKEYWORD) is not None): 
                res.coef += (1)
    ...
    def __calc_coef_after(tt : 'Token') -> float:
        if (tt is not None and tt.is_comma): 
            tt = tt.next0_
        attr = PersonAttrToken.try_attach(tt, PersonAttrToken.PersonAttrAttachAttrs.ONLYKEYWORD)
    ...
    def __try_attach_surnameii(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo') -> 'PersonIdentityToken':
        ...
        if (pits[ind].lastname is None): 
            return None
        res = PersonIdentityToken._new3102(pits[ind].begin_token, pits[ind + 1].end_token, FioTemplateType.SURNAMEI)
        res.coef -= (ind)
        PersonIdentityToken.manage_lastname(res, pits[ind], inf)
            ...
            if ((i1 + 3) == len(pits) and pits[i1 + 1].typ == PersonItemToken.ItemType.INITIAL and pits[i1 + 2].typ == PersonItemToken.ItemType.INITIAL): 
                ...
                while tt is not None: 
                    if (tt.is_newline_before): 
                        ...
                        ok = False
                        break
                    elif ((isinstance(tt, TextToken)) and tt.chars.is_letter and tt != pits[ind].begin_token): 
                        ok = False
                        break
                    tt = tt.previous
                if (ok): 
        ...
        if (not mc.is_proper and mc.is_noun): 
            if (((pits[0].begin_token.morph.number) & (MorphNumber.PLURAL)) != (MorphNumber.PLURAL)): 
                res.coef -= 1
            elif (pits[0].begin_token.is_value("ГЛАВА", None) or pits[0].begin_token.is_value("ЧАСТЬ", "ЧАСТИНА") or pits[0].begin_token.is_value("СТАТЬЯ", "СТАТТЯ")): 
                return None
        return res
    ...
    def __try_attachiisurname(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo') -> 'PersonIdentityToken':
        ...
        if (pits[ind].is_newline_after): 
            return None
        res = PersonIdentityToken._new3102(pits[ind].begin_token, pits[ind + 1].end_token, FioTemplateType.ISURNAME)
        res.coef -= (ind)
        res.firstname = PersonMorphCollection()
        ...
        if (i1 >= len(pits) or pits[i1].typ != PersonItemToken.ItemType.VALUE): 
            return None
        if (pits[i1].is_newline_before and not pits[i1].is_newline_after): 
            return None
        res.end_token = pits[i1].end_token
        prev = None
        if (not pits[ind].is_newline_before): 
            if (ind > 0): 
                ...
                prev = PersonItemToken.try_attach(pits[ind].begin_token.previous, (PersonItemToken.ParseAttr.CANBELATIN if pits[i1].chars.is_latin_letter else PersonItemToken.ParseAttr.NO), None)
                if (prev is not None): 
                    if (PersonAttrToken.try_attach_word(prev.begin_token, True) is not None): 
                        prev = (None)
                        res.coef += 1
    ...
    def __try_attach_king(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', prev_has_this_typ : bool=False) -> 'PersonIdentityToken':
        if (ind > 0 or ind >= len(pits)): 
            return None
        if (pits[0].is_newline_before): 
            return None
        if (pits[0].firstname is None): 
            if (pits[0].is_in_dictionary or not pits[0].chars.is_cyrillic_letter): 
                return None
        if (pits[0].begin_token.is_value("ТОМ", None)): 
            return None
        if (pits[0].typ != PersonItemToken.ItemType.VALUE): 
            return None
        i = 0
        ...
        if (isinstance(t, NumberToken)): 
            if (pits[i].whitespaces_after_count > 2): 
                return None
            ...
            if ((nt is not None and nt.int_value is not None and nt.value != "100") and (nt.whitespaces_before_count < 3) and t.chars.is_all_upper): 
                num = nt.int_value
                roman = True
                numt = nt.end_token
                t = numt
        if (num < 1): 
            if (((pits[0].firstname is not None or len(pits) == 1)) and prev_has_this_typ): 
                if (len(pits) == 1): 
                    ok = True
                elif (len(pits) == 2 and ((pits[0].end_token.next0_.is_hiphen or pits[1].firstname is not None))): 
                    ok = True
            if (not ok): 
                return None
        res = PersonIdentityToken._new3102(pits[0].begin_token, pits[0].end_token, FioTemplateType.KING)
        ...
        if (num > 0): 
            if (res.firstname is None and res.lastname is not None): 
                res.firstname = res.lastname
            res.lastname = PersonMorphCollection()
            res.lastname.number = num
            res.end_token = numt
            if (i == 0 and (numt.whitespaces_after_count < 2)): 
                pits1 = PersonItemToken.try_attach_list(numt.next0_, PersonItemToken.ParseAttr.NO, 2)
                if (pits1 is not None and len(pits1) == 1 and pits1[0].firstname is not None): 
                    PersonIdentityToken.__manage_middlename(res, pits1[0], inf)
                    res.end_token = pits1[0].end_token
        res.coef = ((3 if num > 0 else 2))
        return res
    
    @staticmethod
    def __try_attach_arabic(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', prev_has_this_typ : bool=False) -> 'PersonIdentityToken':
        if (ind > 0 or (len(pits) < 4)): 
            return None
        i = 0
        has_hiph = False
        errs = 0
        ...
        while True:
            ...
            if (not (i < len(pits))): break
            p = pits[i]
            if (i > 0 and p.whitespaces_before_count > 2): 
                break
            if (p.middlename is not None or p.value is None): 
                return None
            if (p.value.find('-') > 0): 
                has_hiph = True
                continue
            if (not p.chars.is_capital_upper): 
                break
            if (p.is_in_dictionary and p.firstname is None): 
                errs += 1
        if ((i < 4) or errs > 1): 
            return None
        if ((((i < 5) or i != len(pits))) and not has_hiph): 
            if (((len(pits) == 4 and i == len(pits) and pits[0].firstname is not None) and pits[1].firstname is not None and pits[2].firstname is not None) and pits[3].firstname is None and pits[3].middlename is None): 
                pass
            else: 
                return None
        res = PersonIdentityToken._new3102(pits[0].begin_token, pits[i - 1].end_token, FioTemplateType.ARABICLONG)
        res.coef = ((3 if has_hiph else 2))
        ...
        if (pits[0].firstname is not None): 
            ...
            res.firstname = (None)
        elif (pits[0].lastname is not None): 
            PersonIdentityToken.manage_lastname(res, pits[0], inf)
        else: 
            res.lastname.items.append(PersonMorphCollection.PersonMorphVariant._new3105(pits[0].value))
        j = 1
        while j < i: 
            p = pits[j]
            for v in res.lastname.items: 
                v.value = "{0} {1}".format(v.value, p.value)
            j += 1
        return res
    ...
    def __try_attach_asian(pits : typing.List['PersonItemToken'], ind : int, inf : 'MorphBaseInfo', cou : int, prev_has_this_typ : bool=False) -> 'PersonIdentityToken':
        ...
        if (pits[0].lastname is not None and pits[0].lastname.is_china_surname and pits[0].chars.is_capital_upper): 
            ...
            if (not pits[1].is_asian_item(True)): 
                return None
        mc0 = pits[0].get_morph_class_in_dictionary()
        if (mc0.is_pronoun and pits[1].firstname is not None): 
            return None
        cou -= 1
        is_chine_sur = pits[0].lastname is not None and pits[0].lastname.is_china_surname
        res = PersonIdentityToken._new3102(pits[0].begin_token, pits[cou].end_token, FioTemplateType.ASIANNAME)
        ...
        if (len(pits) == 1): 
            ...
            for p in pits: 
                if (p.typ != PersonItemToken.ItemType.VALUE): 
                    return None
                if (not p.chars.equals(pits[0].chars)): 
                    return None
        ...
        for i in range(2):
            ...
            if (not cas.is_undefined): 
                if (inf is not None and not inf.case_.is_undefined and not ((inf.case_) & cas).is_undefined): 
                    cas &= inf.case_
                res.add_item(MorphBaseInfo._new3119(g, cas))
        verb_gend = MorphGender.UNDEFINED
        if ((next0__ is not None and (isinstance(next0__, TextToken)) and next0__.chars.is_all_lower) and next0__.morph.class0_.equals(MorphClass.VERB) and next0__.morph.number == MorphNumber.SINGULAR): 
    ...
    def correctxfml(pli0 : typing.List['PersonIdentityToken'], pli1 : typing.List['PersonIdentityToken'], attrs : typing.List['PersonAttrToken']) -> bool:
        ...
        if (not p1.end_token.is_newline_after): 
            if (PersonItemToken.try_attach(p1.end_token.next0_, PersonItemToken.ParseAttr.NO, None) is not None): 
                return False
    ...
    def check_latin_after(pit : 'PersonIdentityToken') -> 'PersonIdentityToken':
        ...
        if (nam is None or sur is None): 
            return None
        res = PersonIdentityToken._new3102(t, et, pit.typ)
        res.lastname = PersonMorphCollection()
    ...
    @staticmethod
    def _new3101(_arg1 : 'Token', _arg2 : 'Token', _arg3 : float, _arg4 : 'FioTemplateType') -> 'PersonIdentityToken':
    ...
    @staticmethod
    def _new3102(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'FioTemplateType') -> 'PersonIdentityToken':
        res = PersonIdentityToken(_arg1, _arg2)
        res.typ = _arg3
        return res