...
class PersonIdToken(MetaToken):
    ...
    def try_attach(t : 'Token') -> 'ReferentToken':
        ...
        if (li[0].typ == PersonIdToken.Typs.NUMBER): 
            ...
            num = li[0].value
            i = 1
        elif (len(li) > 1 and li[0].typ == PersonIdToken.Typs.ORG and li[1].typ == PersonIdToken.Typs.NUMBER): 
            i = 0
            num = li[1].value
        else: 
            return None
    ...
    def __try_parse(t : 'Token', prev : 'PersonIdToken') -> 'PersonIdToken':
        ...
        if (t.is_value("СВИДЕТЕЛЬСТВО", None)): 
            ...
            while True:
                ...
                if (tt.is_value("РЕГИСТРАЦИЯ", None) or tt.is_value("РЕЕСТР", None) or tt.is_value("ЗАРЕГИСТРИРОВАТЬ", None)): 
                    ...
                    break
            if (reg and ip): 
                return PersonIdToken._new3121(t, tt1, PersonIdToken.Typs.KEYWORD, "СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ")
        tok = PersonIdToken.M_ONTOLOGY.try_parse(t, TerminParseAttr.NO)
        if (tok is not None): 
            ty = Utils.valToEnum(tok.termin.tag, PersonIdToken.Typs)
            res = PersonIdToken._new3121(tok.begin_token, tok.end_token, ty, tok.termin.canonic_text)
            if (prev is None): 
                ...
                while True:
                    ...
                    if (r is not None): 
                        break
                    ait = PersonAttrToken.try_attach(t, PersonAttrToken.PersonAttrAttachAttrs.NO)
                    if (ait is not None): 
            ...
            if (ty == PersonIdToken.Typs.NUMBER): 
                ...
                while True:
                    ...
                    if (tt.is_newline_before): 
                        break
                    if (not (isinstance(tt, NumberToken)) and not tt.is_char_of("\\/")): 
                        nnn = NumberHelper.try_parse_roman(tt)
                        if (nnn is not None and ((not nnn.is_whitespace_before or tt == tok.end_token.next0_))): 
                            print(MiscHelper.get_text_value_of_meta_token(nnn, GetTextAttr.NO), end="", file=tmp)
                            tt = nnn.end_token
                            res.end_token = tt
                            continue
                        break
        ...
        if (npt is not None): 
            if (npt.end_token.is_value("УДОСТОВЕРЕНИЕ", "ПОСВІДЧЕННЯ")): 
                return PersonIdToken._new3121(t, npt.end_token, PersonIdToken.Typs.KEYWORD, npt.get_normal_case_text(None, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False))
        if (prev is None): 
            return None
        t0 = t
        ...
        if (isinstance(t, NumberToken)): 
            tmp = io.StringIO()
            res = PersonIdToken._new3124(t0, t, PersonIdToken.Typs.NUMBER)
            tt = t
            ...
            if (t0 != t): 
                res.has_prefix = True
            return res
        if (prev.typ == PersonIdToken.Typs.KEYWORD or t0 != t): 
            rom = NumberHelper.try_parse_roman(t)
            if (rom is not None): 
                tt1 = rom.end_token.next0_
                if (tt1 is not None and tt1.is_hiphen): 
                    tt1 = tt1.next0_
                if ((isinstance(tt1, TextToken)) and (tt1.length_char < 4) and tt1.chars.is_all_upper): 
                    res = PersonIdToken._new3124(t0, tt1, PersonIdToken.Typs.SERIA)
                    res.value = "{0}-{1}".format(MiscHelper.get_text_value_of_meta_token(rom, GetTextAttr.NO), tt1.term)
                    if (t0 != t): 
                        res.has_prefix = True
                    return res
        if (isinstance(t, ReferentToken)): 
            ...
            if (r is not None): 
                if (r.type_name == "DATE"): 
                    return PersonIdToken._new3126(t, t, PersonIdToken.Typs.DATE, r)
                if (r.type_name == "ORGANIZATION"): 
                    return PersonIdToken._new3126(t, t, PersonIdToken.Typs.ORG, r)
                if (r.type_name == "ADDRESS"): 
                    return PersonIdToken._new3126(t, t, PersonIdToken.Typs.ADDRESS, r)
        if ((prev is not None and prev.typ == PersonIdToken.Typs.KEYWORD and (isinstance(t, TextToken))) and not t.chars.is_all_lower and t.chars.is_letter): 
            ...
            if (rr is not None and rr.typ == PersonIdToken.Typs.NUMBER): 
                return PersonIdToken._new3121(t, t, PersonIdToken.Typs.SERIA, t.term)
        if ((t is not None and t.is_value("ОТ", "ВІД") and (isinstance(t.next0_, ReferentToken))) and t.next0_.get_referent().type_name == "DATE"): 
            return PersonIdToken._new3126(t, t.next0_, PersonIdToken.Typs.DATE, t.next0_.get_referent())
        return None
    ...
    def initialize() -> None:
        ...
        t = None
        t = Termin._new92("ПАСПОРТ", PersonIdToken.Typs.KEYWORD)
        ...
        t = Termin._new92("ЗАГРАНИЧНЫЙ ПАСПОРТ", PersonIdToken.Typs.KEYWORD)
        t.add_variant("ЗАГРАНПАСПОРТ", False)
        t.add_abridge("ЗАГРАН. ПАСПОРТ")
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("СВИДЕТЕЛЬСТВО О РОЖДЕНИИ", PersonIdToken.Typs.KEYWORD)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("СВИДЕТЕЛЬСТВО О СМЕРТИ", PersonIdToken.Typs.KEYWORD)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("СПРАВКА О СМЕРТИ", PersonIdToken.Typs.KEYWORD)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ", PersonIdToken.Typs.KEYWORD)
        t.add_variant("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ ОФИЦЕРА", False)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ", PersonIdToken.Typs.KEYWORD)
        ...
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("ВОДИТЕЛЬСКОЕ УДОСТОВЕРЕНИЕ", PersonIdToken.Typs.KEYWORD)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("СЕРИЯ", PersonIdToken.Typs.SERIA)
        t.add_abridge("СЕР.")
        t.add_variant("СЕРИ", False)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("НОМЕР", PersonIdToken.Typs.NUMBER)
        t.add_abridge("НОМ.")
        t.add_abridge("Н-Р")
        t.add_variant("№", False)
        t.add_variant("N", False)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("ВЫДАТЬ", PersonIdToken.Typs.VIDAN)
        t.add_variant("ВЫДАВАТЬ", False)
        t.add_variant("ДАТА ВЫДАЧИ", False)
        t.add_variant("ДАТА РЕГИСТРАЦИИ", False)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("КОД ПОДРАЗДЕЛЕНИЯ", PersonIdToken.Typs.CODE)
        t.add_abridge("К/П")
        t.add_abridge("К.П.")
        t.add_variant("КОД", False)
        t.add_abridge("КОД ПОДР")
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new92("РЕГИСТРАЦИЯ", PersonIdToken.Typs.ADDRESS)
    ...
    @staticmethod
    def _new3121(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Typs', _arg4 : str) -> 'PersonIdToken':
    ...
    @staticmethod
    def _new3124(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Typs') -> 'PersonIdToken':
        res = PersonIdToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new3126(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Typs', _arg4 : 'Referent') -> 'PersonIdToken':
        res = PersonIdToken(_arg1, _arg2)
        res.typ = _arg3
        res.referent = _arg4
        return res