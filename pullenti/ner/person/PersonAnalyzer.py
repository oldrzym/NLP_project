...
class PersonAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
                            ...
                            if (rt.referent is None): 
                                    ...
                                    if (isinstance(s.value, PersonAttrToken)): 
                                        ...
                                        for ss in pat.prop_ref.slots: 
                                            if (ss.type_name == PersonPropertyReferent.ATTR_REF and (isinstance(ss.value, ReferentToken))): 
                                                ...
                                                rr = ReferentToken._new916(rt1.referent, rt1.begin_token, rt1.end_token, rt1.morph)
                                                kit.embed_token(rr)
                                        ...
                                        if (rt0.referent is not None): 
                                            for s1 in rt0.referent.slots: 
                                                if (isinstance(s1.value, PersonAttrToken)): 
                                                    ...
                                                    rt2 = ReferentToken._new916(pat.prop_ref, pat.begin_token, pat.end_token, pat.morph)
                                                    kit.embed_token(rt2)
                                ...
                                for p in pats: 
                                    if (p.prop_ref is not None): 
                                        rr = ReferentToken._new916(p.prop_ref, p.begin_token, p.end_token, p.morph)
                                        kit.embed_token(rr)
                                        if (rr.begin_token == rt.begin_token): 
                                            rt.begin_token = rr
                                        if (rr.end_token == rt.end_token): 
        ...
        while True:
            ...
            first_pass4073 = True
            while True:
                if first_pass4073: first_pass4073 = False
                else: tt = tt.next0_
        ...
        for r in ad.referents: 
            ...
            for s in p.slots: 
                if (s.type_name == PersonReferent.ATTR_ATTR and (isinstance(s.value, PersonPropertyReferent))): 
                    ...
                    li = [ ]
                    wrapli3228 = RefOutArgWrapper(None)
                    inoutres3229 = Utils.tryGetValue(props, pr, wrapli3228)
                    li = wrapli3228.value
                    if (not inoutres3229): 
                        li = list()
                        props[pr] = li
        ...
        first_pass4074 = True
        while True:
            if first_pass4074: first_pass4074 = False
            else: t = t.next0_
            if (not (t is not None)): break
            if (t.is_ignored): 
                continue
            ...
            if (len(pers) == 1): 
                ...
                if (tt is not None and ((tt.is_char('_') or tt.is_newline_before or tt.is_table_control_char))): 
                    ...
                    rt1 = ReferentToken._new916(ppr, pat, pat, pat.morph)
                    kit.embed_token(rt1)
                    rt2 = ReferentToken._new916(pers[0], rt1, rt1, pat.morph)
                    kit.embed_token(rt2)
                    continue
            if (pat.prop_ref is not None): 
                if (pat.can_be_independent_property or len(pers) > 0): 
                    rt = ReferentToken._new916(ad.register_referent(pat.prop_ref), pat.begin_token, pat.end_token, pat.morph)
                    kit.embed_token(rt)
                    t = (rt)
                    continue
            t = pat.end_token
    ...
    def process_referent_stat(begin : 'Token', param : str=None) -> 'ReferentToken':
        ...
        if (pat is None or pat.prop_ref is None): 
            return None
        rt = ReferentToken._new916(pat.prop_ref, pat.begin_token, pat.end_token, pat.morph)
        rt.data = (ad)
        return rt
    ...
    def __try_attach_persons(self, t : 'Token', step : int) -> typing.List['ReferentToken']:
        ...
        if (names is not None): 
            for n in names: 
                pers = PersonReferent()
                bi = MorphBaseInfo._new3234(MorphNumber.SINGULAR, t.kit.base_language)
                bi.class0_ = MorphClass._new3193(True)
    ...
    def _try_attach_person(t : 'Token', for_ext_ontos : bool, step : int, for_attribute : bool=False) -> 'ReferentToken':
        ...
        if (ad is None): 
            return None
        if (ad.overflow_level > 2): 
            return None
        ad.overflow_level += 1
        res = PersonAnalyzer.__try_attach_person_int(t, for_ext_ontos, step, for_attribute)
        ad.overflow_level -= 1
        return res
    ...
    def __try_attach_person_int(t : 'Token', for_ext_ontos : bool, step : int, for_attribute : bool=False) -> 'ReferentToken':
        ...
        first_pass4075 = True
        while True:
            if first_pass4075: first_pass4075 = False
            ...
            if ((a is not None and a.begin_token == a.end_token and not a.begin_token.chars.is_all_lower) and (a.whitespaces_after_count < 3)): 
                ...
                if (pits is not None and len(pits) >= 6): 
                    if (pits[2].is_newline_after and pits[5].is_newline_after): 
                        a = (None)
            if ((a is not None and attrs is not None and not t.chars.is_all_lower) and attrs[0].typ == PersonAttrTerminType.PREFIX): 
                pits = PersonItemToken.try_attach_list(t, PersonItemToken.ParseAttr.IGNOREATTRS, 10)
                if (pits is not None and len(pits) >= 2 and pits[0].lastname is not None): 
                    a = (None)
            if ((a is None and t.is_value("НА", None) and t.next0_ is not None) and t.next0_.is_value("ИМЯ", None)): 
                a = PersonAttrToken._new3010(t, t.next0_, MorphCollection._new3014(MorphCase.GENITIVE))
                is_genitive = True
            if (a is None): 
                break
        ...
        if (attrs is not None): 
            ...
            while t is not None and t.is_table_control_char:
                t = t.next0_
        while t is not None and t.is_char('_'):
            t = t.next0_
        if (t is None): 
            if (attrs is not None): 
        ...
        if (t.previous is not None and t.previous.is_comma_and): 
            pass
        slash = False
        if (attrs is not None and t is not None and t.is_char_of("/\\")): 
            slash = True
            t = t.next0_
        if (t is None): 
            return None
        for k in range(2):
            ...
            if (mi.case_.equals(MorphCase.ALL_CASES) and len(pits) > 0): 
                if (t0.whitespaces_before_count > 4 and pits[len(pits) - 1].whitespaces_after_count > 4): 
                    mi.case_ = MorphCase.NOMINATIVE
            if (pits is not None and len(pits) == 1 and pits[0].firstname is not None): 
                if (pits[0].end_token.next0_ is not None and pits[0].end_token.next0_.is_and and (isinstance(pits[0].end_token.next0_.next0_, ReferentToken))): 
                    ...
                    if (pr is not None): 
                        ...
                        pers = PersonReferent()
                        bi = MorphBaseInfo._new3240(v.gender, MorphNumber.SINGULAR, pits[0].kit.base_language)
                        bi.class0_ = MorphClass._new3193(True)
                ...
                if ((attr is not None and attr.prop_ref is not None and attr.prop_ref.kind == PersonPropertyKind.KIN) and attr.gender != MorphGender.UNDEFINED): 
                    ...
                    if (pr is not None): 
                        pers = PersonReferent()
                        bi = MorphBaseInfo._new3243(MorphNumber.SINGULAR, attr.gender, attr.kit.base_language)
                        bi.class0_ = MorphClass._new3193(True)
                        for s in pr.slots: 
                            if (s.type_name == PersonReferent.ATTR_LASTNAME): 
                                sur = Utils.asObjectOrNull(s.value, str)
                    ...
                    if (gender != MorphGender.FEMINIE and gender != MorphGender.MASCULINE): 
                        if ((pli[0].is_newline_before and pli[0].is_newline_after and pli[0].lastname is not None) and pli[0].lastname.has_lastname_standard_tail): 
                            if (len(pli[0].lastname.values) == 2): 
                                ...
                                first_pass4076 = True
                                while True:
                                    if first_pass4076: first_pass4076 = False
        ...
        if (attrs is not None): 
            attr = attrs[len(attrs) - 1]
            if (attr.can_be_single_person and attr.prop_ref is not None): 
                return ReferentToken._new916(attr.prop_ref, attr.begin_token, attr.end_token, attr.morph)
        return None
