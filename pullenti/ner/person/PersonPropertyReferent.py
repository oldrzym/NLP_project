...
class PersonPropertyReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang'=None, lev : int=0) -> str:
        ...
        for r in self.slots: 
            if (r.type_name == PersonPropertyReferent.ATTR_REF and (isinstance(r.value, Referent)) and (lev < 10)): 
                print("; {0}".format(r.value.to_string_ex(short_variant, lang, lev + 1)), end="", file=res, flush=True)
        hi = self.higher
        if (hi is not None and hi != self and self.__check_correct_higher(hi, 0)): 
            print("; {0}".format(hi.to_string_ex(short_variant, lang, lev + 1)), end="", file=res, flush=True)
    ...
    def can_be_equals(self, obj : 'Referent', typ : 'ReferentsEqualType') -> bool:
        ...
        if (self.find_slot(PersonPropertyReferent.ATTR_REF, None, True) is not None or pr.find_slot(PersonPropertyReferent.ATTR_REF, None, True) is not None): 
            ...
            first_pass4077 = True
            while True:
                if first_pass4077: first_pass4077 = False
                else: i += 1
            ...
            first_pass4078 = True
            while True:
                if first_pass4078: first_pass4078 = False
                else: i += 1
            ...
            if (eq_start): 
                if (self.higher is not None and self.higher.can_be_equals(pr.higher, typ)): 
                    return True
            return False
        return True
    ...
    def can_be_general_for(self, obj : 'Referent') -> bool:
        ...
        for s in self.slots: 
            if (s.type_name == PersonPropertyReferent.ATTR_REF): 
                if (pr.find_slot(PersonPropertyReferent.ATTR_REF, s.value, True) is None): 
                    return False
                refs += 1
        pr_refs = 0
        for s in pr.slots: 
        ...
        if (self.higher is not None): 
            if (pr.higher is None): 
                return False
        if (n1 == n2): 
            if (refs == pr_refs): 
                if (self.higher is not None): 
                    return self.higher.can_be_general_for(pr.higher)
                return False
            if (self.higher is not None): 
                if (not pr.higher.can_be_equals(self.higher, ReferentsEqualType.WITHINONETEXT)): 
                    return False
            return True
        if (n2.startswith(n1)): 
            if (n2.startswith(n1 + " ")): 
    ...
    @staticmethod
    def _new3012(_arg1 : str) -> 'PersonPropertyReferent':
