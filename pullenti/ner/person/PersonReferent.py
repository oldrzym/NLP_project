...
class PersonReferent(Referent):
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang'=None, lev : int=0) -> str:
        if (short_variant): 
            ...
            res = self.__to_full_string(PersonReferent.SHOW_LASTNAME_ON_FIRST_POSITION, lang, False)
            if (self.find_slot(PersonReferent.ATTR_NICKNAME, None, True) is None): 
                return res
            niks = self.get_string_values(PersonReferent.ATTR_NICKNAME)
            if (len(niks) == 1): 
                return "{0} ({1})".format(res, PersonReferent.__out_nik(niks[0]))
            tmp = io.StringIO()
            ...
            for s in niks: 
                if (s != niks[0]): 
                    print(", ", end="", file=tmp)
                print(PersonReferent.__out_nik(s), end="", file=tmp)
            print(")", end="", file=tmp)
            return Utils.toStringStringIO(tmp)
    
    @staticmethod
    def __out_nik(str0_ : str) -> str:
        if (len(str0_) < 4): 
            return str0_
    ...
    def __to_short_string(self, lang : 'MorphLang') -> str:
        ...
        return self.__to_full_string(False, lang, True)
    
    def __to_full_string(self, last_name_first : bool, lang : 'MorphLang', out_niks : bool) -> str:
        id0_ = None
        for a in self.slots: 
            if (a.type_name == PersonReferent.ATTR_IDENTITY): 
                s = str(a.value)
        ...
        if (n is not None): 
            ...
            if ((n) is not None): 
                ...
                if (out_niks): 
                    ...
                    if (nik is not None): 
                        n = "{0} {1}".format(n, PersonReferent.__out_nik(nik))
                return n
        return "?"
    ...
    def _add_identity(self, ident : 'PersonMorphCollection') -> None:
        ...
        for v in ident.values: 
            ...
            li = Utils.splitString(v, ' ', False)
            i = 0
            while i < len(li): 
                a = PersonReferent.ATTR_UNDEFNAME
                if (i == 0 and len(li) > 1): 
                    a = PersonReferent.ATTR_FIRSTNAME
                elif (i == (len(li) - 1) and len(li) > 1): 
                    a = PersonReferent.ATTR_LASTNAME
                self.add_slot(a, li[i], False, 0)
                i += 1
        self._correct_data()
        ...
        if (((tit1 is not None or tit2 is not None)) or ((nick1 is not None and nick1 == nick2))): 
            if (tit1 is None or tit2 is None): 
                if (nick1 is not None and nick1 == nick2): 
                    pass
                elif (nick1 is not None and self.__check_nik_equ_sur(nick1, p.get_string_value(PersonReferent.ATTR_LASTNAME))): 
                    pass
                elif (nick2 is not None and self.__check_nik_equ_sur(nick2, self.get_string_value(PersonReferent.ATTR_LASTNAME))): 
                    pass
                else: 
                    return False
            elif (tit1 != tit2): 
            ...
            if (self.find_slot(PersonReferent.ATTR_FIRSTNAME, None, True) is not None and p.find_slot(PersonReferent.ATTR_FIRSTNAME, None, True) is not None): 
                if (not self.__check_names(PersonReferent.ATTR_FIRSTNAME, p)): 
                    return False
                return True
        if (self.find_slot(PersonReferent.ATTR_UNDEFNAME, None, True) is not None and p.find_slot(PersonReferent.ATTR_UNDEFNAME, None, True) is not None): 
            und1 = self.__get_all_idents()
            und2 = p.__get_all_idents()
            if (len(und1) <= len(und2)): 
                for u in und1: 
                    if (not u in und2): 
                        if (u == und1[len(und1) - 1] and PersonReferent._del_surname_end(u) == PersonReferent._del_surname_end(und2[len(und2) - 1])): 
                            pass
                        else: 
                            return False
                return True
                ...
                for u in und2: 
                    if (not u in und1): 
                        if (u == und2[len(und2) - 1] and PersonReferent._del_surname_end(u) == PersonReferent._del_surname_end(und1[len(und1) - 1])): 
                            pass
                        return False
                return True
        return False
    
    def __check_nik_equ_sur(self, nik : str, sur : str) -> bool:
        if (nik == "I"): 
            return sur == "ПЕРВЫЙ" or sur == "ПЕРВАЯ" or sur == "ПЕРШЕ"
        if (nik == "II"): 
            return (sur == "ВТОРОЙ" or sur == "ВТОРАЯ" or sur == "ДРУГЕ") or sur == "ДРУГA"
        if (nik == "III"): 
            return sur == "ТРЕТИЙ" or sur == "ТРЕТЬЯ" or sur == "ТРЕТИНА"
        return False
    
    def __get_all_idents(self) -> typing.List[str]:
        res = self.get_string_values(PersonReferent.ATTR_UNDEFNAME)
        s = self.get_string_value(PersonReferent.ATTR_FIRSTNAME)
        if (s is not None): 
            res.insert(0, s)
        s = self.get_string_value(PersonReferent.ATTR_LASTNAME)
        if (s is not None): 
            res.append(s)
        return res
    ...
    def __correct_surnames(self) -> None:
        def0_ = self.is_male or self.is_female
        i = 0
        while i < len(self.slots): 
            if (self.slots[i].type_name == PersonReferent.ATTR_LASTNAME and def0_): 
                    ...
                    if (self.slots[j].type_name == PersonReferent.ATTR_LASTNAME): 
                        ...
                        if (s != s1 and PersonReferent._del_surname_end(s) == PersonReferent._del_surname_end(s1) and len(s1) != len(s)): 
                            if (self.is_male): 
                                ...
                                del self.slots[j]
                                j -= 1
                            elif (self.is_female and (len(s1) < len(s))): 
                                del self.slots[j]
                                j -= 1
                            else: 
                                del self.slots[i]
                                ...
                                break
                    j += 1
            elif (self.slots[i].type_name == PersonReferent.ATTR_UNDEFNAME): 
                if (self.find_slot(PersonReferent.ATTR_FIRSTNAME, self.slots[i].value, True) is not None or self.find_slot(PersonReferent.ATTR_LASTNAME, self.slots[i].value, True) is not None): 
                    del self.slots[i]
                    i -= 1
            i += 1
        nik = self.get_string_value(PersonReferent.ATTR_NICKNAME)
        if (nik is not None): 
            for i in range(len(self.slots) - 1, -1, -1):
                if (self.slots[i].type_name == PersonReferent.ATTR_LASTNAME): 
                    if (self.__check_nik_equ_sur(nik, Utils.asObjectOrNull(self.slots[i].value, str))): 
                        del self.slots[i]
        ...
        for i in range(len(self.slots) - 1, -1, -1):
            if (self.slots[i].type_name == attr_name): 
                ...
                if (v in vars0_): 
                    continue
                if (v.find('-') > 0): 
                    if (v.replace("-", "") in vars0_): 
                        continue
                j = 0
                first_pass4079 = True
                while True:
                    if first_pass4079: first_pass4079 = False
                    else: j += 1
    ...
    def create_ontology_item(self) -> 'IntOntologyItem':
        ...
        for a in self.slots: 
            if (a.type_name == PersonReferent.ATTR_IDENTITY): 
                oi.termins.append(Termin._new1414(str(a.value), True))
                ...
                oi.termins.append(t)
            elif (a.type_name == PersonReferent.ATTR_LASTNAME or a.type_name == PersonReferent.ATTR_FIRSTNAME or a.type_name == PersonReferent.ATTR_UNDEFNAME): 
                if (a.type_name == PersonReferent.ATTR_FIRSTNAME and self.find_slot(PersonReferent.ATTR_IDENTITY, None, True) is None): 
                    continue
