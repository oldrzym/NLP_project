...
class Referent:
    ...
    def __str__(self) -> str:
        return self.to_string_ex(False, MorphLang.UNKNOWN, 0)
    ...
    def get_int_value(self, attr_name : str, def_value : int) -> int:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return def_value
        res = 0
        wrapres3496 = RefOutArgWrapper(0)
        inoutres3497 = Utils.tryParseInt(str0_, wrapres3496)
        res = wrapres3496.value
        if (not inoutres3497): 
            return def_value
        return res
    ...
    def add_occurence_of_ref_tok(self, rt : 'ReferentToken') -> None:
        self.add_occurence(TextAnnotation._new894(rt.kit.sofa, rt.begin_char, rt.end_char, rt.referent))
    ...
    def add_occurence(self, anno : 'TextAnnotation') -> None:
        ...
        if (anno.occurence_of != self and anno.occurence_of is not None): 
            anno = TextAnnotation._new3499(anno.begin_char, anno.end_char, anno.sofa)
        if (self.__m_occurrence is None): 
            self.__m_occurrence = list()
    ...
    def clone(self) -> 'Referent':
        ...
        for r in self.slots: 
            rr = Slot._new3500(r.type_name, r.value, r.count)
            rr.owner = res
            res.slots.append(rr)
        return res
    ...
    def merge_slots(self, obj : 'Referent', merge_statistic : bool=True) -> None:
        ...
        for r in obj.slots: 
            ...
            if (s is not None and merge_statistic): 
                ...
                if (r.occurrence is not None): 
                    if (s.occurrence is None): 
                        s.occurrence = list()
                    for o in r.occurrence: 
                        if (len(s.occurrence) >= 10000): 
                            break
                        s.occurrence.append(o)
    ...
    def deserialize(self, stream : Stream, all0_ : typing.List['Referent'], sofa : 'SourceOfAnalysis', new_format : bool=False) -> None:
        ...
        while i < cou: 
            ...
            if (new_format): 
                bb = stream.readbyte()
                if (bb == (2)): 
                    id1 = SerializerHelper.deserialize_int(stream)
                    if (id1 > 0 and id1 <= len(all0_)): 
                        val = (all0_[id1 - 1])
                elif (bb == (1)): 
        ...
        i = 0
        while i < cou: 
            a = TextAnnotation._new3501(sofa, self)
