...
class InnerMorphology:
    ...
    def run(self, text : str, only_tokenizing : bool, dlang : 'MorphLang', good_text : bool, progress : EventHandler) -> typing.List['MorphToken']:
        ...
        while True:
            ...
            if (term.startswith("ЗАВ")): 
                term1 = term[3:]
                if (term1.startswith("ОТДЕЛ") or term1.startswith("ЛАБОРАТ") or term1.startswith("КАФЕДР")): 
                    term = "ЗАВ"
                    j = (i + 3)
            ...
            if (len(term) > 2): 
                if (lang.equals(MorphLang.UA)): 
                    pure_ukr_words += 1
                elif (lang.equals(MorphLang.RU)): 
                    pure_rus_words += 1
                elif (lang.equals(MorphLang.BY)): 
                    pure_by_words += 1
                elif (lang.equals(MorphLang.KZ)): 
                    pure_kz_words += 1
            if (lang.is_ru): 
                tot_rus_words += 1
            if (lang.is_ua): 
        ...
        if (pure_rus_words > pure_ukr_words and pure_rus_words > pure_by_words and pure_rus_words > pure_kz_words): 
        ...
        elif (tot_rus_words > tot_ukr_words and ((tot_rus_words > tot_by_words or ((tot_rus_words == tot_by_words and pure_by_words == 0)))) and tot_rus_words > tot_kz_words): 
            def_lang = MorphLang.RU
            ...
            def_lang = MorphLang.UA
        elif ((pure_kz_words > pure_rus_words and (((tot_kz_words + pure_kz_words) > tot_rus_words)) and pure_kz_words > pure_ukr_words) and pure_kz_words > pure_by_words): 
            ...
            while True:
                ...
                if (ui1.is_quot): 
                    ...
                    if ((p >= 2 and "БбТт".find(text[p - 1]) >= 0 and ((p + 3) < len(text))) and "ЕеЯяЁё".find(text[p + 1]) >= 0): 
                        ...
                        if (def_lang.equals(MorphLang.UA) or res[i].language.is_ua or res[i + 2].language.is_ua): 
        ...
        while True:
            ...
            ui00 = UnicodeInfo.get_char(mt.term[0])
            j = (mt.begin_char + 1)
            while j <= mt.end_char: 
                if (ui0.is_letter): 
                    break
    ...
    def correct_word_by_morph(self, word : str, lang : 'MorphLang', one_var : bool) -> typing.List[str]:
        if (LanguageHelper.is_cyrillic_char(word[0])): 
            if (lang is not None): 
                if (self.__m_engine_ru.language.is_ru and lang.is_ru): 
                    return self.__m_engine_ru.correct_word_by_morph(word, one_var)
                if (self.__m_engine_ua.language.is_ua and lang.is_ua): 
                    return self.__m_engine_ua.correct_word_by_morph(word, one_var)
                if (self.__m_engine_by.language.is_by and lang.is_by): 
                    return self.__m_engine_by.correct_word_by_morph(word, one_var)
                if (self.__m_engine_kz.language.is_kz and lang.is_kz): 
                    return self.__m_engine_kz.correct_word_by_morph(word, one_var)
            return self.__m_engine_ru.correct_word_by_morph(word, one_var)
        else: 
            return self.__m_engine_en.correct_word_by_morph(word, one_var)
    ...
    def __process_one_word(self, wstr : str, def_lang : 'MorphLang') -> typing.List['MorphWordForm']:
        ...
        if (lang.equals(MorphLang.EN)): 
            return self.__m_engine_en.process(wstr, False)
        if (def_lang.value.equals(MorphLang.RU)): 
            if (lang.is_ru): 
                return self.__m_engine_ru.process(wstr, False)
        if (lang.equals(MorphLang.RU)): 
            def_lang.value = lang
            return self.__m_engine_ru.process(wstr, False)
        if (def_lang.value.equals(MorphLang.UA)): 
            if (lang.is_ua): 
                return self.__m_engine_ua.process(wstr, False)
        if (lang.equals(MorphLang.UA)): 
            def_lang.value = lang
            return self.__m_engine_ua.process(wstr, False)
        if (def_lang.value.equals(MorphLang.BY)): 
            if (lang.is_by): 
                return self.__m_engine_by.process(wstr, False)
        if (lang.equals(MorphLang.BY)): 
            def_lang.value = lang
            return self.__m_engine_by.process(wstr, False)
        if (def_lang.value.equals(MorphLang.KZ)): 
            if (lang.is_kz): 
                return self.__m_engine_kz.process(wstr, False)
        if (lang.equals(MorphLang.KZ)): 
            def_lang.value = lang
            return self.__m_engine_kz.process(wstr, False)
        ru = None
        if (lang.is_ru): 
            ru = self.__m_engine_ru.process(wstr, False)
        ua = None
        if (lang.is_ua): 
            ua = self.__m_engine_ua.process(wstr, False)
        by = None
        if (lang.is_by): 
            by = self.__m_engine_by.process(wstr, False)
        kz = None
        if (lang.is_kz): 
            kz = self.__m_engine_kz.process(wstr, False)
        has_ru = False
        has_ua = False
        has_by = False
        has_kz = False
