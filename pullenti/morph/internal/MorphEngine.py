...
class MorphEngine:
    ...
    def calc_total_words(self) -> int:
        if (self.m_root is None): 
            return 0
        return self.__calc_words_count(self.m_root)
    
    def __calc_words_count(self, tn : 'MorphTreeNode') -> int:
        if (tn.lazy_pos > 0): 
            self.__load_tree_node(tn)
        res = 0
        if (tn.rule_ids is not None): 
            for id0_ in tn.rule_ids: 
                rule = self.get_rule(id0_)
                if (rule is not None): 
                    if (rule.morph_vars is not None): 
                        for v in rule.morph_vars: 
                            res += (len(v))
        if (tn.nodes is not None): 
            for kp in tn.nodes.items(): 
                res += self.__calc_words_count(kp[1])
        return res
    
    def process(self, word : str, ignore_no_dict : bool=False) -> typing.List['MorphWordForm']:
        ...
        if (res is not None): 
            for r in res: 
                if ((r.class0_.is_pronoun or r.class0_.is_noun or r.class0_.is_adjective) or (r.class0_.is_misc and r.class0_.is_conjunction) or r.class0_.is_preposition): 
                    ...
                    ok = False
                    for rr in res: 
                        if (rr != r and not rr.class0_.equals(r.class0_)): 
                            ok = True
                            break
                    ...
                    for mvref in tn0.reverce_variants: 
                        ...
                        for rr in res: 
                            if (rr.is_in_dictionary): 
                                if (rr.class0_.equals(mv.class0_) or rr.class0_.is_noun): 
                                    ok = True
                                    break
        ...
        if (mc.equals(MorphClass.VERB) and len(res) > 1): 
            for r in res: 
                if (r.undef_coef > (100) and r.class0_.equals(MorphClass.ADJECTIVE)): 
                    r.undef_coef = (0)
        if (len(res) == 0): 
            return None
        return res
        ...
        while True:
            ...
            while True:
                ...
                if (wf1.contains_attr("инф.", None)): 
                    continue
                if ((wf.class0_.equals(wf1.class0_) and wf.gender == wf1.gender and wf.number == wf1.number) and wf.normal_case == wf1.normal_case): 
                    cas |= wf1.case_
                    del res[j]
                    j -= 1
            if (not cas.equals(wf.case_)): 
                res[i].case_ = cas
        i = 0
        first_pass3624 = True
        while True:
            ...
            while True:
                ...
                if (wf1.contains_attr("инф.", None)): 
                    continue
                if ((wf.class0_.equals(wf1.class0_) and wf.case_.equals(wf1.case_) and wf.number == wf1.number) and wf.normal_case == wf1.normal_case): 
    ...
    def correct_word_by_morph(self, word : str, one_var : bool) -> typing.List[str]:
        vars0_ = list()
        tmp = io.StringIO()
        ch = 0
        while ch < len(word): 
            ...
            if (var is not None): 
                if (not var in vars0_): 
                    vars0_.append(var)
            ch += 1
        if (len(vars0_) == 0 or not one_var): 
            ch = 1
            while ch < len(word): 
                Utils.setLengthStringIO(tmp, 0)
                print(word, end="", file=tmp)
    ...
    def __compare(self, x : 'MorphWordForm', y : 'MorphWordForm') -> int:
        ...
        if (x.undef_coef > (0)): 
            ...
            if (((x.undef_coef) * 2) < y.undef_coef): 
                return 1
        if (not x.class0_.equals(y.class0_)): 
    ...
    def __comp1(self, r1 : 'MorphWordForm', r2 : 'MorphWordForm') -> bool:
        if (r1.number != r2.number or r1.gender != r2.gender): 
            return False
        if (not r1.case_.equals(r2.case_)): 
            return False
