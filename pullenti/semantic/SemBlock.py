...
class SemBlock(ISemContainer):
    ...
    def add_fragment_variantas(self, blk : 'SemBlock') -> None:
        for fr in blk.fragments: 
            for fr1 in self.fragments: 
                if (fr1.begin_char == fr.begin_char and fr1.end_char == fr.end_char): 
                    fr1.alt_fragments.append(fr)
                    fr.m_higher = self
                    break
    ...
    def add_link(self, typ : 'SemFraglinkType', src : 'SemFragment', tgt : 'SemFragment', ques : str=None) -> 'SemFraglink':
        for li in self.links: 
            if (li.typ == typ and li.source == src and li.target == tgt): 
                return li
        res = SemFraglink._new3611(typ, src, tgt, ques)
