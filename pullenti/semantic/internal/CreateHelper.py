...
class CreateHelper:
    ...
    def create_number(gr : 'SemGraph', num : 'NumbersWithUnitToken') -> 'SemObject':
        ...
        sem.morph.normal_case = mr.to_string_ex(True, None, 0)
        sem.morph.normal_full = sem.morph.normal_case
        sem.typ = SemObjectType.NOUN
        sem.measure = mr.kind
        i = 0
        first_pass4145 = True
        while True:
            if first_pass4145: first_pass4145 = False
            else: i += 1
    ...
    def create_verb_group(gr : 'SemGraph', vpt : 'VerbPhraseToken') -> 'SemObject':
        ...
        first_pass4146 = True
        while True:
            if first_pass4146: first_pass4146 = False
            else: i += 1
            ...
            if (v.is_adverb): 
                ...
                if (adv.typ != SemAttributeType.UNDEFINED): 
                    attrs.append(SemAttribute._new3551(adv.not0_, adv.typ, adv.spelling))
                    continue
            ...
            if (v.is_participle or v.is_dee_participle): 
                ...
                if (sem.morph.normal_case == sem.morph.normal_full and v.normal.endswith("Й")): 
                    ...
                    if (grs2 is not None): 
                        for g in grs2: 
                            for w in g.words: 
                                if (w.lang.equals(v.end_token.morph.language) and w.class0_.is_verb and not w.class0_.is_adjective): 
                    ...
                    if (sem.morph.normal_case == sem.morph.normal_full): 
                        ...
                        if (grs2 is not None): 
                            for g in grs2: 
                                for w in g.words: 
                                    if (w.lang.equals(v.end_token.morph.language) and w.class0_.is_verb and w.class0_.is_adjective): 
