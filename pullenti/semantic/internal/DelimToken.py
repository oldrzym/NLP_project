...
class DelimToken(MetaToken):
    ...
    def try_parse(t : 'Token') -> 'DelimToken':
        ...
        if (tok is not None): 
            if (t.is_value("ХОТЕТЬ", None) and (isinstance(t, TextToken))): 
                if (t.term != "ХОТЯ"): 
                    return None
    ...
    def initialize() -> None:
        ...
        t = None
        t = Termin._new92("НО", DelimType.BUT)
        t.add_variant("А", False)
        t.add_variant("ОДНАКО", False)
        t.add_variant("ХОТЯ", False)
        DelimToken.__m_onto.add(t)
        t = Termin._new92("ЕСЛИ", DelimType.IF)
        t.add_variant("В СЛУЧАЕ ЕСЛИ", False)
        DelimToken.__m_onto.add(t)
        t = Termin._new94("КОГДА", DelimType.IF, DelimToken.__m_onto)
        DelimToken.__m_onto.add(t)
        t = Termin._new92("ТО", DelimType.THEN)
        t.add_variant("ТОГДА", False)
        DelimToken.__m_onto.add(t)
        t = Termin._new92("ИНАЧЕ", DelimType.ELSE)
        t.add_variant("В ПРОТИВНОМ СЛУЧАЕ", False)
        DelimToken.__m_onto.add(t)
        t = Termin._new92("ТАК КАК", DelimType.BECAUSE)
        ...
        DelimToken.__m_onto.add(t)
        t = Termin._new92("ЧТОБЫ", DelimType.FOR)
        t.add_variant("ДЛЯ ТОГО ЧТОБЫ", False)
        DelimToken.__m_onto.add(t)
        t = Termin._new92("ЧТО", DelimType.WHAT)
        DelimToken.__m_onto.add(t)