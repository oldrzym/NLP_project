...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3560 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3560)
                pos = wrappos3560.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3564 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3564)
            pos = wrappos3564.value
            while cou > 0: 
                wrappos3562 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3562)
                pos = wrappos3562.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3561 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3561)
                    pos = wrappos3561.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3563 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3563)
            pos = wrappos3563.value
        self.__m_buf = wr
