...
class Sentence(object):
    ...
    def __create_lists(self, s : 'NGSegmentVariant') -> None:
        ...
        first_pass4165 = True
        while True:
            if first_pass4165: first_pass4165 = False
            else: i += 1
    ...
    def __create_links(self, s : 'NGSegmentVariant') -> None:
        ...
        first_pass4166 = True
        while True:
            if first_pass4166: first_pass4166 = False
            else: i += 1
            if (not (i < len(s.links))): break
            link0 = s.links[i]
            if (link0 is None): 
                ...
                if (li.to is not None and li.to.res_object is not None): 
                    ...
                    if (li.typ == NGLinkType.BE): 
                        if ((isinstance(li.from0_.source.source, NumbersWithUnitToken)) and li.to is not None): 
                            ...
                            continue
                        be = SemObject._new3572(gr, SemObjectType.VERB)
    ...
    def __create_result(self, blk : 'SemBlock') -> None:
                ...
                if (isinstance(it.source, NounPhraseToken)): 
                    ...
                    if (npt.multi_nouns and it.result.quantity is None): 
                        ...
                        while i < len(npt.adjectives): 
                            so = SemObject._new3572(it.res_graph, it.result.typ)
        ...
        first_pass4167 = True
        while True:
            if first_pass4167: first_pass4167 = False
            else: i += 1
        ...
        for it in self.items: 
            if (it.result is not None and it.typ == SentItemType.VERB and (isinstance(it.source, VerbPhraseToken))): 
                if (agent is not None): 
                    ...
                    for li in it.res_graph.links: 
                        if (li.typ == SemLinkType.PACIENT and li.source == it.result): 
                            ...
                            break
                    if (not has_pac): 
                        ni0 = NGItem._new3565(agent)
                        gli0 = NGLink._new3578(ni0, Utils.asObjectOrNull(it.source, VerbPhraseToken), NGLinkType.PACIENT)
                ...
                if (agent is None): 
                    continue
                ni = NGItem._new3565(agent)
                gli = NGLink._new3578(ni, Utils.asObjectOrNull(it.source, VerbPhraseToken), NGLinkType.AGENT)
                if (agent.result_list is not None): 
        ...
        first_pass4168 = True
        while True:
            if first_pass4168: first_pass4168 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
            ...
            if (link is None): 
                ...
                first_pass4169 = True
                while True:
                    if first_pass4169: first_pass4169 = False
                    else: j += 1
    ...
    def add_to_block_variants(self, blk : 'SemBlock') -> None:
        if (self.res_block is not None): 
            for fr in self.res_block.fragments: 
                fr.rank = self.coef
            blk.add_fragment_variantas(self.res_block)
    ...
    def parse_variants(t0 : 'Token', t1 : 'Token', lev : int, max_count : int=0, regime : 'SentItemType'=SentItemType.UNDEFINED) -> typing.List['Sentence']:
        ...
        first_pass4170 = True
        while True:
            if first_pass4170: first_pass4170 = False
            else: t = t.next0_
            ...
            for it in items_: 
                nexts = None
                wrapnexts3581 = RefOutArgWrapper(None)
                inoutres3582 = Utils.tryGetValue(m_nexts, it.end_token.end_char, wrapnexts3581)
                nexts = wrapnexts3581.value
                if (not inoutres3582): 
    ...
    def calc_coef(self, no_result : bool) -> None:
        ...
        first_pass4171 = True
        while True:
            if first_pass4171: first_pass4171 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
    ...
    @staticmethod
    def _new3595(_arg1 : 'NGLinkType') -> 'Sentence':
        res = Sentence()
